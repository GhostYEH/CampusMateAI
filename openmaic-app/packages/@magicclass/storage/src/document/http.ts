import { migrate, validateScene, validateStage } from '@magicclass/dsl';
...
> implements DocumentStore<TScene, TStage> {
  ...
  constructor(options: HttpDocumentStoreOptions) {
    if (options.baseUrl === '') {
      throw new Error('@magicclass/storage: HttpDocumentStore baseUrl must be non-empty');
    }
    ...
    if (typeof selectedFetch !== 'function') {
      throw new Error('@magicclass/storage: HttpDocumentStore requires a fetch implementation');
    }
    ...
    if (!response.ok) {
      ...
      const message =
        typeof errorBody?.error?.message === 'string'
          ...
          : `@magicclass/storage: DocumentStore HTTP request failed with status ${response.status}`;
      if (
        response.status === 409 &&
        code === 'FUTURE_VERSION' &&
  ...
  async listDocuments(): Promise<DocumentSummary[]> {
    ...
    if (!Array.isArray(summaries)) {
      throw new HttpDocumentStoreError(
        200,
        'MALFORMED_RESPONSE',
        '@magicclass/storage: DocumentStore HTTP listDocuments response must be an array',
      );
    }
