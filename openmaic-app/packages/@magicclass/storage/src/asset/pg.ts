...
function registryConfigurationFailure(problem: string): Error {
  return new Error(`@magicclass/storage: ${problem}`);
}

function registryFailure(operation: string): Error {
  return new Error(`@magicclass/storage: asset registry ${operation} failed`);
}
...
export class PgAssetStore implements AssetStore {
  ...
  constructor(
    ...
    if (typeof options?.withTransaction !== 'function') {
      throw new Error(
        '@magicclass/storage: withTransaction is required and must pin a fresh connection and transaction for every call',
      );
    }
    if (!options.byteStore) {
      throw new Error('@magicclass/storage: byteStore is required for PgAssetStore');
    }
    if (
      options.quotaBytes !== undefined &&
      (!Number.isSafeInteger(options.quotaBytes) || options.quotaBytes < 0)
    ) {
      throw new Error('@magicclass/storage: quotaBytes must be a non-negative safe integer');
    }
    if (
      options.pendingTtlMs !== undefined &&
      (!Number.isSafeInteger(options.pendingTtlMs) || options.pendingTtlMs < 1)
    ) {
      throw new Error('@magicclass/storage: pendingTtlMs must be a positive safe integer');
    }
