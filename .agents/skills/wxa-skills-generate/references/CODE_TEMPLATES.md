...
async function queryDrugUsage({ name }) {
  ...
  return {
    ...
    handoff: ({ result }) => ({
      query: `drugId=${result.structuredContent.drugId}`,   // string，页面 query 字符串，宜短
...
> 形态选择：无需模型改动 result → 用**对象**（立即模式，更快）；需要用模型修改后的 result 拼 query/payload → 用**函数**（延迟模式，入参 `{ result }`，`result` 为模型修改后的完整 result）。`handoff.query` 为 **string**（如 `'drugId=xxx'`、`'orderId=1&spec=large'`），原样注入 `onLoad(query)` 与 `wx.onAgentHandoff` 回调。
