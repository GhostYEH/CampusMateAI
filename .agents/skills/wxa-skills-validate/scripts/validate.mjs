...
import { runCli, DEFAULT_CLI_PATH, screenDestructiveCandidates } from "./lib.mjs";
...
const BUILTIN_RULES = {
  ...
  crossFileRules: [
    ...
    { id: "V019", name: "敏感接口初筛待模型判断", stage: "registration", level: "error" },
  ],
};
...
const CROSS_CHECKERS = {
  ...
  V019: checkV019_DestructiveMarking,
};
    ...
    for (const item of parseMcpApis(mcp)) {
      ...
      if (missing.length === 0) {
        ...
        out.push(fail(rule, `接口 '${name}' handoff.query 传了 [${queryKeys.join(", ")}]，但接力页 '${ppStr}' 中未引用 [${missing.join(", ")}]——参数名不匹配或页面忽略了该参数，用户点卡片进接力页时 handoff 意图丢失`, {
          file: relative(skillsPath, found.path),
          fix: `读接力页 ${ppStr}.js 的 onLoad 确认其读取的参数名，将 apis/${name}.js 中 handoff.query 的参数名改为页面实际读取的名称；或确认页面无需该参数时改为 handoff: { query: '' }`,
        }));
      }
    }
  }
...
async function checkV019_DestructiveMarking(rule, _skillsPath, ctx = {}) {
  const out = [];
  if (!ctx.projectRoot) return out;
  let candidates;
  try { candidates = screenDestructiveCandidates(ctx.projectRoot); }
  catch { candidates = []; }

  const manifestPath = join(ctx.projectRoot, "cli-agent-run", "destructive-manifest.json");

  // 读现有 manifest，保留模型已做的判断
  const existing = {};
  try {
    const old = JSON.parse(await readFile(manifestPath, "utf-8"));
    for (const a of (old.apis || [])) existing[`${a.skill}/${a.apiName}`] = a;
  } catch {}

  // 合并：脚本初筛候选 + 保留模型判断
  const apis = candidates.map(c => {
    const key = `${c.skill}/${c.apiName}`;
    const old = existing[key];
    if (old && typeof old.destructive === "boolean") {
      return { skill: c.skill, apiName: c.apiName, hitKeyword: c.hitKeyword, hitField: c.hitField, destructive: old.destructive, destructiveReason: old.destructiveReason || "" };
    }
    return { skill: c.skill, apiName: c.apiName, hitKeyword: c.hitKeyword, hitField: c.hitField, destructive: null, destructiveReason: "" };
  });

  const manifest = { generatedAt: new Date().toISOString(), project: ctx.projectRoot, apis };
  try {
    const manifestDir = join(ctx.projectRoot, "cli-agent-run");
    await mkdir(manifestDir, { recursive: true });
    await writeFile(manifestPath, JSON.stringify(manifest, null, 2), "utf-8");
  } catch {}

  if (candidates.length === 0) {
    out.push(pass(rule, `${rule.name}: 无接口命中敏感关键词，manifest 已落盘（空清单）`));
    return out;
  }
  const pending = apis.filter(a => a.destructive === null);
  if (pending.length > 0) {
    for (const a of pending) {
      out.push(fail(rule, `接口 '${a.apiName}'(${a.skill}) 的 ${a.hitField} 命中敏感关键词 '${a.hitKeyword}'，待模型判断是否真敏感——编辑 cli-agent-run/destructive-manifest.json 将 destructive 填为 true（真敏感，execute 跳过）或 false（误判，execute 放行）并填 destructiveReason`, {
        file: `${a.skill}/mcp.json`,
        fix: `编辑 cli-agent-run/destructive-manifest.json：destructive=true+destructiveReason（真敏感）或 destructive=false+destructiveReason（误判如只读查询）`,
      }));
    }
    return out;
  }
  const t = apis.filter(a => a.destructive === true).length;
  const f = apis.filter(a => a.destructive === false).length;
  out.push(pass(rule, `${rule.name}: ${apis.length} 个候选已全部判断（true=${t}, false=${f}），manifest 已落盘`));
  return out;
}
...
function extractHandoffQueryKeys(js) {
  ...
  const afterQuery = handoffBody.slice(qIdx);
  const strMatch = afterQuery.match(/['"`]([^'"`]+)['"`]/);
  if (!strMatch) return keys;
  const queryString = strMatch[1].trim();
  if (!queryString) return keys;
...
async function validate(projectPath, opts = {}) {
  ...
  const projectLevelRuleIds = new Set(["V016", "V019"]);
  for (const skillDir of skillDirs) {
    const pkgRoot = packageRootOf(skillDir);
