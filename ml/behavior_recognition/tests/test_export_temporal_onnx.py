...
def _write_frame_encoder(path: Path) -> None:
    input_info = helper.make_tensor_value_info("input", TensorProto.FLOAT, [1, 3, 4, 4])
...
def test_fused_temporal_onnx_matches_frame_encoder_plus_gru(tmp_path: Path):
    ...
    output = session.run(["logits"], {"frames": np.zeros((1, 2, 3, 4, 4), np.float32)})[0]
    parity = json.loads((tmp_path / "export" / "parity.json").read_text(encoding="utf-8"))

    assert input_shape == [1, 2, 3, 4, 4]
