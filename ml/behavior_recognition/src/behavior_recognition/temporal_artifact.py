...
def _graph_tensor_metadata(
    value: onnx.ValueInfoProto | None,
...
def _runtime_tensor_metadata(
    ...
    if input_count != 1 or output_count != 1:
        input_name, input_shape = _graph_tensor_metadata(
            graph_inputs[0] if graph_inputs else None
        )
        output_name, output_shape = _graph_tensor_metadata(
            graph_outputs[0] if graph_outputs else None
        )
        return (
            input_count,
            input_name,
    ...
    actual_input = inputs[0] if input_count == 1 else None
    actual_output = outputs[0] if output_count == 1 else None
    return (
        input_count,
        actual_input.name if actual_input is not None else "",
        list(actual_input.shape) if actual_input is not None else [],
        output_count,
        actual_output.name if actual_output is not None else "",
        list(actual_output.shape) if actual_output is not None else [],
    )
...
def audit_temporal_artifact(
    ...
    (
        ...
        output_name,
        output_shape,
    ) = _runtime_tensor_metadata(model_path)
    ...
    if input_count != 1:
        failures.append("input_count")
    if input_name != EXPECTED_INPUT_NAME or card_input.get("name") != input_name:
    ...
    if output_count != 1:
        failures.append("output_count")
    if output_name != EXPECTED_OUTPUT_NAME or card_output.get("name") != output_name:
    ...
    if not isinstance(provenance, Mapping):
        ...
        if (
            not isinstance(provenance.get("seed"), int)
