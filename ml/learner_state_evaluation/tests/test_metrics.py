...
def test_binary_metrics_and_per_hypothesis_counts_are_hand_checkable() -> None:
    annotations = [
        annotation("1", "pointer", True),
        annotation("2", "pointer", False),
        annotation("3", "pointer", False),
        annotation("4", "loop", True),
        annotation("5", "loop", True),
        annotation("6", "loop", False),
    ]
    predictions = [
    ...
    assert report["per_hypothesis"]["pointer"]["counts"] == {
        "tp": 1,
        "fp": 1,
        "tn": 1,
        "fn": 0,
...
def test_brier_and_ece_use_positive_class_score() -> None:
    annotations = [annotation("1", "pointer", True), annotation("2", "pointer", False)]
...
def test_zero_denominators_are_reported_as_zero() -> None:
    annotations = [annotation("1", "pointer", False)]
...
def test_positive_prediction_with_single_attempt_counts_as_unsupported_assertion() -> None:
    annotations = [annotation("1", "pointer", False, sufficient=False)]
