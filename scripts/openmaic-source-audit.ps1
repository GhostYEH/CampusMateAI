...
function Resolve-NodeExecutable {
    ...
    throw 'node is required to run the OpenMAIC source audit but was not found on PATH'
}

$auditScript = Join-Path $PSScriptRoot 'openmaic-source-audit.mjs'
if (-not (Test-Path -LiteralPath $auditScript -PathType Leaf)) {
    throw "OpenMAIC source audit implementation is missing: $auditScript"
}
