...
import { audit, buildNotice, capabilityMapping } from '../magicclass-source-audit.mjs';

const repoRoot = new URL('../..', import.meta.url).pathname.replace(/^\//, '').replaceAll('/', '\\');
const wrapper = join(repoRoot, 'scripts', 'magicclass-source-audit.ps1');
...
function makeFixture() {
  const root = mkdtempSync(join(tmpdir(), 'magicclass-audit-fixture-'));
  for (const directory of [
    'app/api/health',
    'packages/@magicclass/dsl/src',
  ...
  writeFileSync(join(root, 'packages/@magicclass/dsl/src/stage.ts'), 'export type SceneType = "slide" | "quiz" | "interactive" | "pbl";\n');
  ...
  run('git', ['config', 'user.name', 'magic class audit test'], root);
  run('git', ['add', '.'], root);
...
function makeOutput() {
  return join(mkdtempSync(join(tmpdir(), 'magicclass-audit-output-')), 'audit');
}
...
test('honours an overridden repository and tag in the notice', () => {
  ...
  audit({
    ...
    repository: 'https://example.invalid/magicclass.git',
    tag: 'v9.9.9',
  });
  const notice = normalize(readFileSync(join(output, 'NOTICE.md'), 'utf8'));
...
test('capability mapping classifies the audited surfaces', () => {
  ...
  assert.equal(capabilityMapping('packages/@magicclass/dsl/src/stage.ts'), 'dsl');
