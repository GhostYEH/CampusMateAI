#!/usr/bin/env node
/**
 * magic class source audit.
 *
 * Walks a pinned magic class checkout, hashes every auditable file and emits the
 * provenance artefacts under `third_party/magicclass/`: the provenance notice, the
 ...
 *
 * Usage:
 *   node scripts/magicclass-source-audit.mjs \
...
export function capabilityMapping(relativePath) {
  ...
  if (/^packages\/@magicclass\/dsl\//.test(relativePath)) return 'dsl';
  if (/^packages\/@magicclass\/(renderer|editor)\//.test(relativePath)) return 'editor/player';
  if (/^packages\/@magicclass\/(importer|storage)\//.test(relativePath)) return 'import/export';
  if (/^packages\/@magicclass\/generation\//.test(relativePath)) return 'generation';
...
export function audit({ source, output, expectedCommit, repository = DEFAULT_REPOSITORY, tag = DEFAULT_TAG }) {
  ...
  if (lstatSync(licensePath, { throwIfNoEntry: false })?.isFile() !== true) {
    throw new Error('magic class LICENSE is missing');
  }
