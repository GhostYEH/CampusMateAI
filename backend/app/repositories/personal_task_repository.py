...
class PersonalTaskRepository:
    ...
    def list_chaoxing_for_event_backfill(
        ...
        *,
        ...
        with self._db.query() as conn:
            total = int(
                conn.execute(f"SELECT COUNT(*) AS n FROM personal_tasks{where}", params)
                .fetchone()["n"]
            )
            rows = conn.execute(
                f"SELECT * FROM personal_tasks{where} ORDER BY user_id ASC, id ASC LIMIT ? OFFSET ?",
                params + [page_size, offset],
            ).fetchall()
        return [PersonalTaskRow.from_row(row) for row in rows], total
