...
class LearningPlanRepository:
            ...
            conn.execute(
                """INSERT INTO learning_plan_runs
                   (run_id,user_id,planner_version,input_digest,as_of,valid_until,available_minutes,
                    allocated_minutes,course_scope,window_start,window_end,warning_codes_json,idempotency_key,created_at)
                   VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?)""",
                (run["run_id"], user_id, run["planner_version"], run["input_digest"], run["as_of"],
                 ...
                 json.dumps(run.get("warning_codes", []), ensure_ascii=False), run.get("idempotency_key"), now),
            )
            conn.execute(
                "INSERT INTO learning_plans(plan_id,run_id,user_id,status,llm_summary,created_at) VALUES (?,?,?,?,?,?)",
                (plan_id, run["run_id"], user_id, "PROPOSED", None, now),
            )
            for item in items:
    ...
    def get_plan(self, plan_id: str, *, user_id: str) -> LearningPlanRow | None:
        ...
        return LearningPlanRow(
            plan_id=plan_id, run=run, user_id=row["user_id"], status=row["status"],
            ...
            row = conn.execute(
                """SELECT p.plan_id FROM learning_plans p JOIN learning_plan_runs r ON r.run_id=p.run_id
                   WHERE p.user_id=? AND r.planner_version=? AND r.input_digest=?
                     AND r.valid_until>? AND p.status != 'REJECTED'
    ...
    def update_status(self, *, plan_id: str, user_id: str, status: str) -> LearningPlanRow | None:
        with self._db.transaction() as conn:
    ...
    def create_action(self, *, plan_id: str, item_id: str, user_id: str, action_type: str) -> LearningPlanActionRow:
        now = _now()
        with self._db.transaction() as conn:
            conn.execute(
    ...
    def finish_action(self, *, action_id: str, status: str, target_task_id: str | None = None, error_code: str | None = None) -> None:
        with self._db.transaction() as conn:
            conn.execute(
                "UPDATE learning_plan_execution_actions SET status=?,target_task_id=?,error_code=?,completed_at=? WHERE action_id=?",
                (status, target_task_id, error_code, _now(), action_id),
            )

    def mark_item(self, *, item_id: str, status: str) -> None:
        with self._db.transaction() as conn:
