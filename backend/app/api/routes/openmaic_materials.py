...
- **intake policy** — the file is size-checked, its filename is validated as a
  *name* (never a path) and its text is extracted here, in
  :mod:`...services.openmaic.material_extraction`. The browser never chooses the
...
from ...services.openmaic.material_extraction import (
    MAX_MATERIAL_BYTES,
    MAX_REFERENCE_COUNT,
    extract_material,
)
...
router = APIRouter(prefix="/courses", tags=["openmaic-materials"])
