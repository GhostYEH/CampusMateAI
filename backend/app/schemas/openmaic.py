...
class OpenMAICMaterialOut(BaseModel):
...
class OpenMAICPlanOut(BaseModel):
    ...
    # 形态只是"意图"，最终内容组合由 OpenMAIC 生成器决定
    intent_note: str = "内容形态只是生成意图，最终包含哪些形式由生成器根据课程内容决定。"
    materials: List[OpenMAICMaterialOut] = Field(default_factory=list)
...
class OpenMAICSessionOut(BaseModel):
    ...
    # **浏览器公开**地址：由 public_url 按当前 OPENMAIC_EMBED_ORIGIN 现场投影。
    ...
    # 终态语义：terminal / retryable / partial（不创造 OpenMAIC 不存在的状态值）
    terminal: bool = False
    ...
    @classmethod
    def from_session(cls, s, *, settings) -> "OpenMAICSessionOut":
        """`settings` 是**必填**关键字参数。

        刻意不给默认值：任何新增的下发路径若忘记传入配置，会立刻 `TypeError`，
        而不是静默把数据库里的原始 URL 泄漏出去。
        """
        from ..services.openmaic.public_url import project_session_url
...
class OpenMAICSceneCountOut(BaseModel):
...
class OpenMAICWidgetCountOut(BaseModel):
...
class OpenMAICCompositionOut(BaseModel):
    ...
    classroom_id: str
    scene_total: int = 0
    scenes: List[OpenMAICSceneCountOut] = Field(default_factory=list)
    widget_types: List[OpenMAICWidgetCountOut] = Field(default_factory=list)
...
class OpenMAICClassroomOut(BaseModel):
    session_id: str
    classroom_id: Optional[str] = None
    # **浏览器公开**地址（按 OPENMAIC_EMBED_ORIGIN 现场构造）。
    ...
    composition: Optional[OpenMAICCompositionOut] = None
...
__all__ = [
    ...
    "OpenMAICSceneCountOut",
    "OpenMAICWidgetCountOut",
    "MODE_INTENT_LABELS",
    "VALID_MODES",
