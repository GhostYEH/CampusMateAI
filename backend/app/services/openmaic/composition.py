"""读取并统计 OpenMAIC 课堂的**真实**组成。

为什么需要它：OpenMAIC 的生成接口只接受一个 `requirement` 字符串，**没有任何
...
# 参考实现里真实存在的场景类型（packages/@openmaic/dsl/src/stage.ts）
KNOWN_SCENE_TYPES = ("slide", "quiz", "interactive", "pbl")

# interactive 内部真实存在的 widget 形态（packages/@openmaic/dsl/src/interactive.ts）
KNOWN_WIDGET_TYPES = (
    "simulation",
    "diagram",
    "code",
...
class ClassroomComposition:
    """真实课堂组成的稳定 DTO（与 `schemas/openmaic.py` 一一对应）。"""
