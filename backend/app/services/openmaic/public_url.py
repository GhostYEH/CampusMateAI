...
背景：生产部署下 OpenMAIC 的**内部**服务地址与**浏览器公开**地址必然不同。
...
# 与 OpenMAIC 的 `isValidClassroomId` 保持一致（/^[a-zA-Z0-9_-]+$/），另加长度上限
...
def resolve_public_classroom_url(settings: Any, classroom_id: Any) -> Optional[str]:
    ...
    return build_public_classroom_url(
        public_origin=getattr(settings, "openmaic_public_origin", "") or "",
...
def is_internal_url_leaked(url: Optional[str], settings: Any) -> bool:
    ...
    if not url:
        return False
    internal = getattr(settings, "openmaic_origin", "") or ""
    if not internal:
        return False
