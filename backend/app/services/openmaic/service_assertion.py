...
def _resolve_secret(secret: Optional[str]) -> str:
    ...
    try:
        ...
        configured = get_settings().openmaic_internal_secret
    except (ImportError, AttributeError):
        configured = os.getenv("OPENMAIC_INTERNAL_SECRET", "")
    if not configured:
        raise ServiceAssertionError("OPENMAIC_INTERNAL_SECRET is not configured")
    return configured
