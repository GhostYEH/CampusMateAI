...
为什么需要：`OpenMAICClient` 会把上游响应里的 `error` / `message` 原样取出来，
最终经由 `OpenMAICSessionOut.error` / `.message` 下发到学生客户端。上游文本**不可信**：

    {"success": false, "error": "connect ECONNREFUSED http://openmaic:3000/api/classroom"}
...
_INTERNAL_HOST_RE = re.compile(
    r"https?://[A-Za-z0-9._-]*openmaic[A-Za-z0-9._-]*(?::\d+)?[^\s\"'<>]*",
    re.IGNORECASE,
)
...
#: 兜底：形如 `openmaic_access=<value>` 的 cookie 名
_COOKIE_NAME_RE = re.compile(r"(?i)\bopenmaic_access\s*=\s*\S+")
...
def redact_public_text(text: Optional[Any], settings: Any = None) -> Optional[str]:
    ...
    if not out:
        return ""
    secrets = [
        getattr(settings, "openmaic_access_code", "") if settings is not None else "",
        getattr(settings, "openmaic_base_url", "") if settings is not None else "",
        getattr(settings, "openmaic_origin", "") if settings is not None else "",
        getattr(settings, "openmaic_public_origin", "") if settings is not None else "",
    ]
