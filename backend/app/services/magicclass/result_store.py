...
class MagicClassSession:
    ...
    # 部分完成：succeeded 但生成器只产出少于预期的场景（不创造 magic class 没有的状态值）
    partial: bool = False
    # 稳定错误码（对应 magic class 侧 error / 我方归一化结果），用于客户端分支
    ...
    @classmethod
    def from_dict(cls, data: Dict[str, Any]) -> "MagicClassSession":
...
class MagicClassReservation:
    ...
    @classmethod
    def from_dict(cls, data: Dict[str, Any]) -> Optional["MagicClassReservation"]:
...
class MagicClassResultStore:
    ...
    def save(self, session: MagicClassSession) -> None:
    ...
    def _read_session_file(self, path: Path, *, user_id: str, course_id: str) -> Optional[MagicClassSession]:
        ...
        if data.get("user_id") != user_id or data.get("course_id") != course_id:
            return None
        return MagicClassSession.from_dict(data)
    ...
    def list_sessions(self, *, user_id: str, course_id: str) -> List[MagicClassSession]:
        ...
        sessions: List[MagicClassSession] = []
    ...
    def list_user_sessions(self, *, user_id: str) -> List[MagicClassSession]:
    ...
    def read_reservation(
    ...
    ) -> Optional[MagicClassReservation]:
        ...
        if not isinstance(data, dict):
            return None
        return MagicClassReservation.from_dict(data)
    ...
    def _acquire_reservation_unlocked(
        ...
        *,
        ...
        path = self._reservation_path(user_id, course_id)
        path.parent.mkdir(parents=True, exist_ok=True)
        reservation = MagicClassReservation(session_id=session_id, mode=mode)
    ...
    def steal_reservation(
        ...
        *,
        ...
        session_id: str,
        mode: str,
        expected: MagicClassReservation,
    ) -> bool:
