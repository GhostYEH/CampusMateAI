...
受管服务自身的归属、去重与引用解析由 ``magicclass-service/tests/materials.test.mjs``
覆盖,这里只测网关。
"""
...
def _setup(script=None, **settings_overrides):
    ...
    app = create_app()
    app.dependency_overrides[magicclass_materials._client] = lambda: client
    http = TestClient(app)
    login = http.post(
...
def test_an_oversized_upload_is_refused_before_reaching_upstream():
    ...
    assert response.json()["code"] == "MAGICCLASS_DOCUMENT_REJECTED"
    assert transport.calls == []
...
def test_delete_requires_if_match_and_translates_a_stale_revision_to_409():
    ...
    assert stale.json()["code"] == "MAGICCLASS_REVISION_CONFLICT"
...
def test_the_fusion_switch_answers_503_rather_than_an_empty_list():
    _, _, http, headers, course_id = _setup(magicclass_fusion_enabled=False)
    response = http.get(f"/api/v1/courses/{course_id}/materials", headers=headers)
