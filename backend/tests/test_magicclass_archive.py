...
from app.api.routes import magicclass_archive
...
def _setup(script=None, **settings_overrides):
    ...
    app = create_app()
    app.dependency_overrides[magicclass_archive._client] = lambda: client
    http = TestClient(app)
    login = http.post(
...
def test_an_oversize_archive_is_refused_before_reaching_upstream():
    _, transport, http, headers, course_id = _setup()
    response = _import(http, headers, course_id, content=b"x" * (magicclass_archive.MAX_ARCHIVE_BYTES + 1))
    assert response.status_code == 400
    assert transport.calls == []
