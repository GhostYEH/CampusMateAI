...
覆盖：
- 内部 `OPENMAIC_BASE_URL` 与浏览器 `OPENMAIC_EMBED_ORIGIN` 分离，公开 Origin 不得是内部地址；
...
全部通过 `httpx.MockTransport` 模拟 OpenMAIC，不伪造真实生成结果。
"""
...
INTERNAL = "http://openmaic.internal:3000"
...
def _client(handler, **overrides) -> OpenMAICClient:
...
def _service(tmp_path, handler, **overrides) -> OpenMAICClassroomService:
    settings = _test_settings(**overrides)
    store = OpenMAICResultStore(tmp_path / "rooms")
    client = _client(handler, **overrides)
    return OpenMAICClassroomService(settings, store, client=client)
...
def test_embed_origin_accepts_distinct_public_origin():
    s = _test_settings()
    assert s.openmaic_embed_origin == PUBLIC
    assert s.openmaic_origin == INTERNAL
...
def test_embed_origin_rejects_credentials_query_fragment_and_path():
    for bad in (
        ...
        "https://classroom.example.com/openmaic",
        "ftp://classroom.example.com",
        "classroom.example.com",
    ):
        with pytest.raises(ValueError):
            _test_settings(openmaic_embed_origin=bad)
...
def test_embed_origin_rejects_unspecified_and_link_local_in_any_env():
    for bad in ("http://0.0.0.0:3000", "http://169.254.169.254", "http://[::]"):
        with pytest.raises(ValueError):
            _test_settings(openmaic_embed_origin=bad)
        with pytest.raises(ValueError):
            _production_settings(openmaic_embed_origin=bad)
...
def test_embed_origin_allows_loopback_in_development():
    s = _test_settings(openmaic_embed_origin="http://127.0.0.1:3000")
    assert s.openmaic_embed_origin == "http://127.0.0.1:3000"


def test_embed_origin_must_be_https_in_production():
    with pytest.raises(ValueError):
        _production_settings(openmaic_embed_origin="http://classroom.example.com")


def test_allowed_versions_spec_validated_on_startup():
    with pytest.raises(ValueError):
        _test_settings(openmaic_allowed_versions="not-a-range")
    assert _test_settings(openmaic_allowed_versions=">=1.0.0 <2.0.0").openmaic_allowed_versions


def test_new_openmaic_limits_are_validated():
    with pytest.raises(ValueError):
        _test_settings(openmaic_health_timeout_seconds=0)
    with pytest.raises(ValueError):
        _test_settings(openmaic_probe_ttl_seconds=-1)
    with pytest.raises(ValueError):
        _test_settings(openmaic_poll_interval_ms=10)
    with pytest.raises(ValueError):
        _test_settings(openmaic_poll_max_seconds=1)


def test_external_3d_defaults_available_and_can_be_disabled():
    assert _test_settings().openmaic_external_3d_available is True
    assert _test_settings(openmaic_external_3d_available=False).openmaic_external_3d_available is False


# ===== C. 契约指纹 =====


def test_probe_passes_against_real_openmaic_contract(tmp_path):
...
def test_version_out_of_range_is_recorded_but_never_fatal(tmp_path):
    ...
    OpenMAIC 的 Docker 镜像不经过 npm 脚本启动，`npm_package_version` 取不到，
...
def test_operator_switches_and_health_intersect(tmp_path):
    ...
    status = asyncio.run(
        _service(tmp_path, handler, openmaic_enable_image_generation=False).status()
    )
...
def test_status_probe_cache_expires(tmp_path):
    rec: List[httpx.Request] = []
    service = _service(tmp_path, _contract_handler(rec), openmaic_probe_ttl_seconds=0)
    import asyncio
...
def _url_client() -> OpenMAICClient:
    return OpenMAICClient(base_url=INTERNAL, origin="https://classroom.example.com:8443")
