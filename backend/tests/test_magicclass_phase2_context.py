...
复用 `test_magicclass_student_integration` 的脚手架（同一测试目录，pytest 会把它
加入 sys.path），因此本文件只写行为断言，不重复实现假 magic class。
"""
...
def test_plan_endpoint_creates_no_generation_task(tmp_path):
    ...
    assert _submits(recorder) == [], "生成前计划不得创建任何 magic class 任务"
...
def test_context_is_length_limited_and_marked(tmp_path):
    ...
    _, tc, headers, _ = _bootstrap(
        tmp_path, _recording_handler(recorder), magicclass_course_context_max_chars=30
    )
    cid = _first_course(tc, headers)
...
def test_adaptive_never_recommends_visualization_when_3d_switch_is_off(tmp_path):
    recorder: List[httpx.Request] = []
    _, tc, headers, _ = _bootstrap(
        tmp_path, _recording_handler(recorder), magicclass_external_3d_available=False
    )
    cid = _first_course(tc, headers)
    for _ in range(3):
