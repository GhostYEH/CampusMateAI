"""`GET /api/v1/magicclass/fusion/recent` 的聚合与隔离测试。
...
会话直接写进 result_store，不伪造 magic class 生成过程；需要走 HTTP 的用例
复用真实登录与真实课程可见性策略。
"""
...
def _session(*, user_id: str, course_id: str, session_id: str, mode="adaptive",
             status="succeeded", updated_at="2026-01-01T00:00:00+00:00") -> MagicClassSession:
...
def _setup(tmp_path):
    ...
    store = MagicClassResultStore(tmp_path / "classrooms")
    container.magicclass_result_store = store
    container.magicclass_classroom_service = MagicClassClassroomService(container.settings, store)
...
def test_recent_aggregates_across_courses_in_updated_at_order(tmp_path):
    ...
    body = client.get("/api/v1/magicclass/fusion/recent", headers=headers).json()
...
def test_recent_items_carry_a_native_deep_link(tmp_path):
    ...
    item = client.get("/api/v1/magicclass/fusion/recent", headers=headers).json()["items"][0]
...
def test_recent_honours_the_limit_and_reports_more(tmp_path):
    ...
    body = client.get("/api/v1/magicclass/fusion/recent?limit=2", headers=headers).json()
...
def test_recent_rejects_a_limit_above_the_ceiling(tmp_path):
    ...
    assert client.get("/api/v1/magicclass/fusion/recent?limit=51", headers=headers).status_code == 422
    assert client.get("/api/v1/magicclass/fusion/recent?limit=0", headers=headers).status_code == 422
...
def test_recent_only_returns_finished_classrooms(tmp_path):
    ...
    ids = [item["id"] for item in
           client.get("/api/v1/magicclass/fusion/recent", headers=headers).json()["items"]]
    assert ids == ["done"]


def test_recent_requires_authentication(tmp_path):
    _, _, client, _, _, _ = _setup(tmp_path)
    assert client.get("/api/v1/magicclass/fusion/recent").status_code == 401
...
def test_service_excludes_courses_the_resolver_rejects(tmp_path):
    ...
    service = MagicClassClassroomService(_test_settings(), store)
