...
from app.api.routes import openmaic_generation, openmaic_tts
...
def test_tts_route_forwards_course_bound_text_and_answers_202():
    ...
    http.app.dependency_overrides[openmaic_tts._client] = lambda: _client(container, transport)
...
def test_tts_requires_idempotency_before_contacting_service():
    ...
    http.app.dependency_overrides[openmaic_tts._client] = lambda: OpenMAICFusionClient(
...
def test_artifact_route_decodes_base64_envelope_into_audio_bytes():
    ...
    http.app.dependency_overrides[openmaic_generation._client] = lambda: _client(container, transport)
