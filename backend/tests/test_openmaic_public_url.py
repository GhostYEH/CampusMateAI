...
生产部署形态：
    ...
    OPENMAIC_BASE_URL   = http://openmaic:3000          （内部，Docker 网络）
    OPENMAIC_EMBED_ORIGIN = https://classroom.example.edu （浏览器公开）
...
def _handler(
    ...
    *,
    ...
    """假 OpenMAIC：health / access-code / 探针 / 提交 / 轮询，返回**内部**课堂 URL。"""
...
def _bootstrap(tmp_path, handler, **overrides) -> Tuple[ServiceContainer, TestClient, Dict[str, str], OpenMAICResultStore]:
...
def test_success_response_uses_public_origin_not_internal(tmp_path):
    ...
    assert "openmaic:3000" not in json.dumps(body, ensure_ascii=False)
...
def test_no_response_anywhere_leaks_internal_address(tmp_path):
    ...
    for path in (tmp_path / "openmaic_classrooms").rglob("*.json"):
...
def test_generation_succeeds_without_public_origin_but_exposes_no_url(tmp_path):
    rec: List[httpx.Request] = []
    _, tc, headers, _ = _bootstrap(tmp_path, _handler(rec), openmaic_embed_origin="")
    cid = _course(tc, headers)
    body = _generate_and_finish(tc, headers, cid)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        "http://openmaic:3001/classroom/room_1",  # 端口不同
        "https://openmaic:3000/classroom/room_1",  # scheme 不同
        "http://openmaic:3000/classroom/room_1?x=1",  # query
        "http://openmaic:3000/classroom/room_1#f",  # fragment
        "http://user:pw@openmaic:3000/classroom/room_1",  # credentials
        "http://openmaic:3000/classroom/../admin",  # 路径穿越
        "http://openmaic:3000/not-classroom/room_1",  # 路径不对
        "javascript:alert(1)",
        "file:///etc/passwd",
    ],
)
...
def test_client_validator_rejects_public_url_as_upstream(tmp_path):
    ...
    client = OpenMAICClient(base_url=INTERNAL, origin=INTERNAL)
...
def test_no_credentials_in_responses_or_store(tmp_path):
    ...
    _, tc, headers, store = _bootstrap(
        tmp_path, _handler(rec), openmaic_access_code="s3cret-access-code"
    )
    cid = _course(tc, headers)
    body = _generate_and_finish(tc, headers, cid)
    blob = json.dumps(body, ensure_ascii=False)
    for token in ("s3cret-access-code", "openmaic_access", "Cookie", "Authorization"):
...
#
# 修复前 `OpenMAICSessionOut.from_session` 直接返回 `session.classroom_url`。
# 历史列表已经会重算公开地址，但 `GET .../jobs/{session_id}` 仍会把数据库里
# 历史保存的 `http://openmaic:3000/classroom/room_1` 原样下发 —— 内部拓扑泄露。
...
def test_legacy_job_endpoint_reprojects_public_url(tmp_path):
    ...
    assert "openmaic:3000" not in resp.text
...
def test_job_endpoint_without_public_origin_returns_null_url(tmp_path):
    ...
    rec: List[httpx.Request] = []
    container, tc, headers, store = _bootstrap(tmp_path, _handler(rec), openmaic_embed_origin="")
    cid = _course(tc, headers)
    _seed_legacy_session(
        store, container, cid, url=f"{INTERNAL}/classroom/room_1", classroom_id="room_1"
    )
...
def test_history_and_job_follow_current_public_origin(tmp_path):
    ...
    # 运维把 OPENMAIC_EMBED_ORIGIN 换掉（历史数据不变）→ 所有下发面都必须跟着变
    container.settings.openmaic_embed_origin = new_origin
    item = next(
        i
...
def test_no_route_or_schema_serializes_raw_classroom_url():
    """契约测试：URL 只能由 services/openmaic/public_url.py 投影产生。
...
def test_session_out_schema_requires_explicit_public_origin():
    ...
    from app.schemas.openmaic import OpenMAICSessionOut

    params = inspect.signature(OpenMAICSessionOut.from_session).parameters
...
def test_upstream_error_text_is_redacted_before_exposing(tmp_path):
    ...
    _, tc, headers, _ = _bootstrap(
        ...
        _handler_with_error(
            rec,
            error="connect ECONNREFUSED http://openmaic:3000/api/classroom (access_code=s3cret-access-code)",
            message="重试 http://openmaic:3000/api/generate-classroom 失败",
        ),
        openmaic_access_code="s3cret-access-code",
    )
    cid = _course(tc, headers)
    body = _generate_and_finish(tc, headers, cid)
    blob = json.dumps(body, ensure_ascii=False)
    assert body["status"] == "failed"
    assert "openmaic:3000" not in blob, f"内部地址泄露: {blob[:300]}"
...
def test_redaction_helper_covers_cookie_and_internal_hosts():
    from app.services.openmaic.redaction import redact_public_text

    settings = _settings(openmaic_access_code="topsecret-code")
    text = (
        "ECONNREFUSED http://openmaic:3000/x; Cookie: openmaic_access=abc; "
        "Authorization: Bearer deadbeef; access_code=topsecret-code"
    )
    out = redact_public_text(text, settings)
    assert "openmaic:3000" not in out
