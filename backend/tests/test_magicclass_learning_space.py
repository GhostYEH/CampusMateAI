...
2. 下发给浏览器的只有公开 Origin，内部地址（`MAGICCLASS_BASE_URL`）不得出现在响应里；
...
STATUS_PATH = "/api/v1/magicclass/learning-space/status"
...
def test_status_is_honest_when_not_configured(tmp_path):
    _, tc, headers = _bootstrap(tmp_path, magicclass_enabled=False)
    resp = tc.get(STATUS_PATH, headers=headers)
    assert resp.status_code == 200, resp.text
    body = resp.json()
    assert body["enabled"] is False
...
def test_status_fails_closed_when_public_origin_is_missing(tmp_path):
    ...
    _, tc, headers = _bootstrap(tmp_path, magicclass_embed_origin="")
    resp = tc.get(STATUS_PATH, headers=headers)
    body = resp.json()
    assert body["enabled"] is True
    assert body["embed_origin"] is None
