...
class FakeOpenMAIC:
    """假 OpenMAIC：记录每次 POST /api/generate-classroom 的 requirement。"""
...
def _client_for(settings: Settings, fake: FakeOpenMAIC) -> OpenMAICClient:
...
def _bootstrap(
    tmp_path, fake: FakeOpenMAIC, *, with_materials: bool = False, **overrides
...
def _session(container, cid, session_id) -> OpenMAICSession:
    user = container.user_repository.get_user_by_username("student_demo")
    return container.openmaic_classroom_service.get_session(
...
def test_retry_survives_service_restart(tmp_path):
    ...
    restarted.openmaic_result_store = store
    restarted.openmaic_classroom_service = OpenMAICClassroomService(
        settings, store, client=_client_for(settings, fake)
    )
...
def test_retry_while_in_progress_does_not_submit_again(tmp_path):
    fake = FakeOpenMAIC(auto_complete=False)
...
def test_snapshot_never_stores_credentials_or_internal_urls(tmp_path):
    ...
    container, tc, headers, store = _bootstrap(
        tmp_path, fake, with_materials=True, openmaic_access_code="s3cret-access-code"
    )
    cid = _course(tc, headers)
    ids = [m["id"] for m in _materials(tc, headers, cid)[:1]]
    gen = _generate_personalized(tc, headers, cid, ids)
    ...
    for token in ("s3cret-access-code", "openmaic_access", INTERNAL, "Cookie", "Authorization"):
...
def test_snapshot_ignores_unknown_keys(tmp_path):
    ...
    from app.services.openmaic.requirement_builder import GenerationRequestSnapshot
...
def test_retry_after_submit_failure_reuses_snapshot(tmp_path):
    ...
    fake = FakeOpenMAIC(fail_submit=True)
    container, tc, headers, _ = _bootstrap(tmp_path, fake, with_materials=True)
    cid = _course(tc, headers)
    ids = [m["id"] for m in _materials(tc, headers, cid)[:1]]
    ...
    user = container.user_repository.get_user_by_username("student_demo")
    sessions = container.openmaic_classroom_service.list_sessions(user_id=user.id, course_id=cid)
