...
def test_the_context_exposes_real_course_facts_without_touching_the_managed_service():
    ...
    response = http.get(f"/api/v1/courses/{course_id}/openmaic-context", headers=headers)
...
def test_sync_state_is_reported_honestly():
    ...
    body = http.get(f"/api/v1/courses/{course_id}/openmaic-context", headers=headers).json()
...
def test_an_unknown_course_is_a_404_and_never_a_fabricated_context():
    ...
    response = http.get("/api/v1/courses/crs_does_not_exist/openmaic-context", headers=headers)
...
def test_the_context_requires_authentication():
    ...
    assert http.get(f"/api/v1/courses/{course_id}/openmaic-context").status_code == 401
