"""magicclass 学生侧集成 —— 行为测试(不是读源码的正则断言)。

覆盖用户要求的 9 项验收：
1. 后端安全返回可信 magicclass Origin(含端口)，未配置时 fail-closed；
...
7. 知识点/知识图谱统计/真实掌握率进入 magicclass 与 CPM 上下文，严格按用户+课程过滤、限长；
8. 每用户每课程最多 20 条历史，只裁剪最早的终态记录。

全部通过 httpx.MockTransport 模拟 magicclass 服务，不伪造真实生成结果。
"""
...
from app.services.magicclass.classroom_service import PUBLIC_STEPS, MagicClassClassroomService
from app.services.magicclass.client import (
    GENERATION_STEPS,
    JOB_STATUS_VALUES,
    JOB_STEP_VALUES,
    PROBE_JOB_ID,
...
from app.services.magicclass.course_context import build_course_context, build_cpm_course_block
from app.services.magicclass import result_store as result_store_module
...
def _recording_handler(
    ...
    *,
    ...
    """一个可复用的假 magicclass：health / submit / poll 都记录请求。"""
    lock = threading.Lock()
...
def test_public_step_contract_matches_real_magicclass():
...
def test_client_maps_terminal_states_and_error():
    ...
    succeeded = asyncio.run(
        MagicClassClient(base_url=BASE, origin=BASE, transport=httpx.MockTransport(ok)).poll("j")
    )
    ...
    failed = asyncio.run(
        MagicClassClient(base_url=BASE, origin=BASE, transport=httpx.MockTransport(bad)).poll("j")
    )
...
def test_status_contract_when_not_configured(tmp_path):
    _, tc, headers, _ = _bootstrap(
        tmp_path, _recording_handler([]), magicclass_enabled=False
    )
    cid = _first_course(tc, headers)
    body = tc.get(
        f"/api/v1/courses/{cid}/interactive-classroom/status", headers=headers
    ...
    def handler(request: httpx.Request) -> httpx.Response:
        ...
        if path.endswith("/api/access-code/status"):
            return httpx.Response(
                ...
                json={
                    "success": True,
                    "enabled": True,
                    "authenticated": "magicclass_access=" in cookie,
                },
            )
        if path.endswith("/api/access-code/verify"):
            ...
            return httpx.Response(
                ...
                headers={"set-cookie": "magicclass_access=token-abc; Path=/; HttpOnly"},
            )
        # 真实 middleware：除白名单外所有 /api/* 都需要有效 cookie
        if "magicclass_access=token-abc" not in cookie:
            return httpx.Response(
                401,
                json={
                    "success": False,
...
def test_access_code_is_verified_and_cookie_is_reused(tmp_path):
    recorder: List[httpx.Request] = []
    _, tc, headers, _ = _bootstrap(
        tmp_path, _access_code_handler(recorder), magicclass_access_code=ACCESS_CODE
    )
    cid = _first_course(tc, headers)
    ...
    assert submits and "magicclass_access=token-abc" in submits[0].headers.get("cookie", "")
...
def test_access_code_required_but_not_configured_fails_closed(tmp_path):
    recorder: List[httpx.Request] = []
    _, tc, headers, _ = _bootstrap(
        tmp_path, _access_code_handler(recorder), magicclass_access_code=""
    )
    cid = _first_course(tc, headers)
    status = tc.get(
        f"/api/v1/courses/{cid}/interactive-classroom/status", headers=headers
    ...
    assert gen.json()["code"] == "MAGICCLASS_AUTH_ERROR"
...
def test_wrong_access_code_is_rejected(tmp_path):
    recorder: List[httpx.Request] = []
    _, tc, headers, _ = _bootstrap(
        tmp_path, _access_code_handler(recorder), magicclass_access_code="wrong-code"
    )
    cid = _first_course(tc, headers)
    status = tc.get(
        f"/api/v1/courses/{cid}/interactive-classroom/status", headers=headers
...
def test_stale_reservation_can_be_taken_over(tmp_path):
    ...
    _, tc, headers, store = _bootstrap(
        tmp_path, _recording_handler(recorder), magicclass_reservation_ttl_seconds=30
    )
    cid = _first_course(tc, headers)
...
def test_concurrent_stale_takeover_has_exactly_one_winner(tmp_path, monkeypatch):
    ...
    store = MagicClassResultStore(tmp_path / "om")
    assert store.acquire_reservation(
    ...
    service = MagicClassClassroomService(
        _test_settings(magicclass_reservation_ttl_seconds=30), store, client=None
    )
...
def test_history_capped_at_20_pruning_oldest_terminal(tmp_path):
    store = MagicClassResultStore(tmp_path / "om", max_results=20)
    ids = []
    for i in range(21):
...
def test_history_pruning_never_removes_running_records(tmp_path):
    store = MagicClassResultStore(tmp_path / "om", max_results=3)
    for i in range(3):
        store.save(
            MagicClassSession(
...
def test_magicclass_context_includes_knowledge_and_mastery(tmp_path):
...
def test_magicclass_route_accepts_own_chaoxing_course(tmp_path):
    container, tc, headers, _ = _bootstrap(tmp_path, _recording_handler([]))
    user = _student(container)
    course = _own_chaoxing_course(container, user)
    resp = tc.get(
...
def test_magicclass_route_rejects_foreign_chaoxing_course(tmp_path):
    container, tc, headers, _ = _bootstrap(tmp_path, _recording_handler([]))
    other = _student(container, "student_demo_02")
    foreign = container.course_repository.create_course(
        name="别人的学习通课程",
