...
import { useWidgetIframeStore } from './lib/widget-iframe-store.js';
import { useInteractiveIframePool } from './lib/interactive-iframe-pool.js';
import { useSceneRuntimeErrors } from './lib/scene-runtime-errors.js';
import {
  GENUI_LOGICAL_HEIGHT,
  GENUI_LOGICAL_WIDTH,
  fitGenUiViewport,
} from './lib/logical-viewport.js';
import { intersectClientBoxes } from './lib/visible-client-rect.js';

/**
 * 移植自参考项目 `components/scene-renderers/InteractiveIframeHost.tsx`。
 *
 * 机械改写：
 *   - TypeScript 类型 / interface / 泛型全部擦除，去掉 `'use client'`。
 *   - `@/lib/store/*`、`@/lib/interactive/*`、`@/lib/edit/*` 改为相对路径；
 *     三个 store 用本层的普通 React store 实现（见 lib/store.js）。
 *   - `useI18n()` -> zh-CN 文案（`chat.elementReference.instruction` =
 *     「点击一个课件元素 · Esc 退出」）。
 *   - **沙箱属性逐字保留**：`sandbox="allow-scripts allow-forms allow-popups"`
 *     （刻意不含 `allow-same-origin`，见下方注释），不放大。
 *   - 注入脚本与错误回放协议（`__maicErrorReplayRequest`）逐字保留。
 *
 * 未移植的部分（已在报告中标注）：Pro 编辑模式的元素拾取链路——它依赖
 * `lib/store/canvas`（pickTarget）、`lib/store/element-refs` 和
 * `lib/workbench/element-refs` 三个编辑器 store。播放侧的拾取
 * （`playback-stable-id`）完整保留。
 */

export function resolveInteractivePickerMode(editorArmed, playbackArmed) {
  if (playbackArmed) return 'playback-stable-id';
  if (editorArmed) return 'editor';
  return null;
}

/** 参考项目里 editor 分支的处理器；编辑器 store 未移植，故此处仅保留签名。 */
export function handleInteractivePickerMessage(_sceneId, _data, _t) {
  return false;
}
 ...
 * editing. Visibility is driven by the placeholder's ownership (gone → hidden,
 * never unmounted), so the document is preserved for a zero-reload return.
 */
...
function PooledIframe({
  ...
  // Pro 编辑模式的 pickTarget / element refs 未移植：课堂播放路径上恒为 false，
  // 于是 effectiveMode 只会是 'playback-stable-id' 或 null。
  const editorArmed = false;
  ...
  // Capture runtime errors the iframe's error shim posts out (see iframe.js), so
  ...
  useEffect(() => {
    const onMessage = (e) => {
      ...
      if (effectiveMode === 'editor') handleInteractivePickerMessage(sceneId, d);
    };
    window.addEventListener('message', onMessage);
    iframeRef.current?.contentWindow?.postMessage({ __maicErrorReplayRequest: true }, '*');
    return () => window.removeEventListener('message', onMessage);
  }, [sceneId, entry.srcDoc, effectiveMode, onPlaybackCancel, onPlaybackPick, playbackArmed]);
  ...
  return (
    <div style={wrapStyle}>
      ...
      {playbackArmed && (
        ...
        >
          {/* chat.elementReference.instruction */}
          点击一个课件元素 · Esc 退出
        </div>
      )}
    </div>
