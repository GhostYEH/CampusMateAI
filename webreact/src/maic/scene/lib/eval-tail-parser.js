...
function splitLastFencedJsonTail(text) {
...
export function parseEvaluationTail(text) {
  if (!text) return null;
  ...
  return parseLastObjectCandidate(candidates);
}

function parseLastObjectCandidate(candidates) {
  for (let i = candidates.length - 1; i >= 0; i--) {
    const parsed = parseJsonResponse(candidates[i].trim());
...
function extractTrailingBalancedObjectRange(text) {
...
export function stripEvaluationTail(text) {
  if (!text) return '';
 ...
 */
export function sanitizeMilestoneEvaluationFeedback(text) {
  if (!text) return '';
  const { narrative, tail } = splitLastFencedJsonTail(text);
 ...
 *  - objects / arrays
 */
export function normalizeStars(raw) {
...
export function normalizeStringList(raw, max = 8) {
...
export function normalizeOptionalString(raw) {
