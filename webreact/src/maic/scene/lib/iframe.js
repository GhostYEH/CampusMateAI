import { injectIntoDocumentHead } from './html-document.js';
import { INTERACTIVE_REFERENCE_EXCLUDED_TAG_NAMES } from './element-reference-policy.js';

/**
 * 移植自参考项目 `lib/utils/iframe.ts`。
 *
 * 唯一的机械改写：`@/lib/interactive/element-reference-policy` -> 相对路径；
 * 去掉 TypeScript 类型标注。三段注入脚本（存储 shim、错误捕获 shim、元素选择器
 * shim）与注入 CSS **逐字保留**——沙箱安全姿态和父窗口↔iframe 的消息协议都靠它们，
 * 任何"顺手简化"都会改变行为。
 */
 ...
 */
export function patchHtmlForIframe(html) {
