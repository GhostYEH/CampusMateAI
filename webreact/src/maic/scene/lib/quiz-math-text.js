/**
 * 移植自参考项目 `lib/quiz/math-text.ts`。
 *
 * 唯一的**近似替换**：参考实现直接 `import katex from 'katex'` 做 LaTeX ->
 * HTML。目标项目没有 katex 依赖（package.json 里没有，node_modules 里也没有），
 * 所以这里把渲染器改成可注入的：
 *
 *   import { setLatexRenderer } from './quiz-math-text.js';
 *   setLatexRenderer((latex, displayMode) => katex.renderToString(latex, {...}));
 *
 * 未注入渲染器时 `renderLatexToHtml` 返回 null——这与参考实现里 katex 抛错时的
 * 失败路径**完全一致**：`parseQuizMathText` 会把该片段原样降级为纯文本，
 * `renderQuizMathText` 会退回整体的纯文本。也就是说，在没有 katex 的环境里
 * 题目文本仍然完整可读，只是不做数学排版。解析器本身逐字保留。
 *
 * 其余部分仅擦除 TypeScript 类型标注。
 */

let latexRenderer = null;

/** 注入 LaTeX 渲染器。签名与参考实现里的 katex 调用点一致。 */
export function setLatexRenderer(renderer) {
  latexRenderer = typeof renderer === 'function' ? renderer : null;
}
...
export function isLikelyStandaloneMathText(value) {
...
export function renderLatexToHtml(value, displayMode = false) {
  if (!latexRenderer) return null;
  try {
    return latexRenderer(escapeLiteralPercents(value), {
      displayMode,
      output: 'html',
      strict: false,
      throwOnError: true,
...
function escapeLiteralPercents(value) {
  let escaped = '';
...
export function parseQuizMathText(value) {
  const segments = [];
  let cursor = 0;
...
function parseEmbeddedMathText(value) {
  const segments = [];
  let cursor = 0;
  let hasMath = false;
...
function trimEmbeddedMathCandidate(value, start, end) {
...
function isLikelyEmbeddedMathText(value) {
  const text = value.trim();
...
function isLikelyDelimitedMathText(value) {
  const text = value.trim();
...
export function renderQuizMathText(value) {
...
function findNextDelimiter(value, startIndex) {
  let match = null;
...
function findUnescaped(value, search, startIndex) {
...
function isCodeLikeAssignment(value) {
  const text = value.trim();
