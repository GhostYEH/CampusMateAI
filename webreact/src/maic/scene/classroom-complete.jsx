...
import {
  ...
  summarizeScenes,
} from './lib/complete-summary.js';
import { loadQuizAttemptState } from './lib/quiz-runtime.js';

/**
 * 移植自参考项目 `components/scene-renderers/classroom-complete.tsx`（661 行）。
 *
 * 机械改写（类名、内联 style、SVG、motion 参数、DOM 结构全部逐字保留）：
 *   1. TypeScript 类型 / interface / 泛型 / `as` 全部擦除，去掉 `'use client'`。
 *   2. `useI18n()` 的 `t()` 全部换成参考项目 zh-CN locale 的真实文案；
 *      `t('classroomComplete.quizScoreLabel', {correct,total})` ->
 *      `答对 ${correct} / ${total}`；动态 key
 *      `classroomComplete.trailLabels.*` 与 `classroomComplete.encouragement.*`
 *      换成同值的中文常量表（键仍然一一对应）。
 *   3. `locale` 固定为 'zh-CN'（用于 Intl.DateTimeFormat）。
 *   4. `useStageStore`（zustand，51KB）-> 本层的极小 stage store；
 *      `ClassroomCompletePageConnected` 保持同名同行为。
 *   5. `@/lib/classroom/complete-summary`、`@/lib/quiz/runtime`、
 *      `@/lib/logger` 改为本层相对路径。
 */
...
const TYPE_ORDER = ['slide', 'quiz', 'interactive', 'pbl'];

/** classroomComplete.trailLabels.*（zh-CN） */
const TRAIL_LABELS = {
  slide: '页',
  quiz: '小测',
  interactive: '互动',
...
/** classroomComplete.encouragement.*（zh-CN） */
const ENCOURAGEMENT = {
  high: '太棒了，完美发挥！',
  mid: '表现不错，继续加油！',
  low: '万事开头难，回去再练练吧。',
...
function AnimatedCounter({ value, delay = 0, duration = 0.9 }) {
  const prefersReducedMotion = useReducedMotion();
...
function QuizRing({ pct, delay = 0, compact = false }) {
  const prefersReducedMotion = useReducedMotion();
  const radius = 34;
...
export function ClassroomCompletePage({ scenes, title }) {
  const prefersReducedMotion = useReducedMotion();
                ...
                >
                  <div className={cn('flex items-center', compact ? 'gap-3' : 'gap-5')}>
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      >
                        {/* classroomComplete.quizScoreLabel */}
                      ...
                      >
                        {/* classroomComplete.encouragement.<high|mid|low> */}
...
/**
 * 参考实现从 stage store 里取 `stage` / `scenes`。本层的 stage store 只保留
 * 这两个字段的订阅语义，调用点与上游一致。
 */
