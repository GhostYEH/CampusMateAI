...
import { resolvePBLContent, upgradeLegacyPBLConfigToProjectV2 } from './lib/pbl-legacy-read.js';
import { normalizeProjectRuntime } from './lib/pbl-progress.js';
import { transitionProjectUiPhase } from './lib/pbl-runtime-events.js';
import { useStageStore } from './lib/stage-store.js';
import { cn } from '../utils/cn.js';
import { PBLV2Hero } from './pbl/hero.jsx';
import { PBLV2Completion } from './pbl/completion.jsx';
import { rectsEqual } from './pbl/host-rect.js';

/**
 * 移植自参考项目 `components/scene-renderers/pbl-renderer.tsx`（541 行）。
 *
 * 机械改写：
 *   1. TypeScript 类型 / interface / 泛型 / `as` 全部擦除，去掉 `'use client'`。
 *   2. `useI18n()` 的 3 个 key 换成 zh-CN 真实文案：
 *        pbl.emptyProject                 -> PBL项目尚未生成，请通过课程生成创建
 *        pbl.v2.workspace.exitNativeFullscreen -> 退出全屏
 *        pbl.v2.workspace.exitFullscreen      -> 退出全屏工作台
 *        pbl.v2.workspace.enterFullscreen     -> 展开为全屏工作台
 *   3. `@/lib/pbl/...`、`@/lib/store/stage`、`@/lib/utils/cn` 改为本层相对路径；
 *      `useStageStore` 换成本层极小 stage store（`getState().scenes` /
 *      `getState().updateScene` 签名一致）。
 *   4. **未移植 `PBLV2Workspace`**（`pbl/v2/workspace.tsx`）——它牵出 chat /
 *      submission / use-instructor-stream / SSE 运行时等一大片依赖图（见报告）。
 *      这里的 `PBLV2WorkspaceUnavailable` 是同一 frame 内的显式占位：完整保留
 *      portal、fixed 定位、docked/expanded 动画、全屏进出控件与返回 Hero 的能力，
 *      只有工作台主体内容缺失。`completed` 阶段仍渲染完整的 `PBLV2Completion`。
 *
 * 未改写的行为：Hero 的 1.3s 首启展开、手动最大化/收起的快动画、body 滚动锁、
 * host rect 的 rAF 追踪（含 ResizeObserver + dirty check）、native fullscreen 的
 * portal 跟随与独立退出按钮、以及 workspace 写入时"学员还在 Hero 就保持在 Hero"
 * 的 uiPhase 守卫。
 */
...
const HERO_LAUNCH_EXPAND_EASE = [0.4, 0, 0.2, 1];
const IMMERSIVE_EXIT_DURATION_SECONDS = 0.4;
const IMMERSIVE_LAUNCH_EASE = [0.16, 1, 0.3, 1];
const IMMERSIVE_EXIT_EASE = [0.4, 0, 0.2, 1];

export function PBLRenderer({ content, mode: _mode, sceneId }) {
...
function PBLV2Container({ sceneId, projectV2, onProjectV2Change }) {
  ...
  // live inside whatever element is natively fullscreened — magic class fullscreens
  ...
  // is on. NOTE: this never reads/writes magic class code — it only chooses this
  ...
  // True while the browser is in native (OS) fullscreen — magic class enters it
  // on the stage via `stageRef.requestFullscreen()`. In that mode magic class's
  ...
  const handleInstructorStreamingChange = useCallback((active) => {
    ...
    <div ref={hostRef} className="h-full w-full">
      ...
      {(runtimeProject.uiPhase === 'workspace' || runtimeProject.uiPhase === 'completed') &&
        ...
        createPortal(
          ...
          // launch), so magic class's native fullscreen still shows the workspace.
...
function measureHostRect(hostRef) {
 ...
 * stays stable — meaning the inner workspace (and its chat / stream state)
 * is never remounted by an expand/collapse toggle.
 *
...
function PBLV2WorkspaceLayer({
  ...
  const [hostRect, setHostRect] = useState(() => measureHostRect(hostRef));
  ...
  return (
    <>
      ...
      <motion.div
        ...
        transition={transition}
      >
        {/* Native (OS) fullscreen takes over the top-right slot: magic class's
            own exit bar is hidden for PBL, so this is the only click target
            to leave true fullscreen (Esc still works, handled by magic class).
            `document.exitFullscreen()` triggers magic class's own fullscreenchange
            cleanup (keyboard unlock etc.) — we never touch magic class code. The
            violet, borderless-ish treatment mirrors magic class's native control
        ...
        {nativeFullscreen ? (
          <button
            ...
            /* pbl.v2.workspace.exitNativeFullscreen */
            aria-label="退出全屏"
            title="退出全屏"
          >
          ...
          <>
            {expanded && (
              <button
                ...
                /* pbl.v2.workspace.exitFullscreen */
                aria-label="退出全屏工作台"
                title="退出全屏工作台"
              >
                <Minimize2 className="h-4 w-4" />
            ...
            {!expanded && isCompleted && (
              <button
                ...
                className="absolute right-4 top-4 z-40 flex h-8 w-8 items-center justify-center rounded-md border border-white/15 bg-background/75 text-foreground shadow-sm backdrop-blur transition-colors hover:bg-muted"
                /* pbl.v2.workspace.enterFullscreen */
                aria-label="展开为全屏工作台"
                title="展开为全屏工作台"
              >
 ...
 * **移植缺口占位组件**（不属于参考项目）。
 *
 * 参考项目这里渲染 `PBLV2Workspace`（`pbl/v2/workspace.tsx`）。该文件以及它引出的
 * chat / submission / use-instructor-stream / SSE 运行时、AI 模型配置、云存储客户端
 * 与设置 store 都未移植（见报告），因此工作台阶段用一个**结构上等价、视觉上克制**的
 * 占位替代：保留返回 Hero 的入口、全屏按钮、以及 Busy/流式状态的回调签名，
...
function PBLV2WorkspaceUnavailable({
  ...
  // 保持与上游一致的 streaming 计数契约：这里没有任何流，因此挂载/卸载时各报告一次
  // 「无流」状态，保证父级的计数不会被这个占位永久卡住。
  useEffect(() => {
    ...
    <div className="flex h-full w-full flex-col gap-5 overflow-auto bg-background px-6 py-6 text-left">
      ...
      <div className="mx-auto flex w-full max-w-4xl items-center gap-2">
        <button
          ...
          className="inline-flex items-center gap-1.5 rounded-xl border border-border/70 bg-background/50 px-4 py-2 text-sm font-medium text-foreground transition-colors hover:bg-muted"
        >
          {/* pbl.v2.workspace.returnToHero */}
          返回项目主页
        </button>
        {onExpand && (
          <button
