 ...
 * 里的断言随类型检查一起去掉。
 *
 * 测量契约未变：目标元素仍按 `${elementIdPrefix}${elementId}` 取 DOM id
 * （`SlideElement` / `ScreenElement` 写的是同一个 id），先找
 * `.element-content`，找不到再退回根节点。
 */
import { useRef, useState, useLayoutEffect, useCallback } from 'react';
import { motion, AnimatePresence } from 'motion/react';

export function SpotlightOverlay({ options, elementIdPrefix = 'slide-element-', measurementKey }) {
