 ...
 *
 * 关于语法高亮：参考项目动态 `import('shiki')`（github-light 主题）。目标项目
 * 没有安装 `shiki`，也没有 `streamdown`，因此：
 *
 *   - `getHighlighter()` 恒返回 `null`，`highlightedLines` 为 null，
 *     `lineHtmlMap` 落到 `fallbackLines`（`escapeHtml(line.content)`）。
 *     这正是参考文件里 shiki 不可用时的 catch 分支——容器标记、字体栈、
 *     行号、动画时序完全一致，只是没有 token 着色。
 *   - 如果宿主将来能提供 shiki（或任何同形状的 highlighter），可以用
 *     `setCodeHighlighterFactory(factory)` 在运行时接上，无需改动本文件其余逻辑。
 */
...
/**
 * 默认工厂：目标项目没有 shiki，返回 null → 走 `escapeHtml` 兜底。
 * `setCodeHighlighterFactory` 可以在运行时替换它。
 */
let highlighterFactory = async () => null;

export function setCodeHighlighterFactory(factory) {
...
export function BaseCodeElement({ elementInfo, animate }) {
