...
import StageEditorPanel from "../components/magicclass/StageEditorPanel.jsx";
import StagePlayerPanel from "../components/magicclass/StagePlayerPanel.jsx";
import ProviderToolsPanel from "../components/magicclass/ProviderToolsPanel.jsx";
...
import { filenameFromContentDisposition } from "../features/magicclass/archiveModel.js";
...
function StageExportActions({ courseId, workspaceId, stage, canExportPptx }) {
  ...
  async function exportPptx() {
    ...
    try {
      const result = await api.exportMagicClassStageFormat(courseId, workspaceId, stage.id, "pptx");
  ...
  return <div className="magicclass-stage-export" aria-label="场景导出">
    ...
    {error ? <span className="magicclass-hint magicclass-hint--error" role="alert">{error}</span> : null}
  </div>;
}

export default function MagicClassWorkspacePage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const [workspacePayload, stagePayload] = await Promise.all([api.getMagicClassWorkspace(courseId, workspaceId), api.listMagicClassStages(courseId, workspaceId, { limit: 50 })]);
  ...
  const loadProviderStatus = useCallback(async () => {
    try { setProviderStatus(await api.getMagicClassProviderStatus()); }
    catch { setProviderStatus(null); }
  }, []);

  const loadFusionStatus = useCallback(async () => {
    try { setFusionStatus(await api.getMagicClassFusionStatus()); }
  ...
  return <PageFrame eyebrow="magicclass / Workspace" title={workspace?.name || "学习工作台"} description={workspace?.description || "课程内容、场景编辑与播放"} actions={<><BackLink to="/courses">返回课程</BackLink><Button variant="secondary" icon="PhArrowClockwise" onClick={load} disabled={loading}>刷新</Button></>}>
    <AsyncState loading={loading} error={error} onRetry={load}><div className="magicclass-workspace-page">
      {notice ? <div className="page-notice notice-info" role="status">{notice}</div> : null}
      {launchPrompt ? <p className="magicclass-launch-context" role="status">{launchRoleMode === "auto" ? "自动生成角色" : `已选择角色：${selectedRoles(launchRoleIds).map((role) => role.name).join("、")}`}</p> : null}
      <Panel className="magicclass-generation-panel"><SectionHeading title="创建学习内容" detail="生成结果会写入当前 workspace，服务不可用时不会伪造内容。" /><form className="magicclass-command" onSubmit={generate}><label className="magicclass-command__input"><Icon name="PhSparkle" size={18} /><input value={prompt} onChange={(event) => setPrompt(event.target.value)} placeholder="例如：讲解函数极限的直觉、定义和一个例题" maxLength={2000} aria-label="学习内容主题" /></label><select value={mode} onChange={(event) => setMode(event.target.value)} aria-label="学习内容类型">{MODES.map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select><Button type="submit" disabled={busy || !prompt.trim()}>{busy ? "提交中…" : "生成"}</Button></form>{job ? <div className="magicclass-job-status" role="status"><span>{job.status === "completed" ? "已完成" : job.status === "cancelled" ? "已中断" : job.status === "failed" ? "失败" : "生成中"}</span><strong>{Number(job.progress) || 0}%</strong>{jobBusy ? <Button variant="quiet" disabled={busy} onClick={cancelJob}>中断</Button> : null}{["failed", "cancelled"].includes(job.status) ? <Button variant="quiet" disabled={busy} onClick={retryJob}>重试</Button> : null}</div> : null}</Panel>
      <div className="magicclass-workspace-page__grid"><Panel><SectionHeading title="场景目录" detail={`${stages.length} 个内容`} />{stages.length ? <ol className="magicclass-stage-browser__list">{stages.map((stage) => <li key={stage.id} className={stage.id === selectedStageId ? "is-selected" : ""}><button type="button" className="magicclass-stage-select" onClick={() => { setSelectedStageId(stage.id); setPlaying(false); }}><span className="row-copy"><strong>{stage.title}</strong><small>DSL {stage.dslVersion || "—"} · revision {stage.revision}</small></span><Icon name="PhCaretRight" size={15} /></button></li>)}</ol> : <div className="magicclass-home__empty"><Icon name="PhLayout" size={24} /><p>还没有内容</p><small>在上方输入主题，创建第一份学习内容。</small></div>}</Panel><div className="magicclass-workspace-page__content">{selectedStage ? <Panel><SectionHeading title={selectedStage.title} detail={`最近更新于 ${dateText(selectedStage.updatedAt)}`} action={<div className="magicclass-stage-heading-actions"><StageExportActions courseId={courseId} workspaceId={workspaceId} stage={selectedStage} canExportPptx={describeFusionState(fusionStatus).canExportPptx} /><Button variant="secondary" onClick={() => setPlaying((value) => !value)}>{playing ? "关闭播放" : "播放"}</Button></div>} />{playing ? <StagePlayerPanel courseId={courseId} workspaceId={workspaceId} stageId={selectedStage.id} onClose={() => setPlaying(false)} /> : <StageEditorPanel courseId={courseId} workspaceId={workspaceId} stageId={selectedStage.id} onSaved={load} />}</Panel> : <Panel><div className="magicclass-home__empty"><p>选择一个场景开始编辑。</p></div></Panel>}</div></div>
      <ProviderToolsPanel
        courseId={courseId}
