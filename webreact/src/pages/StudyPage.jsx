...
export default function StudyPage() {
  ...
  return <><PageFrame className="study-page" showHeading={false}>
    ...
    {loading ? <div className="state-card loading-state" aria-busy="true"><span className="loading-orb" /><p>正在加载内容…</p></div> : <div className="stack reveal">
      ...
      <StudyTiltedCard className="study-plan-card"><Panel className="study-plan-panel"><SectionHeading title="今日学习计划" detail="把大目标拆成下一步" action={<Button variant="quiet" icon="PhSparkle" onClick={() => setExperience(studyExperienceModel("plan", {}))}>打开 AI 学习路线</Button>} />{tasks.slice(0, 4).length ? <div className="study-today-list">{tasks.slice(0, 4).map((item, index) => <button type="button" className="study-today-task" key={item.id} onClick={() => setExperience(studyExperienceModel("task", { title: "计划详情", value: item.title || "学习计划", detail: item.deadline ? `计划截止 ${dateText(item.deadline)}` : "打开任务页可以继续编辑和完成计划。", task: item }))}><span className={`study-today-check ${index < 2 ? "is-highlighted" : ""}`}><Icon name={index < 2 ? "PhCheck" : "PhSquare"} size={15} weight="bold" /></span><span><strong>{item.title}</strong><small>{item.deadline ? dateText(item.deadline) : "待安排"}</small></span><Icon name="PhDotsThree" size={18} weight="bold" /></button>)}</div> : <div className="inline-empty"><Icon name="PhCheckCircle" size={28} />当前没有待完成计划</div>}<Link className="study-plan-link" to="/plans">查看完整计划 <Icon name="PhArrowRight" size={14} /></Link></Panel></StudyTiltedCard>
