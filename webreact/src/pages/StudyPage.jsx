...
function ExperienceLayer({ experience, active, seconds, goal, mode, soundOn, blockNotifications, breaking, breakdown, onClose, onStart, onTogglePause, onFinish, onBreakdown, onGoalChange, onModeChange, onToggleSound, onToggleNotifications, onReuse, onSaveTask, onCompleteTask }) {
...
function StudyMetric({ label, value, unit, detail, icon, tone, onClick }) {
  const rotateX = useSpring(useMotionValue(0), { damping: 18, stiffness: 220 });
  const rotateY = useSpring(useMotionValue(0), { damping: 18, stiffness: 220 });
  const lift = useSpring(useMotionValue(1), { damping: 18, stiffness: 220 });
  ...
  return <motion.button type="button" className={`stat-card study-stat-trigger tone-${tone}`} onClick={onClick} aria-label={`${label}，${value}${unit || ""}，${detail}`} style={{ rotateX, rotateY, scale: lift, transformPerspective: 600 }} onMouseMove={handleMove} onMouseLeave={handleLeave} onTouchStart={handleMove} onTouchEnd={handleLeave}><span className="stat-icon"><Icon name={icon} size={22} /></span><strong>{value}<small className="study-stat-unit">{unit}</small></strong><span className="study-stat-label">{label}</span><small>{detail}</small></motion.button>;
}
...
export default function StudyPage() {
  ...
  return <><div className="study-page-backdrop" style={{ backgroundImage: `url("${background}")` }} aria-hidden="true" /><PageFrame className="study-page" showHeading={false}>
    ...
    {loading ? <div className="state-card loading-state" aria-busy="true"><span className="loading-orb" /><p>正在加载内容…</p></div> : <div className="stack reveal">
      ...
      <Panel className="study-trend-panel"><SectionHeading title="专注趋势（本周）" detail={`${weekMinutes} 分钟已完成`} action={<Button variant="quiet" icon="PhArrowsOut" onClick={() => setExperience(studyExperienceModel("trend", { title: "本周专注趋势", value: `${weekMinutes} 分钟`, detail: "完成会话会按开始日期累积到本周趋势中。", trend }))}>展开分析</Button>} /><div className="trend-bars" aria-label="本周专注趋势">{trend.map((item) => <span key={item.date}><i style={{ height: `${Math.max(item.count ? 14 : 4, Math.round(item.count / Math.max(...trend.map((value) => value.count), 1) * 100))}%` }} /><small>{item.label}</small><b>{item.count || ""}</b></span>)}</div></Panel>
      <Panel className="study-experience-entry"><SectionHeading title="沉浸模式" detail="隐藏干扰，只保留当前目标和专注计时" action={<Button icon="PhArrowsDownUp" onClick={() => setExperience(studyExperienceModel("focus", { title: active ? "沉浸专注" : "准备专注", goal: active?.goal || goal || "给今天一段专注时间", value: active ? `${String(Math.floor(seconds / 60)).padStart(2, "0")}:${String(seconds % 60).padStart(2, "0")}` : "", detail: active ? "正在进行的学习会话" : "选择时长后开始，计时与当前页面保持同步。" }))}>进入沉浸模式</Button>} /></Panel>
