...
export default function CounselorPage() {
  ...
  return <main className="counselor-reference">
    ...
    <section className="counselor-reference-grid">
      ...
      <aside className="counselor-reference-right"><DigitalHumanPanel speaking={digitalHuman.speaking} muted={digitalHuman.muted} status={digitalHuman.unityStatus} canReplay={Boolean(digitalHuman.lastText)} onReady={digitalHuman.setUnityReady} onError={digitalHuman.setUnityError} onToggleMuted={digitalHuman.toggleMuted} onStop={digitalHuman.stop} onReplay={digitalHuman.replay} />{lastAssistant && <section className="counselor-panel counselor-side-actions"><button type="button" onClick={turnToTask}><Icon name="PhCheckSquare" size={16} />把回答保存为待办</button><p>回答会保留在本地会话记录中，确认后再加入你的任务清单。</p></section>}</aside>
    </section>
