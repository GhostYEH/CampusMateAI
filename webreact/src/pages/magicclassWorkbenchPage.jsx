...
// 与参考项目的课堂相差太远。全局播放走 MagicClassClassroomStage；
// StagePlayerPanel 仍由 StageEditorPanel 的单场景「播放」入口使用。
import MagicClassClassroomStage from "../components/magicclass/magicclassClassroomStage.jsx";
import ProviderToolsPanel from "../components/magicclass/ProviderToolsPanel.jsx";
import ContainBox from "../components/magicclass/ContainBox.jsx";
import ResizeHandle from "../components/magicclass/ResizeHandle.jsx";
import WorkspaceCourseTabs from "../components/magicclass/WorkspaceCourseTabs.jsx";
import WorkspaceSceneRail from "../components/magicclass/WorkspaceSceneRail.jsx";
...
import {
  ...
  useNarrowViewport,
  useResizableWidth,
} from "../features/magicclass/workbenchLayoutModel.js";
import { resolveGenerationPhase } from "../features/magicclass/enterClassroomModel.js";
...
const RAIL_STORAGE_KEY = "campus_magicclass_workbench_rail_width";
 ...
 * 导航区，任何面板被选中时它都在。
 */
export default function MagicClassWorkbenchPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const [workspacePayload, stagePayload] = await Promise.all([
        api.getMagicClassWorkspace(courseId, workspaceId),
        api.listMagicClassStages(courseId, workspaceId, { limit: 50 }),
      ]);
  ...
  return <main
    ...
    data-testid="magicclass-workbench"
  ...
  >
    ...
    >
      <header className="ow-pane-head">
        <span className="ow-pane-eyebrow">magic class</span>
...
function PlaybackTakeover({ loading, selectedStage, courseId, workspaceId, onExit }) {
  ...
  return <MagicClassClassroomStage
    courseId={courseId}
    workspaceId={workspaceId}
