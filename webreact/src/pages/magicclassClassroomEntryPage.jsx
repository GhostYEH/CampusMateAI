 ...
 */
export default function MagicClassClassroomEntryPage() {
    ...
    const run = (async () => {
      ...
      try {        // 1) 真实课程事实（只读，不走受管服务）
        const [coursePayload, contextPayload] = await Promise.all([
          api.getCourses(),
          api.getMagicClassCourseContext(courseId).catch(() => null),
        ]);
        if (mine !== epoch.current) return;
        ...
        const fusion = await api.getMagicClassFusionStatus().catch(() => null);
        if (mine !== epoch.current) return;
        ...
        if (!canEnter) {
          throw {
            response: {
              status: 503,
              data: {
                code: "MAGICCLASS_FUSION_UNAVAILABLE",
                message: fusionView.detail || "受管 magic class 学习工作台当前不可用。",
        ...
        const listed = await api.listMagicClassWorkspaces(courseId, { limit: 1 });
        if (mine !== epoch.current) return;
        let target = normalizeWorkspaceList(listed)[0] || null;
        if (!target) {
          const created = await api.createMagicClassWorkspace(courseId, {
            name: facts.name ? `${facts.name} · 学习课堂` : "magic class 学习课堂",
        ...
        const result = await api.generateMagicClassStage(courseId, target.id, {
          mode: "slide",
          prompt,
          idempotencyKey: key,
        });
        ...
        if (!nextJob?.id) {
          ...
          const after = normalizeStageList(await api.listMagicClassStages(courseId, target.id, { limit: 50 }));
          if (mine !== epoch.current) return;
  ...
  const createManually = useCallback(async () => {
    ...
    try {
      ...
      if (!target) {
        const created = await api.createMagicClassWorkspace(courseId, {
          name: course?.name ? `${course.name} · 学习课堂` : "magic class 学习课堂",
  ...
  return <PageFrame
    className="magicclass-entry-page"
    eyebrow="magicclass / Classroom"
    title={title}
    description="正在为你准备这堂课的互动课堂。"
    actions={<Button variant="secondary" icon="PhArrowLeft" onClick={() => navigate("/courses")}>返回课程</Button>}
  >
    <div className="magicclass-entry">
      {/* 生成过程。这是用户看得见的"生成课程大纲 / 构建学习路径"，不是装饰。 */}
      {!failure ? <section className="magicclass-entry__progress" aria-live="polite" aria-busy={busy}>
        <div className="magicclass-entry__head">
          <span className="magicclass-entry__mark" aria-hidden="true"><Icon name="PhCube" size={26} weight="duotone" /></span>
          <div>
        ...
        <ol className="magicclass-entry__steps">
          {steps.map((step, index) => {
            const state = currentPhase.done || index < currentPhase.index
              ? "is-done"
              : index === currentPhase.index ? "is-current" : "is-next";
            return <li key={step.key} className={`magicclass-entry__step ${state}`}>
              <span className="magicclass-entry__dot" aria-hidden="true">
                {state === "is-done" ? <Icon name="PhCheck" size={12} weight="bold" /> : null}
              </span>
              <span className="magicclass-entry__step-copy">
        ...
        <div className="magicclass-entry__bar" role="progressbar" aria-valuemin={0} aria-valuemax={100} aria-valuenow={currentPhase.percent} aria-label="生成进度">
          <span style={{ width: `${currentPhase.percent}%` }} />
        </div>
        <p className="magicclass-entry__hint" role="status">
      ...
      {failure ? <section className="magicclass-entry__error" role="alert">
        ...
        <div>
          ...
          <div className="magicclass-entry__error-actions">
      ...
      {workspace?.id && !failure ? <p className="magicclass-entry__link">
        {entered ? "正在打开工作台…" : <>工作台已就绪：<button type="button" className="magicclass-entry__jump" onClick={() => goToWorkspace(workspace.id, "")}>直接进入「{workspace.name}」</button></>}
      </p> : null}

      {!online && !failure && !workspace ? <p className="magicclass-entry__hint">正在确认受管服务状态…</p> : null}
    </div>
  </PageFrame>;
}
