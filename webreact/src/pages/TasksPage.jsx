...
import {
  ...
  agendaMatchesStatus,
  countGroupedTasks,
  groupTasksWithAgenda,
} from "../data/agendaModel.js";
...
export default function TasksPage() {
  ...
  // 今日待办必须服从与本地清单相同的搜索/类型/状态筛选，否则"筛选"对今天分组无效。
  const agendaFilter = useMemo(() => (item) => (
    (kind === "all" || agendaFilterKind(item) === kind)
    && agendaMatchesStatus(item, status)
    && agendaMatchesQuery(item, query)
  ), [kind, status, query]);
  const groups = useMemo(
    () => groupTasksWithAgenda({
      tasks: visible, agenda: todayAgenda, stateOf: taskGroupState, filter: agendaFilter,
    }),
    [visible, todayAgenda, agendaFilter],
  );
  // 空状态要看"最终有没有渲染出条目"：只有课程/考试、没有普通任务时列表并不为空。
  const groupedCount = useMemo(() => countGroupedTasks(groups), [groups]);
