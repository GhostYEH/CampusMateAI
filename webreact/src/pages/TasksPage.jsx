...
function TaskRow({ task, onToggle, onEdit, onDelete, onPostpone, onOpen, onDragStart, onDragOver, onDrop }) {
  ...
  return <article className={`task-row panel task-state-${state}`} draggable onDragStart={() => onDragStart?.(task)} onDragOver={(event) => { event.preventDefault(); onDragOver?.(task); }} onDrop={() => onDrop?.(task)}>
    ...
    <div className="task-row-actions">
      ...
      {task.kind === "final-review" && <Link className="text-button" to={`/tasks/final-review/${task.sourceId}`}>查看运行</Link>}
    </div>
  </article>;
}
...
export default function TasksPage() {
  ...
  async function load() { setLoading(true); setError(""); try { setData(await Promise.all([api.getAssignments(), api.getTasks(), agentApi.listCampaigns()])); } catch (err) { setError(errorText(err, "待办数据加载失败")); } finally { setLoading(false); } }
  useEffect(() => { load(); }, []);
  const tasks = useMemo(() => {
    const [assignments, personal, campaigns] = data || [[], [], []];
    return [...list(assignments).map((item) => ({ ...item, kind: "assignment", typeLabel: "课程作业", sourceId: item.id, source: item.course_name || item.class_name || "课程作业", done: isCompletedSubmissionStatus(item.submission_status), progress: item.progress ?? taskAssignmentProgress(item.submission_status), statusLabel: taskAssignmentStatusLabel(item.submission_status), priority: item.priority || "medium", importance: item.importance || "unknown" })), ...list(personal).map((item) => ({ ...item, kind: "personal", typeLabel: "个人待办", sourceId: item.id, source: item.source_name || "个人安排", done: item.status === "completed", progress: item.progress ?? (item.status === "completed" ? 100 : 0), statusLabel: item.status === "completed" ? "已完成" : "待完成", priority: item.priority || "medium", importance: item.importance || "unknown" })), ...list(campaigns).map((item) => ({ ...item, kind: "final-review", typeLabel: "期末复习", sourceId: item.campaign_id, title: `期末复习计划 · ${item.exam_id}`, deadline: null, source: item.run_id ? `智能体运行 · ${item.run_status || "进行中"}` : "复习活动", done: item.run_id ? item.run_status === "SUCCEEDED" : item.status === "ACTIVE", progress: item.run_status === "SUCCEEDED" ? 100 : 0, statusLabel: item.pending_approval_id ? "待审批" : item.run_status === "SUCCEEDED" ? "已完成" : item.run_status === "FAILED" || item.run_status === "CANCELLED" ? "已终止" : item.run_id ? "进行中" : "未启动", priority: "medium", importance: item.pending_approval_id ? "urgent" : "unknown" })), ...localTasks.map((item) => ({ ...item, kind: "local", typeLabel: "本地待办", sourceId: item.id, source: "当前浏览器", deadline: item.deadline || (item.due && item.due !== "待设置" ? item.due : null), description: item.description || item.details || "", done: Boolean(item.done), progress: item.progress ?? (item.done ? 100 : 0), statusLabel: item.done ? "已完成" : "待完成", priority: item.priority || "medium", importance: item.importance || "unknown" }))];
  }, [data, localTasks]);
