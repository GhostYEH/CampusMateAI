...
export function useSceneNarration({ courseId, workspaceId, stageId, sceneId }) {
  ...
  const loadArtifact = useCallback(async (artifactJob, mine) => {
    ...
    const artifact = await api.getOpenMAICArtifact(courseId, artifactJob.artifact_id);
  ...
  const poll = useCallback(async (jobId, mine) => {
    const job = await api.getOpenMAICJob(courseId, jobId);
    if (mine !== epoch.current) return;
  ...
  const generate = useCallback(async () => {
    ...
    setState("generating");
    try {
      const result = await api.synthesizeOpenMAICSceneNarration(courseId, workspaceId, stageId, sceneId, {
