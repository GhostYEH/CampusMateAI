/**
 * CampusMate's small, course-facing copy of magic class's classroom roster.
 *
 ...
 * preset mode. Auto mode leaves the roster to the magic class generator.
 */
export const MAGICCLASS_AGENT_ROLES = [
...
export function normalizeSelectedRoleIds(ids) {
  const known = new Set(MAGICCLASS_AGENT_ROLES.map((role) => role.id));
...
export function selectedRoles(ids) {
  ...
  return MAGICCLASS_AGENT_ROLES.filter((role) => selected.has(role.id));
}
