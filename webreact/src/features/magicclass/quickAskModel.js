 ...
 *
 * 1. **课程 magic class 生成必须进入工作台。** 这条入口不是校园助手的快捷链接；
...
export function workspaceHref(courseId, workspaceId, prompt, options) {
  if (!courseId || !workspaceId) throw new Error("打开 magic class 工作台需要真实的课程和工作台");
...
/**
 * The reference magic class flow always shows a generation preview before any
...
export function generationPreviewHref(courseId, prompt, { mode = "preset", selectedRoleIds = [], webSearch = false } = {}) {
  if (!courseId) throw new Error("打开 magic class 预览需要真实的课程");
  const parts = [
  ...
  return `/courses/${encodeURIComponent(courseId)}/magicclass-preview?${parts.join("&")}`;
}
