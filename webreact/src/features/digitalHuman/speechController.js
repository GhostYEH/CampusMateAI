export class DigitalHumanSpeechController {
  ...
  async speak(text) {
    ...
    try {
      await this.streamSpeech(text, { signal: aborter.signal, onChunk: (chunk) => this.player.append(chunk) });
  ...
  stop() { this.aborter?.abort(); this.aborter = null; this.player.stop(); }
}
