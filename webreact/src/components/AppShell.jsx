...
export default function AppShell() {
  ...
  const isOpenMAIC = location.pathname === "/courses" || location.pathname.includes("/openmaic-preview") || location.pathname.includes("/workspaces/");
  ...
  return <div style={studyBackgroundStyle} className={`app-layout ${isCourses ? "iridescence-background-active" : ""} ${isOpenMAIC ? "openmaic-shell" : ""} ${isCounselor ? "counselor-mode" : ""} ${isProfile ? "profile-mode" : ""} ${isStudy ? "study-mode" : ""} ${reduceMotion ? "reduce-motion" : ""}`}>{pageEffects}{!isProfile && <SmoothCursor pointsCount={32} lineWidth={0.45} springStrength={0.38} dampening={0.52} color="var(--blue)" blur={3} mixBlendMode="screen" velocityScale trailOpacity={0.24} smoothFactor={1.4} paused={motionPaused} />}<a className="skip-link" href="#main-content">跳到主要内容</a>
