...
function SearchBox() {
  ...
  useEffect(() => {
    ...
    timer.current = setTimeout(async () => {
      ...
      try {
        ...
        setResults([
          ...(list(courses).filter((item) => includes(item.name, item.code, item.semester)).map((item) => ({ title: item.name, detail: item.code || "?¦Ã?????", path: `/courses/${item.id}` }))),
          ...(list(assignments).filter((item) => includes(item.title, item.course_name, item.class_name)).map((item) => ({ title: item.title, detail: item.course_name || "?¦Ã????", path: `/tasks/assignment/${item.id}` }))),
        ].slice(0, 8));
  ...
  return <div className="search-wrap"><GlassSurface className="topbar-search-surface" width="100%" height={64} borderRadius={999} backgroundOpacity={0.04} saturation={1.18} distortionScale={-120} redOffset={0} greenOffset={0} blueOffset={0}><label className="topbar-search"><Icon name="PhMagnifyingGlass" size={17} /><input name="global-search" value={query} onFocus={() => setOpen(true)} onChange={(event) => { setQuery(event.target.value); setOpen(true); }} onKeyDown={submitSearch} placeholder="?????¦Ã???????" /><kbd>? K</kbd></label></GlassSurface>{open && query.trim().length >= 2 && <div className="search-results" role="listbox">{loading ? <span>??????????</span> : <>{results.map((item) => <button key={item.path} onClick={() => { navigate(item.path); setQuery(""); setOpen(false); }}><Icon name="PhArrowUpRight" size={15} /><span><strong>{item.title}</strong><small>{item.detail}</small></span></button>)}<button className="search-home-result" onClick={() => { navigate(`/home?q=${encodeURIComponent(query.trim())}`); setQuery(""); setOpen(false); }}><Icon name="PhMagnifyingGlass" size={15} /><span><strong>?????????????</strong><small>{query.trim()}</small></span></button></>}</div>}</div>;
}
...
export default function AppShell() {
  ...
  const today = useMemo(() => new Intl.DateTimeFormat("zh-CN", { month: "long", day: "numeric", weekday: "short" }).format(new Date()).replace("????", "??"), []);
  ...
  return <div className={`app-layout ${isHome ? "home-background-active" : ""} ${isCounselor ? "counselor-mode" : ""} ${isProfile ? "profile-mode" : ""} ${reduceMotion ? "reduce-motion" : ""}`}>{isHome && <Iridescence className="app-iridescence" color={IRIDESCENCE_COLOR} speed={0.38} amplitude={0.14} mouseReact={!motionPaused} paused={motionPaused} />}{isCounselor && <Prism className="app-counselor-prism" animationType="3drotate" timeScale={0.34} height={4.8} baseWidth={7.4} scale={2.2} hueShift={-0.12} colorFrequency={1.05} noise={0.012} glow={0.75} bloom={1.1} suspendWhenOffscreen={false} paused={motionPaused} />}{isProfile && <Prism className="app-profile-prism" animationType="3drotate" timeScale={0.2} height={4.3} baseWidth={5.8} scale={1.25} hueShift={0.1} colorFrequency={0.95} noise={0.008} glow={0.9} bloom={1.05} suspendWhenOffscreen={false} paused={motionPaused} />}<SmoothCursor pointsCount={32} lineWidth={0.45} springStrength={0.38} dampening={0.52} color="var(--blue)" blur={3} mixBlendMode="screen" velocityScale trailOpacity={0.24} smoothFactor={1.4} paused={motionPaused} /><a className="skip-link" href="#main-content">???????????</a>
    <div className="app-content"><header className="topbar"><SearchBox /><FloatingNav tone={floatingNavTone} pendingCount={pendingCount} unreadCount={unreadCount} reduceMotion={reduceMotion} /><GlassSurface className="topbar-info-surface" width="auto" height={64} borderRadius={999} backgroundOpacity={0.04} saturation={1.18} distortionScale={-120} redOffset={0} greenOffset={0} blueOffset={0}><div className="topbar-info"><span className="topbar-date">{today}</span><div className="topbar-actions"><button className="icon-button" aria-label="??" onClick={() => navigate("/notifications")}><Icon name="PhBell" size={18} /></button><button className="topbar-account" aria-label="????????" onClick={() => navigate("/profile")}><span className="avatar"><img src={session?.avatar_url || "/assets/generated/home-reference-student-avatar.png"} alt="" width="30" height="30" /></span><Icon name="PhCaretDown" size={14} /></button></div></div></GlassSurface></header><div className="route-stage"><Outlet /></div></div>
  </div>;
}
