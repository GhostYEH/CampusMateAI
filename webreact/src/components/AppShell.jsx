...
function SearchBox() {
  ...
  return <div className="search-wrap"><GlassSurface className="topbar-search-surface" width="100%" height={64} borderRadius={999} backgroundOpacity={0.04} saturation={1.18} distortionScale={-120}><label className="topbar-search"><Icon name="PhMagnifyingGlass" size={17} /><input name="global-search" value={query} onFocus={() => setOpen(true)} onChange={(event) => { setQuery(event.target.value); setOpen(true); }} onKeyDown={submitSearch} placeholder="搜索课程、作业…" /><kbd>? K</kbd></label></GlassSurface>{open && query.trim().length >= 2 && <div className="search-results" role="listbox">{loading ? <span>正在搜索…</span> : <>{results.map((item) => <button key={item.path} onClick={() => { navigate(item.path); setQuery(""); setOpen(false); }}><Icon name="PhArrowUpRight" size={15} /><span><strong>{item.title}</strong><small>{item.detail}</small></span></button>)}<button className="search-home-result" onClick={() => { navigate(`/home?q=${encodeURIComponent(query.trim())}`); setQuery(""); setOpen(false); }}><Icon name="PhMagnifyingGlass" size={15} /><span><strong>在首页筛选全部结果</strong><small>{query.trim()}</small></span></button></>}</div>}</div>;
}
