...
function SearchBox() {
  ...
  useEffect(() => {
    ...
    timer.current = setTimeout(async () => {
      ...
      try {
        ...
        setResults([
          ...(list(courses).filter((item) => includes(item.name, item.code, item.semester)).map((item) => ({ title: item.name, detail: item.code || "????????", path: `/courses/${item.id}` }))),
          ...(list(assignments).filter((item) => includes(item.title, item.course_name, item.class_name)).map((item) => ({ title: item.title, detail: item.course_name || "???????", path: `/tasks/assignment/${item.id}` }))),
        ].slice(0, 8));
  ...
  return <div className="search-wrap"><GlassSurface className="topbar-search-surface" width="100%" height={64} borderRadius={999} backgroundOpacity={0.04} saturation={1.18} distortionScale={0} redOffset={0} greenOffset={0} blueOffset={0}><label className="topbar-search"><Icon name="PhMagnifyingGlass" size={17} /><input name="global-search" value={query} onFocus={() => setOpen(true)} onChange={(event) => { setQuery(event.target.value); setOpen(true); }} onKeyDown={submitSearch} placeholder="??????????????" /><kbd>? K</kbd></label></GlassSurface>{open && query.trim().length >= 2 && <div className="search-results" role="listbox">{loading ? <span>??????????</span> : <>{results.map((item) => <button key={item.path} onClick={() => { navigate(item.path); setQuery(""); setOpen(false); }}><Icon name="PhArrowUpRight" size={15} /><span><strong>{item.title}</strong><small>{item.detail}</small></span></button>)}<button className="search-home-result" onClick={() => { navigate(`/home?q=${encodeURIComponent(query.trim())}`); setQuery(""); setOpen(false); }}><Icon name="PhMagnifyingGlass" size={15} /><span><strong>?????????????</strong><small>{query.trim()}</small></span></button></>}</div>}</div>;
}
