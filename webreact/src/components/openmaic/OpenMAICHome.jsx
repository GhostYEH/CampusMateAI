...
export default function OpenMAICHome({ courses = [], assignments = [], recentItems = [], fusion = null, providerStatus = null, recentError = "" }) {
  ...
  return <section className="openmaic-home openmaic-home--reference" aria-label="magic'class 首页"><div className="openmaic-reference-glow openmaic-reference-glow--top" aria-hidden="true" /><div className="openmaic-reference-glow openmaic-reference-glow--bottom" aria-hidden="true" /><div className="openmaic-reference__brand"><span className="openmaic-reference__mark"><Icon name="PhCube" size={29} weight="duotone" /></span><span><strong>{"magic'class"}</strong><small>Generative Learning in Multi-Agent Interactive Classroom</small></span></div>
    ...
    <RecentClassrooms items={recentItems} error={recentError} /><CourseLibrary courses={courses} assignments={assignments} /><p className="openmaic-reference-footer">{"magic'class Open Source Project"}</p>
  </section>;
}
