...
export default function DiscoveryPanel({ courseId, canBrowseFolders = false, canSearch = false }) {
  ...
  const load = React.useCallback(async () => {
    ...
    setError("");
    try {
      const payload = await api.listOpenMAICFolders(courseId, { limit: DISCOVERY_PAGE_LIMIT });
  ...
  async function createFolder(event) {
    ...
    try {
      await api.createOpenMAICFolder(courseId, { name: normalized, idempotencyKey: pendingKey.current });
  ...
  async function removeFolder(folder) {
    ...
    setNotice("");
    try {
      await api.deleteOpenMAICFolder(courseId, folder.id, { revision: folder.revision });
  ...
  async function runSearch(event, { append = false } = {}) {
    ...
    setSearchError("");
    try {
      const payload = await api.searchOpenMAICContent(courseId, {
        query: query.trim(),
  ...
  return <Panel className="openmaic-discovery-panel">
    <SectionHeading title="内容整理" detail="文件夹与站内搜索" />

    {notice ? <p className="openmaic-discovery__notice" role="status">{notice}</p> : null}
    {error ? <p className="openmaic-discovery__error" role="alert">{error}</p> : null}

    {canBrowseFolders ? <div className="openmaic-discovery__folders">
      <form className="openmaic-discovery__create" onSubmit={createFolder}>
        <label className="openmaic-discovery__input">
      ...
      {loading ? <p className="muted-copy">正在读取文件夹…</p>
        : tree.roots.length ? <ul className="openmaic-folder-tree">
    ...
    {canSearch ? <div className="openmaic-discovery__search">
      <form className="openmaic-discovery__create" onSubmit={runSearch}>
        <label className="openmaic-discovery__input">
      ...
      {searchError ? <p className="openmaic-discovery__error" role="alert">{searchError}</p> : null}

      {searchedFor && !hits.length && !searchError ? <p className="muted-copy">没有找到与「{searchedFor}」匹配的内容。</p> : null}

      {hits.length ? <div className="openmaic-search-hits">
        <p className="muted-copy">「{searchedFor}」的匹配结果</p>
        {hits.map((hit) => hit.href ? <Link className="openmaic-search-hit" key={searchHitKey(hit)} to={hit.href}>
          <span className="openmaic-search-hit__kind">{hit.kind === "stage" ? "内容" : "工作台"}</span>
          <strong>{hit.title}</strong>
          {hit.snippet ? <small>{hit.snippet}</small> : null}
          <span className="openmaic-search-hit__date">{dateText(hit.updatedAt)}</span>
        </Link> : <div className="openmaic-search-hit" key={searchHitKey(hit)}>
          <span className="openmaic-search-hit__kind">{hit.kind === "stage" ? "内容" : "工作台"}</span>
          <strong>{hit.title}</strong>
...
function FolderRow({ folder, tree, expanded, onToggle, onRemove, busy }) {
  ...
  return <li className="openmaic-folder-tree__node">
    <div className="openmaic-folder-tree__row">
      {children.length ? <button
        type="button"
        className="openmaic-folder-tree__toggle"
        aria-expanded={isOpen}
        aria-label={`${isOpen ? "收起" : "展开"}「${folder.name}」`}
        onClick={() => onToggle(folder.id)}
      ><Icon name={isOpen ? "PhCaretDown" : "PhCaretRight"} size={14} /></button>
        : <span className="openmaic-folder-tree__spacer" aria-hidden="true" />}
      <Icon name="PhFolderSimple" size={17} />
      <span className="openmaic-folder-tree__name">{folder.name}</span>
    ...
    {children.length && isOpen ? <ul className="openmaic-folder-tree__children">
