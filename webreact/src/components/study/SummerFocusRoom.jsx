...
export default function SummerFocusRoom({
  ...
  useEffect(() => {
    ...
    if (SCENES.some((item) => item.key === stored)) setScene(stored);
  }, []);
  ...
  const activeLabel = active?.status === "paused" ? "暂时休息" : "正在专注";
      ...
      <div className="study-summer-grid">
        <section className={`study-summer-focus ${active ? "is-active" : ""}`} aria-label="专注计时">
          <div className="study-summer-focus__heading">
            ...
            <span className="study-summer-focus__status">{active ? activeLabel : "尚未开始"}</span>
          </div>

          <div className="study-summer-timer" aria-live="polite">
            <strong>{timerText(seconds)}</strong>
            <span>{active ? activeLabel : `${selectedMinutes} 分钟`}</span>
          </div>

          {!active && (
            <div className="study-summer-presets" role="group" aria-label="选择专注时长">
              {[25, 45, 60].map((minutes) => (
                <button key={minutes} type="button" className={preset === minutes ? "is-active" : ""} onClick={() => onPresetChange(minutes)}>
                  {minutes} 分钟
                </button>
              ))}
              <button type="button" className={preset === "custom" ? "is-active" : ""} onClick={() => onPresetChange("custom")}>自定义</button>
              {preset === "custom" && <input type="number" min="5" max="180" aria-label="自定义专注分钟数" value={customMinutes} onChange={(event) => onCustomMinutesChange(event.target.value)} />}
            </div>
          )}
          ...
          {active ? (
            ...
            <form className="study-summer-start" onSubmit={(event) => { event.preventDefault(); onStart(); }}>
