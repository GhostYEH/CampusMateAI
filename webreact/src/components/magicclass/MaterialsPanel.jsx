...
} from "../../features/magicclass/materialsModel.js";
...
export default function MaterialsPanel({ courseId, canManageMaterials = false }) {
  ...
  const load = React.useCallback(async ({ append = false, cursor: pageCursor = null } = {}) => {
    ...
    setError("");
    try {
      const payload = await api.listMagicClassMaterials(courseId, {
  ...
  async function submit(event) {
    ...
    try {
      const created = normalizeMaterialList({ items: [await api.uploadMagicClassMaterial(courseId, {
        file,
  ...
  async function remove(material) {
    ...
    setNotice("");
    try {
      await api.deleteMagicClassMaterial(courseId, material.id, { revision: material.revision });
  ...
  async function open(material) {
    ...
    try {
      const payload = await api.getMagicClassMaterial(courseId, material.id);
  ...
  return <Panel className="magicclass-materials-panel">
    <SectionHeading title="课程资料" detail={`上传后由服务端解析正文，单个文件上限 ${formatBytes(MAX_UPLOAD_BYTES)}`} />

    {notice ? <p className="magicclass-materials__notice" role="status">{notice}</p> : null}
    {error ? <p className="magicclass-materials__error" role="alert">{error}</p> : null}
    {localReject ? <p className="magicclass-materials__error" role="alert">{localReject}</p> : null}

    <form className="magicclass-materials__upload" onSubmit={submit}>
      <label className="magicclass-materials__picker">
    ...
    {loading && !materials.length ? <p className="muted-copy">正在读取课程资料…</p>
      : materials.length ? <ul className="magicclass-materials">
        {materials.map((material) => {
          const described = describeExtractionStatus(material.extractionStatus);
          return <li className="magicclass-material" key={material.id}>
            <div className="magicclass-material__row">
              <Icon name="PhFileText" size={18} />
              <span className="magicclass-material__name">{material.filename}</span>
              <span className={`magicclass-material__status magicclass-material__status--${described.tone}`}>
                {described.label}
              </span>
    ...
    {detail ? <div className="magicclass-material-detail">
      <div className="magicclass-material-detail__head">
      ...
      {detail.extractionStatus === "extracted"
        ? <pre className="magicclass-material-detail__text">{detail.text}</pre>
