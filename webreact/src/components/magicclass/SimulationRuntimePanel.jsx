...
import { createSimulationState, stepSimulation } from "../../features/magicclass/sceneRuntimeModel.js";
...
export default function SimulationRuntimePanel({ content, sceneId }) {
  ...
  return <section className="magicclass-simulation-runtime" aria-label={`${title}实验`} data-scene-id={sceneId || undefined}>
    <div className="magicclass-runtime-cover">
      <span className="magicclass-runtime-kicker">互动实验</span>
      <strong>{title}</strong>
      <p>调整参数，运行实验，观察结果变化。</p>
    </div>
    <div className="magicclass-simulation-runtime__controls" aria-label="实验参数">
      {parameters.map((parameter) => <label className="magicclass-simulation-runtime__control" key={parameter.id}>
        ...
        />
        <span className="magicclass-simulation-runtime__value">{state.values[parameter.id]} {parameter.unit || ""}</span>
      </label>)}
    </div>
    <div className="magicclass-stage-heading-actions">
      <Button type="button" onClick={run}>运行实验</Button>
      <Button type="button" variant="secondary" onClick={reset}>重置实验</Button>
    </div>
    {state.hasRun && state.result ? <div className="magicclass-simulation-runtime__result" role="status" aria-label="实验结果">
      <span>{state.result.label}</span>
      <strong>{state.result.value}</strong>
      <small>根据当前参数计算得到</small>
    </div> : <p className="magicclass-hint">调整参数后点击“运行实验”查看结果。</p>}
  </section>;
}
