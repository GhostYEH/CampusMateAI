...
import {
  SCENE_TYPE_LABELS,
} from "../../features/magicclass/editorModel.js";
import {
  actionTimeline,
  degradeNotice,
  ...
  playerNavigation,
  sandboxPolicyFor,
} from "../../features/magicclass/playerModel.js";
...
export default function StagePlayerPanel({ courseId, workspaceId, stageId, startSceneId = "", onClose }) {
  ...
  const load = React.useCallback(async () => {
    ...
    setError("");
    try {
      const payload = await api.getMagicClassStagePlayback(courseId, workspaceId, stageId, {
  ...
  React.useEffect(() => {
    ...
    void (async () => {
      try {
        const payload = await api.getMagicClassStageScene(courseId, workspaceId, stageId, currentScene.id);
  ...
  return <Panel className="magicclass-player">
    <SectionHeading
      title="场景播放"
      detail={plan ? `${plan.title} · 第 ${nav.total ? nav.index + 1 : 0}/${nav.total} 个场景` : "读取中"}
    />

    <div className="magicclass-player__toolbar">
    ...
    {loading ? <p className="magicclass-hint">正在读取播放计划…</p> : null}

    {currentScene ? <>
      <div className="magicclass-player__stage" data-render-kind={currentScene.render.kind}>
        <header className="magicclass-player__title">
          <span className="magicclass-player__type">{SCENE_TYPE_LABELS[currentScene.type] || currentScene.type}</span>
        ...
        {policy.allowIframe && currentScene.render.widget_type !== "simulation" ? <iframe
          className="magicclass-player__frame"
        ...
        {currentScene.render.kind === "unsupported" ? <div className="magicclass-player__degraded" role="status">
      ...
      {timeline.length ? <ol className="magicclass-player__timeline" aria-label="场景动作时间线">
      ...
      {currentScene.droppedActions.length ? <p className="magicclass-hint">
...
function NativeScene({ scene, fallbackTitle, courseId, workspaceId, stageId }) {
  ...
  if (!content) {
    return <p className="magicclass-hint">「{fallbackTitle}」的正文暂时读不到，仅显示标题。</p>;
  }
  ...
  if (content.type === "slide") {
    ...
    return <div className="magicclass-player__slide">
      ...
      </li>)}</ul> : <p className="magicclass-hint">这张幻灯片还没有内容元素。</p>}
      {Array.isArray(scene.whiteboards) && scene.whiteboards.length ? <div className="magicclass-player__whiteboards" aria-label="白板内容">
  ...
  return <p className="magicclass-hint">该场景类型的内容由 CampusMate 原生渲染。</p>;
}
