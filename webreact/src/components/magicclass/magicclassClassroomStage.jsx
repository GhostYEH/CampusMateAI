 ...
 */
export default function MagicClassClassroomStage({
  courseId,
  workspaceId,
  stageId,
  fallbackTitle = "",
  ...
  const load = React.useCallback(async () => {
    ...
    try {
      ...
      const [playback, stageDocument] = await Promise.all([
        api.getMagicClassStagePlayback(courseId, workspaceId, stageId),
        api.getMagicClassStage(courseId, workspaceId, stageId),
      ]);
  ...
  const runDiscussion = React.useCallback(async () => {
    ...
    setMessages([]);
    try {
      const started = await api.runMagicClassDiscussion(courseId, {
        prompt: topic.trim(),
  ...
  return <div className={cn(
    ...
    {onToggleActionTimeline ? <div className="flex items-center gap-1 min-w-0 shrink-0">
      <button
        ...
        data-testid="magicclass-action-playback-toggle"
      >
        {actionSession?.status === "playing" ? <Pause className="w-3.5 h-3.5" /> : <Play className="w-3.5 h-3.5" />}
      </button>
      {playbackNotice ? <output className="text-[10px] text-gray-400 dark:text-gray-500 truncate max-w-40" data-testid="magicclass-action-playback-status">
        {playbackNotice}
      </output> : null}
    </div> : null}
