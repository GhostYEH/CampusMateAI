...
export default function StageEditorPanel({ courseId, workspaceId, stageId, onSaved }) {
  ...
  const load = React.useCallback(async () => {
    ...
    try {
      ...
      const stagePayload = await api.getMagicClassStage(courseId, workspaceId, stageId);
  ...
  async function editScene(sceneId) {
    ...
    try {
      ...
      if (!scene) scene = await api.getMagicClassStageScene(courseId, workspaceId, stageId, sceneId);
      if (mine !== epoch.current) return;
  ...
  async function save() {
    ...
    try {
      ...
      for (const commands of batches) {
        ...
        last = await api.applyMagicClassStageCommands(courseId, workspaceId, stageId, {
          commands,
  ...
  return <Panel className="magicclass-stage-editor">
    <SectionHeading
      title="场景编辑"
    ...
    <div className="magicclass-stage-editor__toolbar">
      <label className="magicclass-stage-editor__type">
        <span>新建</span>
    ...
    {loading ? <p className="magicclass-hint">正在读取场景…</p>
      : scenes.length ? <div className="magicclass-stage-editor__workspace">
        <ol className="magicclass-scene-list">
        {scenes.map((scene, index) => <li
          className={`magicclass-scene-item${selectedScene?.id === scene.id ? " is-selected" : ""}`}
          key={scene.id}
        >
          <span className="magicclass-scene-item__order">{index + 1}</span>
          <span className="magicclass-scene-item__type">{SCENE_TYPE_LABELS[scene.type] || scene.type}</span>
          <button type="button" className="magicclass-scene-item__select" aria-label={`预览场景：${scene.title}`} aria-pressed={selectedScene?.id === scene.id} onClick={() => setSelectedSceneId(scene.id)}><span className="row-copy"><strong>{scene.title}</strong>
    ...
    {editingScene ? <section className="magicclass-scene-content-editor" aria-label="场景内容编辑器">
