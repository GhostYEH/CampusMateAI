...
import { pblProgress } from "../../features/magicclass/sceneRuntimeModel.js";

function storageKey(sceneId) { return sceneId ? `campusmate:magicclass:pbl:${sceneId}` : ""; }
...
export default function PblRuntimePanel({ content, sceneId }) {
  ...
  return <section className="magicclass-pbl-runtime" aria-label="项目式学习任务">
    <div className="magicclass-runtime-result">
      <span className="magicclass-runtime-kicker">项目式学习</span>
      <strong>{progress.completed} / {progress.total} 项任务已完成</strong>
      <p>{progress.percent === 100 ? "项目已完成，可以整理并提交你的结论。" : "按阶段完成任务，逐步形成最终交付物。"}</p>
    </div>
    {phases.map((phase, index) => <article className="magicclass-pbl-runtime__phase" key={phase.id || index}>
      <h3>{index + 1}. {phase.title || "项目阶段"}</h3>
      {(Array.isArray(phase.tasks) ? phase.tasks : []).map((task, taskIndex) => <label className="magicclass-pbl-runtime__task" key={task.id || taskIndex}>
