...
} from "../src/features/magicclass/workbenchLayoutModel.js";

const read = (rel) => readFileSync(new URL(`../${rel}`, import.meta.url), "utf8");
const layoutSource = read("src/features/magicclass/workbenchLayoutModel.js");
const containSource = read("src/components/magicclass/ContainBox.jsx");
const workbenchSource = read("src/pages/magicclassWorkbenchPage.jsx");
const cssSource = read("src/styles/magicclass-workbench.css");
...
test("the workbench renders exactly the panes the layout resolves", () => {
  ...
  assert.match(workbenchSource, /<MagicClassClassroomStage/,
...
test("the workbench reuses the existing panels and API rather than reimplementing them", () => {
  ...
  assert.match(workbenchSource, /api\.getMagicClassStagePlayback|StagePlayerPanel/);
  assert.match(workbenchSource, /api\.generateMagicClassStage/);
  assert.match(workbenchSource, /api\.exportMagicClassStageFormat/);
  // 不得引入预览页那条被推翻的路径。
...
test("runtimes stay reachable through the player, not reimplemented in the workbench", () => {
  const player = read("src/components/magicclass/StagePlayerPanel.jsx");
