...
const previewSource = fs.readFileSync(new URL("../src/pages/OpenMAICGenerationPreviewPage.jsx", import.meta.url), "utf8");
...
test("generation preview is a real route and does not jump to counselor", () => {
  ...
  assert.match(previewSource, /api\.listOpenMAICWorkspaces\(courseId/);
  assert.match(previewSource, /api\.createOpenMAICWorkspace\(courseId/);
...
test("preview preserves the G-project flow requirements", () => {
  ...
  assert.match(previewSource, /openmaicAttachment/);
  assert.match(previewSource, /openmaicWebSearch/);
});
