...
import {
  ...
  defaultMagicClassCourseId,
  describeFusionState,
  filterMagicClassHomeItems,
  normalizeRecentItems,
} from "../src/features/magicclass/homeModel.js";
...
const roleSource = fs.readFileSync(new URL("../src/features/magicclass/roleModel.js", import.meta.url), "utf8");
...
test("home search filters only records supplied by the server", () => {
  const results = filterMagicClassHomeItems(
    [{ id: "s1", courseName: "数据结构", title: "复习课堂" }],
    "数据",
  );
  assert.equal(results.length, 1);
  assert.deepEqual(filterMagicClassHomeItems([], "数据"), []);
});

test("courses route calls the aggregate endpoints instead of per-course history", () => {
  assert.match(pageSource, /MagicClassHome/);
  assert.match(pageSource, /api\.getMagicClassRecent\(/);
  assert.match(pageSource, /api\.getMagicClassFusionStatus\(/);
...
test("course context defaults to the first real course after asynchronous loading", () => {
  assert.equal(defaultMagicClassCourseId([]), "");
  assert.equal(defaultMagicClassCourseId([{ id: "c1", name: "数据结构" }]), "c1");
  assert.equal(defaultMagicClassCourseId([{ id: 42, title: "大学英语" }]), "42");
});
...
test("home uses the reference root composition and submits a real course-bound prompt", () => {
  ...
  assert.match(homeSource, /api\.generateMagicClassHome/);
  assert.doesNotMatch(homeSource, /api\.createMagicClassWorkspace/);
  assert.doesNotMatch(homeSource, /api\.generateMagicClassStage/);
  assert.match(homeSource, /stageGenerationIdempotencyKey/);
  assert.match(homeSource, /最近课堂/);
  assert.doesNotMatch(homeSource, /更多学习工具/);
  assert.match(homeSource, /<details className="magicclass-reference-library"/);
});
