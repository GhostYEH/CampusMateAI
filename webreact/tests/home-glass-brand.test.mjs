...
test("homepage brand canvas uses a visible CSS stage without image decoration", () => {
  ...
  assert.match(baseStyles, /\.home-footer-brand\s*\{[^}]*background:\s*radial-gradient/);
  assert.doesNotMatch(baseStyles, /\.home-footer-brand::before/);
  assert.doesNotMatch(baseStyles, /\.home-footer-brand::after/);
  assert.match(baseStyles, /\.home-footer-fixed-brand \.home-brand-underlay \.home-footer-brand\s*\{[^}]*border-top:/);
