...
test("search sends the trimmed query and omits an absent cursor", async () => {
  ...
  await apiModule.searchOpenMAICContent("c1", { query: "线性代数", limit: 20 });
...
test("search carries the cursor back verbatim when paging", async () => {
  const mock = createMockClient(apiModule.default);
  mock.onGet("/courses/c1/search", { items: [], next_cursor: null });
  await apiModule.searchOpenMAICContent("c1", { query: "复习", limit: 20, cursor: "abc" });
...
test("a search that cannot reach the service is retryable; a bad query is not", () => {
  const unavailable = describeDiscoveryError({ response: { status: 503, data: { code: "OPENMAIC_FUSION_UNAVAILABLE" } } });
  assert.equal(unavailable.kind, "unavailable");
  assert.equal(unavailable.retryable, true);

  const invalid = describeDiscoveryError({ response: { status: 400, data: { code: "OPENMAIC_INVALID_REQUEST", message: "搜索关键词不能为空" } } });
