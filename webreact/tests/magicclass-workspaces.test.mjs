...
} from "../src/features/magicclass/workspaceModel.js";
...
test("creating a workspace sends the idempotency key the protocol requires", async () => {
  ...
  await apiModule.createMagicClassWorkspace("c1", { name: "期末复习", idempotencyKey: "key-1" });
...
test("updating and deleting send the revision back as If-Match", async () => {
  ...
  await apiModule.updateMagicClassWorkspace("c1", "ws_1", { revision: 1, name: "新名字" });
  assert.equal(mock.lastRequest().headers["If-Match"], "1");
  assert.deepEqual(mock.lastRequest().data, { name: "新名字" });

  await apiModule.deleteMagicClassWorkspace("c1", "ws_1", { revision: 2 });
...
test("replacing a stage sends both the document and the revision", async () => {
  ...
  await apiModule.replaceMagicClassStage("c1", "ws_1", "stg_1", { revision: 1, document });
...
test("an omitted document is not sent as null", async () => {
  ...
  await apiModule.createMagicClassStage("c1", "ws_1", { title: "第一节", idempotencyKey: "k" });
...
test("listing workspaces passes pagination through as query parameters", async () => {
  ...
  await apiModule.listMagicClassWorkspaces("c1", { limit: 5, cursor: "abc" });
