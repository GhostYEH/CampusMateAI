...
const workspaceSource = read("src/components/openmaic/WorkspacePanel.jsx");
...
test("idempotency, unavailable, notFound and invalid each keep their own kind", () => {
  const cases = [
    ...
    [{ response: { status: 503, data: { code: "OPENMAIC_FUSION_UNAVAILABLE" } } }, "unavailable"],
...
test("saving sends commands with If-Match and an Idempotency-Key, never a document", async () => {
  ...
  await apiModule.applyOpenMAICStageCommands("c1", "ws_1", "stg_1", {
...
test("the outline and a single scene are read from the editor endpoints", async () => {
  ...
  await apiModule.getOpenMAICStageOutline("c1", "ws_1", "stg_1");
  await apiModule.getOpenMAICStageScene("c1", "ws_1", "stg_1", "scn_1");
...
test("the editor model exposes no unbounded history", () => {
  ...
  assert.match(read("src/features/openmaic/editorModel.js"), /while \(past\.length > limit\) past\.shift\(\);/);
});
