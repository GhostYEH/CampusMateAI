...
} from "../src/features/magicclass/archiveModel.js";
...
test("a rejected archive surfaces the service's own reason", () => {
  const described = describeArchiveError({
    response: { status: 422, data: { code: "MAGICCLASS_DOCUMENT_REJECTED", message: "内容未通过校验", details: { service_message: "archive has no manifest.json" } } },
  });
...
test("exporting asks for a blob from the stage export route", async () => {
  ...
  await apiModule.exportMagicClassStage("c1", "ws_1", "stg_1");
...
test("importing posts a multipart form with the idempotency key", async () => {
  ...
  await apiModule.importMagicClassStage("c1", "ws_1", { file, idempotencyKey: "key-1" });
...
test("format export uses a blob route and PPTX import keeps the multipart boundary", async () => {
  ...
  await apiModule.exportMagicClassStageFormat("c1", "ws_1", "stg_1", "markdown");
  ...
  await apiModule.importMagicClassPptx("c1", "ws_1", { file, idempotencyKey: "pptx-key" });
  const request = mock.lastRequest();
...
test("the panel gates both entries on the capability and never renders them unconditionally", () => {
  ...
  assert.match(panelSource, /\{canImportArchive \? <div className="magicclass-command">/);
  assert.match(panelSource, /\{canImportPptx \? <div className="magicclass-command">/);
  assert.match(panelSource, /exportMagicClassStageFormat/);
});
