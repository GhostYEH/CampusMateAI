...
} from "../src/features/openmaic/discoveryModel.js";
import { normalizeWorkspaceList } from "../src/features/openmaic/workspaceModel.js";

const read = (rel) => readFileSync(fileURLToPath(new URL(`../${rel}`, import.meta.url)), "utf8");
const panelSource = read("src/components/openmaic/DiscoveryPanel.jsx");
...
test("creating a folder sends the idempotency key and only sends parent_id when set", async () => {
  ...
  await apiModule.createOpenMAICFolder("c1", { name: "第一章", idempotencyKey: "key-1" });
  ...
  await apiModule.createOpenMAICFolder("c1", { name: "第一节", parentId: "fd_1", idempotencyKey: "key-2" });
...
test("renaming a folder carries If-Match and keeps an explicit null parent", async () => {
  ...
  await apiModule.updateOpenMAICFolder("c1", "fd_1", { revision: 1, name: "新名字" });
  ...
  await apiModule.updateOpenMAICFolder("c1", "fd_1", { revision: 2, parentId: null });
...
test("deleting a folder carries If-Match", async () => {
  ...
  await apiModule.deleteOpenMAICFolder("c1", "fd_1", { revision: 4 });
...
test("a workspace can be filed and unfiled through the same update call", async () => {
  ...
  await apiModule.updateOpenMAICWorkspace("c1", "ws_1", { revision: 1, folderId: "fd_1" });
  assert.equal(mock.requests.at(-1).data.folder_id, "fd_1");

  await apiModule.updateOpenMAICWorkspace("c1", "ws_1", { revision: 2, folderId: null });
  assert.equal(mock.requests.at(-1).data.folder_id, null, "取消归档必须显式传 null");

  await apiModule.updateOpenMAICWorkspace("c1", "ws_1", { revision: 3, name: "只改名" });
...
test("discovery errors are translated into actionable kinds", () => {
  const cases = [
    [{ response: { status: 503, data: { code: "OPENMAIC_FUSION_UNAVAILABLE" } } }, "unavailable", true],
    [{ response: { status: 409, data: { code: "OPENMAIC_REVISION_CONFLICT" } } }, "conflict", false],
    [{ response: { status: 404, data: { code: "OPENMAIC_WORKSPACE_NOT_FOUND" } } }, "notFound", false],
    [{ response: { status: 400, data: { code: "OPENMAIC_INVALID_REQUEST" } } }, "invalid", false],
    [{ response: { status: 409, data: { code: "OPENMAIC_IDEMPOTENCY_CONFLICT" } } }, "idempotency", false],
  ];
...
test("an unknown failure is retryable and never leaks the raw error text", () => {
  const described = describeDiscoveryError({ response: { status: 500, data: { message: "internal http://openmaic.internal:4010" } } });
...
test("the browser never talks to the managed service directly", () => {
  for (const source of [panelSource, apiSource]) {
    assert.doesNotMatch(source, /openmaic\.internal/);
    assert.doesNotMatch(source, /OPENMAIC_INTERNAL_SECRET|openmaic_service_url/i);
  }
...
test("the filing controls only exist when the folder capability was reported", () => {
  ...
  assert.match(panelSource, /api\.updateOpenMAICWorkspace\(courseId, item\.id, \{\s*revision: item\.revision,/);
  // 取消归档必须显式传 null。
