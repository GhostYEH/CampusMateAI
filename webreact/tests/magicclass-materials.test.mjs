...
} from "../src/features/magicclass/materialsModel.js";

const read = (rel) => readFileSync(fileURLToPath(new URL(`../${rel}`, import.meta.url)), "utf8");
const panelSource = read("src/components/magicclass/MaterialsPanel.jsx");
...
test("a rejected upload and an unavailable service are different kinds", () => {
  const rejected = describeMaterialError({ response: { status: 422, data: { code: "MAGICCLASS_DOCUMENT_REJECTED" } } });
...
test("uploading sends a multipart form with the idempotency key", async () => {
  ...
  await apiModule.uploadMagicClassMaterial("c1", { file, idempotencyKey: "key-1" });
...
test("deleting sends the revision back as If-Match", async () => {
  ...
  await apiModule.deleteMagicClassMaterial("c1", "mt_1", { revision: 4 });
...
test("resolving posts the ids it was given and nothing else", async () => {
  ...
  await apiModule.resolveMagicClassMaterials("c1", { materialIds: ["mt_1", "mt_2"] });
...
test("listing and reading a material stay on GET", async () => {
  ...
  await apiModule.listMagicClassMaterials("c1", { limit: 5 });
  assert.equal(mock.lastRequest().params.limit, 5);
  await apiModule.getMagicClassMaterial("c1", "mt_1");
