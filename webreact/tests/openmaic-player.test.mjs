...
import {
  ...
  playerNavigation,
  sandboxPolicyFor,
} from "../src/features/openmaic/playerModel.js";
...
test("playback failures are classified so the viewer knows what to do", () => {
  assert.equal(describePlaybackError({ response: { status: 503, data: { code: "OPENMAIC_FUSION_UNAVAILABLE" } } }).kind, "unavailable");
  assert.equal(describePlaybackError({ response: { status: 404, data: { code: "OPENMAIC_WORKSPACE_NOT_FOUND" } } }).kind, "notFound");
  assert.equal(describePlaybackError({ response: { status: 400, data: { code: "OPENMAIC_INVALID_REQUEST" } } }).kind, "invalid");
...
test("the playback request carries the resume scene and omits it when absent", async () => {
  ...
  await apiModule.getOpenMAICStagePlayback("c1", "ws_1", "stg_1", { sceneId: "scn_2" });
  assert.equal(mock.requests.at(-1).params.scene_id, "scn_2");

  await apiModule.getOpenMAICStagePlayback("c1", "ws_1", "stg_1");
...
test("the player is reachable from the scene list (not dead code)", () => {
  ...
  assert.match(playerSource, /getOpenMAICStagePlayback/);
  assert.match(playerSource, /getOpenMAICStageScene/);
});
