 ...
 * `classroom: false`（切到工具时），可是 `MagicClassWorkbenchPage.jsx` 里课堂
...
function installHandlers() {
  ...
  mock.onGet("/magicclass/fusion/providers", {
    state: "disabled", providers: {},
  });
  mock.onGet("/magicclass/providers", { state: "disabled", providers: {} });
...
test("closeTab keeps its side effect out of the state updater", async () => {
  const source = (await readFile(
    toPath(new URL("../src/pages/magicclassWorkbenchPage.jsx", import.meta.url)),
    "utf8",
  )).replaceAll("\r\n", "\n");
