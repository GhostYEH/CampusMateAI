...
def metrics(page):
    return page.evaluate("""() => ({
      ...
      composer: document.querySelector('.magicclass-reference-composer')?.getBoundingClientRect().toJSON() || null,
      brand: document.querySelector('.magicclass-reference__brand')?.getBoundingClientRect().toJSON() || null,
      recent: document.querySelector('.magicclass-reference-recent')?.getBoundingClientRect().toJSON() || null,
      ancestors: [...(document.querySelector('.magicclass-reference-composer')?.parentElement?.parentElement?.parentElement ? [document.querySelector('.magicclass-reference-composer').parentElement, document.querySelector('.magicclass-reference-composer').parentElement.parentElement, document.querySelector('.magicclass-reference-composer').parentElement.parentElement.parentElement] : [])].map(el => ({tag: el.tagName, className: el.className, rect: el.getBoundingClientRect().toJSON(), maxWidth: getComputedStyle(el).maxWidth})),
