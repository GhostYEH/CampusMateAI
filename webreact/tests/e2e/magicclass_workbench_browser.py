"""magicclass 学习工作台的**真实浏览器**验收：前端视觉对齐 + 响应式互斥单面板。

与 `magicclass_courses_browser.py` 的分工：那个脚本走的是「/courses → 生成预览 →
...
不做任何 route mock：浏览器打的是 Vite → FastAPI → magicclass-service 的真实链路。
...
# run_magicclass_workbench_e2e.py 会把它指到本次运行的临时目录里，随 finally 一起删。
...
WORKBENCH_GEOMETRY = """() => {
    ...
    const root = document.querySelector('[data-testid="magicclass-workbench"]');
...
def enter_classroom_from_my_courses(page, recorder: Recorder, report: list[str]) -> str:
    ...
    enter = page.locator(".magicclass-classroom-entry__action")
    ...
    assert "/magicclass-preview" not in href, "「进入课堂」不得指向生成预览"
    ...
    assert "/magicclass-preview" not in page.url, f"「进入课堂」不应走生成预览：{page.url}"
    ...
    expect(page.locator('[data-testid="magicclass-workbench"]')).to_be_visible(timeout=30000)
    ...
    workbenches = page.evaluate(
        """async () => {
            ...
            for (const course of (courses.items || [])) {
                const list = await api.listMagicClassWorkspaces(course.id, { limit: 5 });
                for (const ws of (list.items || [])) {
                    const stages = await api.listMagicClassStages(course.id, ws.id, { limit: 5 });
                    out.push({
...
def check_editor_canvas(page, recorder: Recorder, report: list[str]) -> None:
    ...
    editor = page.locator('.magicclass-stage-editor')
    ...
    for width, height in ((1440, 900), (320, 720)):
        ...
        if width == 320:
            canvas = editor.locator('[data-testid="magicclass-editor-canvas"]').evaluate('''node => {
                const box = node.getBoundingClientRect();
    ...
    # 先等待画布确认选中的真实元素。这个可观测状态由 pointerdown 同步写入，
    # 可避免在 React 尚未提交 pointerup 的命令缓冲时抢先读取保存按钮。
    expect(editor.locator('[data-testid="magicclass-selected-element"]')).to_have_attribute(
        'data-selected-element-id', movable_id, timeout=5000,
    )
    ...
    expect(page.locator('.magicclass-stage-editor')).to_be_visible(timeout=30000)
    expect(page.locator('.magicclass-stage-editor .base-element-text').first).to_be_visible(timeout=30000)
    refreshed = page.locator(
        f'.magicclass-stage-editor [data-maic-element-id="{movable_id}"] [class*="base-element-"]'
    ).first.bounding_box()
...
def check_mid_route_survives(page, recorder: Recorder, report: list[str]) -> None:
    """回归：CampusMate 全局壳会影响工作台的只有 magicclass-shell 那几条规则。"""
    page.set_viewport_size({"width": 1440, "height": 900})
    page.goto(f"{BASE}/home", wait_until="domcontentloaded")
    page.wait_for_timeout(600)
    assert page.locator(".app-layout.magicclass-shell").count() == 0, \
        "非 magic class 路由不应带上 magicclass-shell 类（那会误伤其它页面）"
    _step(report, "回归 /home：非 magicclass 路由不受 magicclass-shell 影响")
...
def check_503(page, recorder: Recorder, report: list[str], service_control) -> None:
    ...
    page.set_viewport_size({"width": 1440, "height": 900})
    page.goto(f"{BASE}/courses", wait_until="domcontentloaded")
    # 课程首页的直接入口与正常直达课堂路径共用同一契约；不能在掉线回归中留用
    # 已移除的旧 course-rail 选择器，否则测试根本没有触发真实错误恢复路径。
    enter = page.locator(".magicclass-classroom-entry__action").first
    expect(enter).to_be_visible(timeout=25000)

    _step(report, "已确认服务在线，停掉 magicclass-service 后点「进入课堂」")
    ...
    failed = page.locator(".magicclass-entry__error").first
    ...
    _step(report, "已恢复 magicclass-service，点击「重试生成」")
    recorder.responses.clear()
    if retry.count():
        retry.click()
    else:
...
def run_checks(service_control) -> dict:
    ...
    with sync_playwright() as playwright:
        ...
        try:
            ...
            print("步骤 9：回归非 magic class 路由不受全局壳影响")
