"""magicclass 学习工作台浏览器验收入口：拉起隔离的三服务，跑真实浏览器检查，最后回收。

用法（从仓库根目录）：

    backend/.venv/Scripts/python.exe webreact/tests/e2e/run_magicclass_workbench_e2e.py

和 `run_magicclass_courses_e2e.py` 同构（同样的隔离措施、同样的端口探测、同样在任何
结果下回收进程与端口），只是把浏览器检查换成 `magicclass_workbench_browser.py` 的
...
from run_magicclass_courses_e2e import (  # noqa: E402
    BACKEND,
...
def main() -> int:
    ...
    tmp_root = Path(tempfile.mkdtemp(prefix="campusmate-magicclass-workbench-e2e-"))
    ...
    try:
        ...
        import magicclass_workbench_browser  # noqa: E402  （必须在 WEB_BASE_URL 之后导入）

        result = magicclass_workbench_browser.run_checks(service)
        print("\n===== 工作台浏览器验收报告 =====")
        for line in result["report"]:
            print(f"  · {line}")
        if result["shots"]:
            print("  · 截图：" + "、".join(Path(p).name for p in result["shots"]))
        print("\nALL MAGICCLASS WORKBENCH E2E CHECKS PASSED")
        ...
        if exit_code == 0:
            ...
            print(f"E2E 失败临时目录已保留用于诊断：{tmp_root}", file=sys.stderr)
