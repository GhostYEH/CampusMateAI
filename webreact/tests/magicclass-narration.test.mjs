...
import { canGenerateNarration, describeNarrationFailure, narrationLabel } from "../src/features/magicclass/narrationModel.js";
...
test("the narration hook keeps a script-bearing scene actionable before its first job", () => {
  const hook = read("src/features/magicclass/useSceneNarration.js");
...
test("narration is fetched per scene id, never from a shared or last-used slot", () => {
  ...
  // 读取与生成都走同一个按场景的端点。
  assert.match(hook, /getMagicClassSceneNarration\(courseId, workspaceId, stageId, sceneId\)/);
  assert.match(hook, /synthesizeMagicClassSceneNarration\(courseId, workspaceId, stageId, sceneId/);
...
test("the classroom stage binds narration to the current scene, not to a fixed one", () => {
  const source = read("src/components/magicclass/magicclassClassroomStage.jsx");
  assert.match(
    source,
...
test("the browser never sends narration text, only scene identifiers", () => {
  const api = read("src/data/api.js");
  const start = api.indexOf("export async function synthesizeMagicClassSceneNarration");
