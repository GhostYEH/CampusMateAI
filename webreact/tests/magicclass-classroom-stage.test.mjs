/**
 * 学习态课堂（移植自参考项目 magic class）的 Web 侧行为契约。
 *
...
import { sandboxPolicyFor } from "../src/features/magicclass/playerModel.js";
...
test("the classroom reads the server's render decision, not its own guess", () => {
  assert.match(stageSource, /api\.getMagicClassStagePlayback\(/, "场景列表必须来自播放计划");
  ...
  assert.match(stageSource, /api\.getMagicClassStage\(/, "正文必须来自授权的舞台文档端点");
  assert.match(stageSource, /indexScenesById\(stageDocument\)/, "舞台文档必须按场景 id 建索引");
  assert.doesNotMatch(stageSource, /getMagicClassStageScene\(/, "不得退回逐场景拉取正文");
...
test("the roundtable is driven by the real discussion pipeline", () => {
  ...
  assert.match(stageSource, /api\.runMagicClassDiscussion\(/, "必须走真实讨论接口");
  assert.match(stageSource, /api\.getMagicClassJob\(/, "必须轮询任务终态");
  ...
  assert.match(stageSource, /api\.getMagicClassArtifact\(/, "必须取回讨论产物");
