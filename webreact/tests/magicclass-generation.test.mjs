...
test("generation sends the selected mode, prompt, roster and stable idempotency key", async () => {
  ...
  await api.generateMagicClassStage("c1", "ws1", { mode: "quiz", prompt: "复习极限", idempotencyKey: "generate-1" });
...
test("whiteboard writes carry both revision and idempotency headers", async () => {
  ...
  await api.addMagicClassWhiteboard("c1", "ws1", "stg1", { board: { id: "b1", title: "草稿", elements: [] }, revision: 1, idempotencyKey: "board-1" });
  const request = mock.lastRequest();
...
test("speech and discussion requests stay course-bound and idempotent", async () => {
  ...
  await api.synthesizeMagicClassTts("c1", { text: "讲解极限", voice: "苏打", idempotencyKey: "tts-1" });
  ...
  await api.runMagicClassDiscussion("c1", { prompt: "讨论极限的定义", idempotencyKey: "discussion-1" });
...
test("video export enqueues a render job through the authenticated gateway", async () => {
  ...
  await api.enqueueMagicClassStageVideo("c1", "ws1", "stg1", { idempotencyKey: "video-1" });
  const request = mock.lastRequest();
...
test("workspace provider tools are capability-gated and recover their artifact", async () => {
  const source = await import("node:fs").then(({ readFileSync }) => readFileSync(new URL("../src/components/magicclass/ProviderToolsPanel.jsx", import.meta.url), "utf8"));
  assert.match(source, /canTts/);
  assert.match(source, /canDiscussion/);
  assert.match(source, /getMagicClassArtifact/);
  assert.match(source, /synthesizeMagicClassTts/);
  assert.match(source, /runMagicClassDiscussion/);
});

test("provider status uses the CampusMate route and does not need an exposed key", async () => {
  const mock = createMockClient(api.default);
  mock.onGet("/magicclass/fusion/providers", { state: "ready", providers: { llm: false, tts: false } });
  const status = await api.getMagicClassProviderStatus();
  assert.equal(status.providers.tts, false);
  assert.equal(mock.lastRequest().url, "/magicclass/fusion/providers");
});
...
test("courses home fetches provider status independently from course content", async () => {
  ...
  assert.match(source, /api\.getMagicClassProviderStatus\(\)/);
