...
The tree under `openmaic-app/` is the commit above with exactly **one** additional,
replayable transformation: `scripts/magicclass-brand.mjs`, which renames the user-visible
brand to `magic'class` and the npm identity `openmaic` / `@openmaic/*` to
...
A pass means `openmaic-app/` = this upstream commit + the rules in that one file. Anything
beyond those rules shows up as a mismatch. Upstream MIT license and copyright stay in place
(`openmaic-app/LICENSE`); `render-service/` keeps its two published `@openmaic/*`
