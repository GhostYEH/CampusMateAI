/**
 * @magicclass/render-service — HTTP entrypoint.
 *
...
export function createApp(deps: AppDeps): Hono {
  ...
  app.post('/render', async (c) => {
    ...
    const identity = c.req.header('x-magicclass-client')?.trim() || 'anonymous';
...
async function main(): Promise<void> {
  ...
  // standalone path nothing creates /tmp/magicclass-renders, so without this every
