  ...
  const candidate = builder({
    stdin: {
      ...
      contents: `
        ...
        window.__MAGICCLASS_PREVIEW_MOUNTED__ = true;
      `,
    },
    bundle: true,
    format: 'iife',
  ...
  try {
    await Promise.race([
      (async () => {
        ...
        await page.waitForFunction(() => '__MAGICCLASS_PREVIEW_MOUNTED__' in window);
      })(),
      pageError,
    ]);
  } finally {
              ...
              await page.evaluate(
                (slide, viewport) => {
                  Object.assign(window, {
                    __MAGICCLASS_PREVIEW_PROPS__: { slide, viewport },
                  });
                },
