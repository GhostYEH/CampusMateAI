...
function firstHeadMetaMarker(html: string): string | undefined {
  ...
  return first.attrs.find((attribute) => attribute.name === 'data-magicclass-untrusted-csp')
    ? 'meta'
    : undefined;
}
...
describe('project HTML hardening', () => {
  ...
  async function makeProject(): Promise<string> {
    const dir = await mkdtemp(join(tmpdir(), 'magicclass-csp-'));
    scratch.push(dir);
    return dir;
  }
  ...
  it('is a no-op for a missing directory and for already-hardened HTML', async () => {
    await expect(
      hardenProjectDirectory(join(tmpdir(), 'magicclass-missing-dir')),
