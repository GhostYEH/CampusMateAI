...
async function runRenderPayload(harden) {
  const dir = await mkdtemp(join(tmpdir(), 'magicclass-csp-render-'));
  try {
...
async function runRenderPrefix(target) {
  const dir = await mkdtemp(join(tmpdir(), 'magicclass-csp-prefix-'));
  try {
...
async function runRenderSvgProject({ sanitize }) {
  const dir = await mkdtemp(join(tmpdir(), 'magicclass-csp-svg-'));
  try {
...
async function runRenderXhtmlProject({ sanitize }) {
  const dir = await mkdtemp(join(tmpdir(), 'magicclass-csp-xhtml-'));
  try {
...
async function runRenderFormProject() {
  const dir = await mkdtemp(join(tmpdir(), 'magicclass-csp-form-'));
  try {
...
async function runRenderLegit() {
  const dir = await mkdtemp(join(tmpdir(), 'magicclass-csp-legit-'));
  try {
