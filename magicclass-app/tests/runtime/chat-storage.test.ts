...
describe('chat RuntimeStore cutover', () => {
  ...
  it.each([
    [
      ...
      `@magicclass/storage: cannot append to session "chat:stage-chat:anon%3Achat-test:session-1" with status 'completed' — records may only be appended to an active session`,
    ],
    [
      'post-race reclassification',
      `@magicclass/storage: session "chat:stage-chat:anon%3Achat-test:session-1" is no longer active; its current status is 'completed'`,
    ],
