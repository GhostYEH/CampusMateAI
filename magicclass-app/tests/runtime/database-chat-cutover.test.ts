...
describe('database runtime chat integration', () => {
  ...
  it('acquires both the stage-wide and legacy partition Web Lock names', async () => {
    ...
    expect(requested).toEqual([
      'magicclass:chat-storage:all',
      `magicclass:chat-storage:${encodeURIComponent('stage-compatible-lock')}`,
      `magicclass:chat-storage:${encodeURIComponent(`stage-compatible-lock\0${learnerKey}`)}`,
    ]);
  });
