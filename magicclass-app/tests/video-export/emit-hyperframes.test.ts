...
describe('emitHyperframes', () => {
  ...
  it('emits the self-contained project file set', () => {
    ...
    expect(paths).toEqual(
      [
        ...
        'README.md',
        'index.html',
        'magicclass-video-manifest.json',
        'subtitles.srt',
        'subtitles.vtt',
      ].sort(),
    );
  ...
  it('builds one composition driven by one paused GSAP timeline', () => {
    expect(html).toContain('data-composition-id="magicclass"');
    ...
    expect(html).toContain('window.__timelines["magicclass"] = tl;');
  });
...
describe('emitHyperframes frozen interactive HTML', () => {
  ...
  it('waits for every iframe to freeze or fall back before registering the timeline', () => {
    ...
    const registration = html.indexOf('window.__timelines["magicclass"] = tl;', ready);
    ...
    expect(html).toContain('window.__magicclassVideoDiagnostics');
    expect(html).toContain('data-magicclass-runtime-diagnostics');
    expect(html).toContain('window.__magicclassVideoManifest.runtimeDiagnostics');
  });

  it('keeps runtime diagnostics in the live report instead of an immutable sidecar', () => {
    expect(html).toContain('data-magicclass-runtime-diagnostics');
    expect(html).toContain('window.__magicclassVideoManifest.runtimeDiagnostics');
    expect(html).not.toContain('magicclass-runtime-diagnostics.json');
  });
