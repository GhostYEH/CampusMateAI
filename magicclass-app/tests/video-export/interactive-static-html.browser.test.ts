    ...
    try {
      ...
      await page.waitForFunction(() => {
        ...
        const timelines = (window as typeof window & { __timelines?: Record<string, unknown> })
          .__timelines;
        return (
          host?.getAttribute('data-interactive-static-state') === 'frozen' && !!timelines?.magicclass
        );
      });
      ...
      const before = await child.evaluate(() => ({
        ...
        state: document.documentElement.getAttribute('data-magicclass-static-state'),
      ...
      const result = await page.evaluate(() => {
        ...
        const diagnostics = (
          window as typeof window & {
            __magicclassVideoDiagnostics?: Array<{ code: string; message: string }>;
          }
        ).__magicclassVideoDiagnostics;
        const manifest = (
          window as typeof window & {
            __magicclassVideoManifest?: { runtimeDiagnostics?: Array<{ code: string }> };
          }
        ).__magicclassVideoManifest;
        const runtimeReport = document.querySelector<HTMLElement>(
          '[data-magicclass-runtime-diagnostics]',
        );
        return {
