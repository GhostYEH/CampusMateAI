...
beforeAll(async () => {
  tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), 'magicclass-classrooms-'));
  process.env.MAGICCLASS_CLASSROOMS_DIR = tmpDir;
  vi.resetModules();
  storage = await import('@/lib/server/classroom-storage');
});

afterAll(async () => {
  delete process.env.MAGICCLASS_CLASSROOMS_DIR;
