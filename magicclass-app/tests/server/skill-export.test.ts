...
describe('skill export zips', () => {
  it('packages the shipped magicclass skill verbatim under magicclass/', async () => {
    ...
    const entries = Object.values(loaded.files)
      ...
      .map((file) => file.name);
    expect(new Set(entries)).toEqual(new Set(onDisk.map((path) => `magicclass/${path}`)));
    for (const path of onDisk) {
      expect(await loaded.file(`magicclass/${path}`)!.async('string')).toBe(
  ...
  it('rejects traversal ids', () => {
    ...
    for (const value of ['../magicclass', 'a/b', '..', '']) expect(isSafeSkillId(value)).toBe(false);
  });
