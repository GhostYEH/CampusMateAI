...
describe('agent runtime configuration predicate', () => {
  const ENV_KEYS = ['MAGICCLASS_AGENT_RUNTIME_ENABLED', 'DATABASE_URL'] as const;
  ...
  it.each([
    ...
    (_case, runtimeFlag, databaseUrl, enabled, configured) => {
      if (runtimeFlag !== undefined) process.env.MAGICCLASS_AGENT_RUNTIME_ENABLED = runtimeFlag;
...
describe.each([
  ['MAGICCLASS_ENABLE_PI_NATIVE_CHILD_RUNTIME', isPiNativeChildRuntimeEnabled],
  ['MAGICCLASS_ENABLE_PI_NATIVE_CHILD_SPOTLIGHT', isPiNativeChildSpotlightEnabled],
])('%s', (flag, readFlag) => {
  let original: string | undefined;
...
describe('isVocationalTaskEngineEnabled', () => {
  const flag = 'MAGICCLASS_ENABLE_VOCATIONAL';
  let original: string | undefined;
