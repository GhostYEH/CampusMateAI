...
const editorReadmePath = resolve(process.cwd(), 'packages/@magicclass/editor/README.md');
...
describe('renderer editor app boundary', () => {
  ...
  it('documents a complete third-party controlled-state and style setup', () => {
    ...
    expect(readme).toContain("import '@magicclass/renderer/fonts.css';");
  ...
  it('smoke-tests every published editor entry from packed artifacts', () => {
    ...
    expect(smoke).toContain("from '@magicclass/editor';");
    expect(smoke).toContain("from '@magicclass/editor/core';");
    expect(smoke).toContain("from '@magicclass/editor/react';");
    expect(smoke).toContain("from '@magicclass/editor/ui';");
  });
});
