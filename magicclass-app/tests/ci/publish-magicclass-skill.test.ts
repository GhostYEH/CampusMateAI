...
const publishScript = resolve(repositoryRoot, '.github/scripts/publish-magicclass-skill.sh');
...
describe('publish-magicclass-skill shell contract', () => {
  ...
  it('rejects unsupported arguments', () => {
    ...
    expectFailure(usage.result, 'Usage: publish-magicclass-skill.sh [--dry-run]');
  ...
  it.each([
    ...
    expectFailure(result, 'Usage: publish-magicclass-skill.sh [--dry-run]');
