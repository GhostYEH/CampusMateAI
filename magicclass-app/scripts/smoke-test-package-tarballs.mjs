...
import { INTERNAL_DEPENDENTS, MAGICCLASS_PACKAGES, readManifest } from './magicclass-packages.mjs';
...
const temporaryDirectory = mkdtempSync(join(tmpdir(), 'magicclass-package-smoke-'));
 ...
 * @magicclass packages together resolve ONE copy of the dsl. An exact pin — which
  ...
  for (const name of packageNames) {
    ...
    for (const [peer, range] of Object.entries(manifest.peerDependencies ?? {})) {
      ...
      assert(
        ...
        `@magicclass packages declare different ${peer} peer ranges: ${existing} and ${range}`,
      );
...
`,
  ...
  console.log('Validated @magicclass package tarball imports passed.');
} finally {
