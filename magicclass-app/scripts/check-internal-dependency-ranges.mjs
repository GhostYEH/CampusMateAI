import {
  ...
  readManifest,
} from './magicclass-packages.mjs';

/**
 * An owned @magicclass package may be declared as a dependency of another exactly
...
const OWNED = new Set(MAGICCLASS_PACKAGES.map((name) => `@magicclass/${name}`));
...
if (listProblems.length > 0) {
  report('The shared @magicclass package list has drifted:', listProblems);
}
...
for (const name of MAGICCLASS_PACKAGES) {
  ...
  for (const field of FORBIDDEN_FIELDS) {
    for (const dependency of Object.keys(manifest[field] ?? {})) {
      if (!OWNED.has(dependency)) continue;
      failures.push(
        `@magicclass/${name} declares ${field}."${dependency}". An owned @magicclass package may ` +
...
for (const [name, expected] of Object.entries(INTERNAL_DEPENDENTS)) {
  ...
  if (observed === undefined) {
    failures.push(
      `@magicclass/${name} no longer declares any owned @magicclass dependency in \`dependencies\`; ` +
        `it was expected to depend on ${expectedDependencies.join(', ')}. If that is intended, update ` +
        'INTERNAL_DEPENDENTS in scripts/magicclass-packages.mjs — this check must not go quiet ' +
        'on its own.',
    );
    continue;
  }
...
for (const [name, observed] of seen) {
  ...
  failures.push(
    `@magicclass/${name} declares the owned dependency ${observed.join(', ')} but is absent from ` +
      'INTERNAL_DEPENDENTS in scripts/magicclass-packages.mjs, so nothing checks how it is ' +
...
for (const name of MAGICCLASS_PACKAGES) {
  for (const dependency of Object.keys(readManifest(name).devDependencies ?? {})) {
    if (!OWNED.has(dependency)) continue;
    failures.push(
      `@magicclass/${name} declares devDependencies."${dependency}". An owned @magicclass package ` +
...
if (failures.length > 0) {
  report('Internal @magicclass dependency declarations are not in the required shape:', failures);
}
