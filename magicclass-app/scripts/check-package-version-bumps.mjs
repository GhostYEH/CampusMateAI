...
import { MAGICCLASS_PACKAGES, assertPackageListIsComplete } from './magicclass-packages.mjs';
...
const ignoredPackageInputs = Object.fromEntries(
  MAGICCLASS_PACKAGES.map((name) => [name, ignoredInputOverrides[name] ?? commonIgnoredInputs]),
);
...
function runDiffMode(base) {
  ...
  failIfAny(
    failures,
    'Every @magicclass publishable package change must ship with a new package version:',
  );
...
function runReleaseMode() {
  ...
  failIfAny(failures, 'Refusing to publish @magicclass packages:');
