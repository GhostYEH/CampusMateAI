...
import { MAGICCLASS_PACKAGES, readManifest } from './magicclass-packages.mjs';

const DIGESTS_FILE = 'SHA256SUMS';
const root = resolve(dirname(fileURLToPath(import.meta.url)), '..');

function expectedArtifacts() {
  return MAGICCLASS_PACKAGES.map((shortName) => {
    const manifest = readManifest(shortName);
    return {
      shortName,
      name: `@magicclass/${shortName}`,
      version: manifest.version,
      filename: `magicclass-${shortName}-${manifest.version}.tgz`,
    };
  });
}
