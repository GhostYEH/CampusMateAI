...
/**
 * The one list of owned, publishable @magicclass packages.
 *
 ...
 */
export const MAGICCLASS_PACKAGES = ['dsl', 'generation', 'storage', 'renderer', 'editor', 'importer'];
...
export const PACKAGES_DIRECTORY = join(repositoryRoot, 'packages/@magicclass');
...
export function assertPackageListIsComplete() {
  ...
  const onDisk = readdirSync(PACKAGES_DIRECTORY, { withFileTypes: true })
    ...
    .sort();
  const listed = [...MAGICCLASS_PACKAGES].sort();

  for (const name of onDisk) {
    if (!listed.includes(name)) {
      problems.push(
        `packages/@magicclass/${name} exists but is not in MAGICCLASS_PACKAGES, so every check ` +
  ...
  for (const name of listed) {
    if (!onDisk.includes(name)) {
      problems.push(`MAGICCLASS_PACKAGES lists ${name}, but packages/@magicclass/${name} is gone.`);
    }
  }
...
function describeSetDifference(expected, observed, subject) {
  ...
  for (const name of observed) {
    if (!expected.includes(name)) {
      problems.push(`${subject} names ${name}, which is not in MAGICCLASS_PACKAGES.`);
    }
  }
  return problems;
}
 ...
 * exactly {@link MAGICCLASS_PACKAGES}.
 *
...
function crossCheckPublishWorkflow(workflow) {
  ...
  if (!pathsBlock) {
    ...
    const triggerPaths = [
      ...pathsBlock[1].matchAll(/packages\/@magicclass\/([^/"'\s]+)\/package\.json/g),
    ].map((match) => match[1]);
    problems.push(
  ...
  const filtered = new Set(
    [...source.matchAll(/--filter\s+"?@magicclass\/([^"\s]+)"?/g)].map((match) => match[1]),
  );
  for (const name of MAGICCLASS_PACKAGES) {
  ...
  for (const name of filtered) {
    if (!MAGICCLASS_PACKAGES.includes(name)) {
      problems.push(
        `${PUBLISH_WORKFLOW} filters @magicclass/${name}, which is not in MAGICCLASS_PACKAGES.`,
      );
    }
  }
