...
magic class generation does not automatically reuse the OpenClaw agent's current model or API key.

magic class server APIs resolve their own model and provider keys from magic class server-side config.

This skill does not rely on runtime overrides for model, provider, API key, base URL, or provider type.

If the user wants to change any of those, they must edit magic class server-side config files.
...
- magic class has **no hardcoded model fallback**. If `DEFAULT_MODEL` is unset (and no client model is sent), generation **fails with an error** rather than silently picking a default. So the user must always set `DEFAULT_MODEL` explicitly to match whichever provider key they configured.
...
Do not recommend bare model IDs such as `gemini-2.5-flash` by themselves, because magic class will otherwise parse them as OpenAI models.
...
- "I recommend configuring magic class through `.env.local` first. Please edit that file locally and tell me when you're done."
