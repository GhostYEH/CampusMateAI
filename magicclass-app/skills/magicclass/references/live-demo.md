...
The magic class Live Demo (open.maic.chat) is the cloud edition — the version officially deployed and hosted by the magic class team, so no local setup is required. Use this when the user has an access code from open.maic.chat and wants to skip local setup.

## Access Code Setup

1. Read `accessCode` from skill config (`~/.openclaw/openclaw.json` → `skills.entries.magicclass.config.accessCode`).
...
3. If not found, tell the user how to get an access code and where to put it:
   ...
   - Add it to the config file: edit `~/.openclaw/openclaw.json` and set `skills.entries.magicclass.config.accessCode` to your access code.
