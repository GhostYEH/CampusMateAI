# Extend The magic class Product (Cookbook)

## Scope

You are working **inside a fork of the magic class product** (the Next.js app at the repo root), customizing it in place. If instead you want to consume `@magicclass/*` in a separate app, use [extend-sdk.md](extend-sdk.md) instead.
...
**Gotcha:** magic class has **no hardcoded model fallback**. If `DEFAULT_MODEL` is unset, generation fails rather than picking a default — always set it.
...
- The backends themselves come from `@magicclass/storage` subpaths (`@magicclass/storage/document/pg`, `@magicclass/storage/runtime/pg`, `@magicclass/storage/asset/pg-bytes`, `@magicclass/storage/asset/s3-bytes`) — see the storage table in [extend-sdk.md](extend-sdk.md).

**Gotcha:** bootstrap is client-side and gated on `NEXT_PUBLIC_PERSISTENCE`; restart the dev server after changing `.env.local`. S3 additionally needs `@aws-sdk/client-s3` (optional peer of `@magicclass/storage`) installed in the app, and PG needs a reachable Postgres + the package's schema-ensure step.
...
- Title + fonts: `app/layout.tsx` (the title metadata; fonts include `@magicclass/renderer/fonts.css`).
...
Rebuild sequence when you touch `packages/@magicclass/*` source: consumers resolve to the built `dist/`, not `src/`, so rebuild the changed package (dependency order: `dsl → generation → storage → importer → renderer → editor`). `pnpm install`'s postinstall already does this in order; for a single package use its `pnpm run build`.
