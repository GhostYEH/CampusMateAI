...
- magic class is healthy at the selected `url`
- Provider keys are configured

> **Live Demo mode**: If using the magic class Live Demo (open.maic.chat), all
...
- Do not try to recover from auth, provider, model, or base URL errors by changing request parameters. Tell the user to fix magic class server-side config and retry only after they confirm.
