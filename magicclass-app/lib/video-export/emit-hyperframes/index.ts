...
export interface EmitHyperframesOptions {
  ...
  height?: number;
  /** Composition id used for the root `data-composition-id` and the timeline key. Default `magicclass`. */
  compositionId?: string;
  /** Relative path the emitted HTML loads GSAP from. Default `assets/vendor/gsap.min.js`. */
  gsapVendorPath?: string;
  /** Manifest filename. Default `magicclass-video-manifest.json`. */
  manifestPath?: string;
...
const DEFAULT_MANIFEST = 'magicclass-video-manifest.json';
...
function interactiveStaticBridgeScript(labels: InteractiveFallbackLabels): string {
  ...
  return `
function initializeMagicClassInteractiveStaticFrames() {
  var hosts = Array.from(document.querySelectorAll('[data-interactive-static-host]'));
  window.__magicclassVideoDiagnostics = window.__magicclassVideoDiagnostics || [];
  window.__magicclassVideoManifest = window.__magicclassVideoManifest || { runtimeDiagnostics: [] };
  var labels = ${localized};
  var diagnosticCodes = new Set(${diagnosticCodes});
  var runtimeReport = document.querySelector('[data-magicclass-runtime-diagnostics]');
  function record(sceneId, code, message) {
    ...
    window.__magicclassVideoDiagnostics.push(diagnostic);
    window.__magicclassVideoManifest.runtimeDiagnostics = window.__magicclassVideoDiagnostics.slice();
    if (runtimeReport) runtimeReport.textContent = JSON.stringify(window.__magicclassVideoDiagnostics);
...
- Runtime interactive diagnostics are exposed live through \`window.__magicclassVideoManifest.runtimeDiagnostics\` and the machine-readable DOM report in \`index.html\`.
...
export function emitHyperframes(
  ...
  const compositionId = options.compositionId ?? 'magicclass';
...
${effectHtml.join('\n')}
${subtitles.html}
<script type="application/json" data-magicclass-runtime-diagnostics>[]</script>
</div>
...
${statements.join('\n')}
window.__magicclassVideoManifest = { runtimeDiagnostics: [], manifestPath: ${JSON.stringify(manifestPath)} };
window.__timelines = window.__timelines || {};
${
  hasInteractiveHtml
    ? `${interactiveStaticBridgeScript(labels.interactive)}
window.__magicclassInteractiveReady = initializeMagicClassInteractiveStaticFrames();
window.__magicclassInteractiveReady.then(function () {
