...
// VideoManifest / SlideContent / QuizContent / …) now lives in `@magicclass/dsl` and
...
export { isSlideContent, isQuizContent } from '@magicclass/dsl';

// `@magicclass/dsl` inlines the question-type union on `QuizQuestion.type` rather than
// exporting a named alias; derive it here so editor quiz code can keep importing
// `QuizQuestionType` from `@/lib/types/stage`.
export type QuizQuestionType = import('@magicclass/dsl').QuizQuestion['type'];
...
export type { SceneContent as SceneContentBase } from '@magicclass/dsl';
...
export type { Scene as SceneShape } from '@magicclass/dsl';
...
export type AppScene = DslScene<Action, SceneContent> & {
   ...
   * annotation only — not part of the `@magicclass/dsl` Scene contract. Absent on
