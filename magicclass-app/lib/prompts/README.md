...
app-only outline modes. Generation templates live in `@magicclass/generation`.
...
- Package-owned generation templates follow the same placeholder conventions;
  edit them under `packages/@magicclass/generation/templates/`.
...
- Tests in `tests/prompts/templates.test.ts` and
  `packages/@magicclass/generation/test/` assert that fully rendered app and
