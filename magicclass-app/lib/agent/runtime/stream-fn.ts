 ...
 *
 * Bridges the pi agent loop's LLM call to magic class's existing AI-SDK-based
 ...
 * call through magic class's resolved Vercel `LanguageModel`, then map the AI SDK
...
/** Build a pi `StreamFn` that calls magic class's connector instead of pi-ai providers. */
