...
The **contract keystone** of the MAIC SDK family. `@magicclass/dsl` is *pure spec* — the
...
package can depend on `@magicclass/dsl` without pulling in junk.

## Dependency arrows (acyclic)

```
@magicclass/dsl       ->  (nothing)
@magicclass/renderer  ->  @magicclass/dsl
@magicclass/importer  ->  @magicclass/dsl
@magicclass/exporter  ->  @magicclass/dsl     (reserved, future)
```

`@magicclass/dsl` is the only package everything else depends on, and it depends on
nothing.
...
```ts
import type { Slide, PPTElement, Action } from '@magicclass/dsl';
import { isTextElement, DSL_VERSION, SYNC_ACTIONS } from '@magicclass/dsl';
```
...
1. **JSON Schema artifacts (cross-language mirror)** — `Stage`, the default
   ...
   ```ts
   import stageSchema from '@magicclass/dsl/schema/stage.schema.json' with { type: 'json' };
   import sceneSchema from '@magicclass/dsl/schema/scene.schema.json' with { type: 'json' };
   import actionSchema from '@magicclass/dsl/schema/action.schema.json' with { type: 'json' };
...
2. **Pure validators (in-process boundary)** — `validate*` are hand-written,
   ...
   ```ts
   import { validateStage, validateScene, validateAction } from '@magicclass/dsl';
...
3. **Pure normalizers (repair boundary)** — where `validate*` *reports* on a
   ...
   ```ts
   import { normalizeElement, normalizeScene, ELEMENT_DEFAULTS } from '@magicclass/dsl';
...
```ts
import { DSL_VERSION, migrate, dslVersionOf, needsMigration } from '@magicclass/dsl';
...
outside the document, per learner, through a `RuntimeStore` (`@magicclass/storage`).
...
Both consumers are now wired to `@magicclass/dsl` and no longer vendor their own copy
of the slide types:

- **`@magicclass/importer`**: imports all slide types from `@magicclass/dsl`; vendored
  `magicclass/types/slides.ts` deleted. The importer emits complete DSL `Slide`
  objects directly (the old partial "draft slide" + post-fill step is gone).
- **`@magicclass/renderer`**: imports all slide types from `@magicclass/dsl`; vendored
  `types/slides.ts` deleted. `@magicclass/dsl` is a regular dependency, kept external
  in the rollup build so consumers share one copy. The public
  `@magicclass/renderer/types` surface now re-exports the DSL types.

### Roadmap

- [x] Wire `@magicclass/importer` to import types from `@magicclass/dsl` (vendored copy deleted).
- [x] Wire `@magicclass/renderer` to import types from `@magicclass/dsl` (vendored copy deleted).
...
- [ ] Reserve `@magicclass/exporter` as the 4th family member.
...
```ts
import type { Action, InteractiveContent, PBLContent, Scene, SceneContent, WidgetConfigBase } from '@magicclass/dsl';
...
```bash
pnpm --filter @magicclass/dsl build         # -> dist/ (index.js, index.d.ts, …) + dist/schema/*.json
pnpm --filter @magicclass/dsl build:schema  # regenerate only dist/schema/*.json
pnpm --filter @magicclass/dsl typecheck
pnpm --filter @magicclass/dsl test
```

## License

MIT, matching the rest of the family (`@magicclass/dsl`, `@magicclass/importer`,
`@magicclass/renderer`) and the magic class root, so the license policy is uniform
across the SDK.
