...
persisting app state, depending only on [`@magicclass/dsl`](../dsl).
...
```
@magicclass/storage -> @magicclass/dsl
```
...
| `StorageProvider` (from `@magicclass/dsl`) | the asset seam: `put(blob) → ref`, `resolve(ref) → url`, `remove(ref)` | `BrowserAssetStore` over IndexedDB (`assets` registry + `blobs`) + object URLs |
...
- [x] `StorageProvider` (in `@magicclass/dsl`) + browser asset registry: allocated
