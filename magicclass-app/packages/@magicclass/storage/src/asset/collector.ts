...
export function assertSignedUrlTtlWithinGrace(ttlSeconds: number, graceMs: number): void {
  if (
    ...
    throw new Error(
      '@magicclass/storage: the signed URL lifetime must stay far below the collection grace period',
    );
  }
}
...
export class AssetCollectionFailure extends Error {
  ...
  constructor(stageId?: string, cause?: unknown) {
    super(
      '@magicclass/storage: asset collection failed',
...
export class AssetReferenceTrackingNotEnabledError extends Error {
  constructor() {
    super(
      '@magicclass/storage: the asset collector is configured with documentReferences, but no ' +
...
function collectorConfigurationFailure(): Error {
  return new Error(
    '@magicclass/storage: the asset byte store cannot coordinate collection with the registry. A ' +
...
export class AssetCollector {
  ...
  constructor(
    ...
    if (typeof options?.withTransaction !== 'function') {
      throw new Error('@magicclass/storage: withTransaction is required for AssetCollector');
    }
    const graceMs = options.graceMs ?? DEFAULT_ASSET_COLLECTION_GRACE_MS;
    if (!Number.isSafeInteger(graceMs) || graceMs < 0) {
      throw new Error('@magicclass/storage: graceMs must be a non-negative safe integer');
    }
    const batchSize = options.batchSize ?? DEFAULT_ASSET_COLLECTION_BATCH_SIZE;
    if (!Number.isSafeInteger(batchSize) || batchSize < 1) {
      throw new Error('@magicclass/storage: batchSize must be a positive safe integer');
    }
    ...
    if (!Number.isSafeInteger(referenceBackfillBatchSize) || referenceBackfillBatchSize < 1) {
      throw new Error(
        '@magicclass/storage: referenceBackfillBatchSize must be a positive safe integer',
      );
    }
