...
export class DocumentAssetReferencesDisabledError extends Error {
  constructor(operation: string) {
    super(
      `@magicclass/storage: ${operation} requires a document store constructed with ` +
...
-- current_setting('magicclass.suppress_stage_notify', true) before pg_notify.
-- Batch/backfill writers MUST run SET LOCAL magicclass.suppress_stage_notify =
...
CREATE OR REPLACE FUNCTION magicclass_bump_scene_revision() RETURNS trigger AS $$
DECLARE
  v_stage_id text;
  v_scene_id text;
BEGIN
  ...
  IF coalesce(current_setting('magicclass.suppress_stage_notify', true), '') <> 'on' THEN
    PERFORM pg_notify('magicclass_agent_event_wakeup', json_build_object('kind', 'stage', 'stageId', v_stage_id)::text);
  END IF;
  RETURN NULL;
END;
$$ LANGUAGE plpgsql;

CREATE OR REPLACE FUNCTION magicclass_bump_stage_revision() RETURNS trigger AS $$
DECLARE
  v_stage_id text;
BEGIN
  IF TG_OP = 'DELETE' THEN
...
DROP TRIGGER IF EXISTS magicclass_scene_revision_trigger ON document_scenes;

CREATE TRIGGER magicclass_scene_revision_trigger
AFTER INSERT OR UPDATE OR DELETE ON document_scenes
FOR EACH ROW EXECUTE FUNCTION magicclass_bump_scene_revision();

DROP TRIGGER IF EXISTS magicclass_stage_revision_trigger ON document_stages;

CREATE TRIGGER magicclass_stage_revision_trigger
AFTER INSERT OR UPDATE OR DELETE ON document_stages
FOR EACH ROW EXECUTE FUNCTION magicclass_bump_stage_revision();
`;
...
function assertValid(
  ...
  throw new Error(`@magicclass/storage: invalid ${label}: ${detail}`);
}
...
function assertStorableScene(scene: SceneLike, stageId: string): void {
  ...
  if (typeof candidate.id !== 'string') {
    throw new Error(
      `@magicclass/storage: scene id must be a string, got ${JSON.stringify(candidate.id)}`,
    );
  }
  if (candidate.stageId !== stageId) {
    throw new Error(
      `@magicclass/storage: scene ${JSON.stringify(candidate.id)} has stageId ` +
  ...
  if (typeof candidate.order !== 'number' || !Number.isFinite(candidate.order)) {
    throw new Error(
      `@magicclass/storage: scene ${JSON.stringify(candidate.id)} order must be a finite number, ` +
  ...
  implements DocumentStore<TScene, TStage>, DocumentFolderStore, StageFreshnessManifestStore
{
  ...
  constructor(queryable: Queryable, options: PgDocumentStoreOptions) {
    if (typeof options?.withTransaction !== 'function') {
      throw new Error(
        '@magicclass/storage: withTransaction is required and must pin a fresh connection and ' +
    ...
    if (options.ownerId !== undefined && !isPgQueryableKey(options.ownerId)) {
      throw new Error('@magicclass/storage: PgDocumentStore ownerId must be lossless JSON text');
    }
  ...
  private requireOwner(operation: string): string {
    if (this.ownerId === null) {
      throw new Error(`@magicclass/storage: ${operation} requires an owner-bound document store`);
    }
    return this.ownerId;
  }
  ...
  private async loadStage(
    ...
    if (!isPlainObject(decoded)) {
      throw new Error(
        `@magicclass/storage: corrupt stored row for document ${JSON.stringify(stageId)}: ` +
  ...
  private currentVersionError(
    ...
    return new DocumentVersionError(
      ...
      `@magicclass/storage: cannot ${operation} document ${JSON.stringify(stageId)} at DSL ` +
  ...
  private validateForSave(
    ...
    for (const scene of doc.scenes) {
      ...
      if (seen.has(scene.id)) {
        throw new Error(
          `@magicclass/storage: duplicate scene id ${JSON.stringify(scene.id)} in document ` +
  ...
  private async persistStage(queryable: Queryable, stageRow: StageRow<TStage>): Promise<void> {
    ...
    if (result.rows.length === 0) {
      throw new DocumentNotFoundError(
        stageRow.id,
        `@magicclass/storage: document ${JSON.stringify(stageRow.id)} belongs to another scope`,
      );
    }
  }
  ...
  async saveDocument(doc: MaicDocument<TScene, TStage>): Promise<void> {
    if (isFutureVersioned(doc)) {
      throw new DocumentVersionError(
        ...
        'future',
        doc.dslVersion,
        `@magicclass/storage: refusing to save document ${JSON.stringify(doc.stage.id)} — it was ` +
    ...
    await this.writeTransaction(async (queryable) => {
      ...
      if (existingStage && isFutureVersioned(existingStage)) {
        throw new DocumentVersionError(
          ...
          `@magicclass/storage: refusing to overwrite document ${JSON.stringify(stageId)} — the ` +
  ...
  async createFolder(
    ...
    if (!isPgQueryableKey(folderId) || !isPgQueryableKey(name)) {
      throw new Error('@magicclass/storage: folder id and name must be lossless JSON text');
    }
  ...
  async putStage(stageId: string, stage: TStage): Promise<void> {
    ...
    if (stage.id !== stageId) {
      throw new Error(
        `@magicclass/storage: stage ${JSON.stringify(stage.id)} does not belong to document ` +
          JSON.stringify(stageId),
      );
    }
    ...
    await this.writeTransaction(async (queryable) => {
      ...
      if (!stored) {
        throw new DocumentNotFoundError(
          stageId,
          `@magicclass/storage: cannot putStage into missing document ${JSON.stringify(stageId)}`,
        );
      }
  ...
  async putScene(stageId: string, scene: TScene): Promise<void> {
    ...
    await this.writeTransaction(async (queryable) => {
      ...
      if (!stored) {
        throw new DocumentNotFoundError(
          stageId,
          `@magicclass/storage: cannot putScene into missing document ${JSON.stringify(stageId)}`,
        );
      }
      if (dslVersionOf(stored) !== DSL_VERSION) {
