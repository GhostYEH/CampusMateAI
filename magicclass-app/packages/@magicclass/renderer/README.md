...
Version `0.1.0` removes the experimental `@magicclass/renderer/editing` subpath.
Install `@magicclass/editor` and migrate editing imports to its explicit layers:

```ts
import { EditableSlideCanvas } from '@magicclass/editor/react';
import { EditableSlideCanvasWithUI } from '@magicclass/editor/ui';
```

Read-only rendering imports from `@magicclass/renderer` are unchanged.

## Install

```bash
pnpm add @magicclass/renderer
# or
npm install @magicclass/renderer
```
...
```tsx
import { SlideCanvas, type Slide } from '@magicclass/renderer';
...
```tsx
import { SlideRendererProvider, SlideCanvas, useSlideContext } from '@magicclass/renderer';
...
### Granular components — `@magicclass/renderer/elements`
...
} from '@magicclass/renderer/types';
```
...
export default {
  content: [
    './src/**/*.{ts,tsx}',
    './node_modules/@magicclass/renderer/dist/**/*.{js,cjs}',
  ],
};
```
...
[`@magicclass/importer`](../importer) converts `.pptx` files to the same `Slide[]` shape, so you can do `.pptx → @magicclass/renderer` end-to-end.
