/**
 * High-level entry: parsed pptxtojson(-pro) JSON → magic class canvas `Slide[]`.
 *
 ...
 * whole import either.
 */
import { normalizeSlideWith, type Slide } from '@magicclass/dsl';
...
/**
 * Convert a pre-parsed pptxtojson(-pro) `Output` JSON into magic class slides.
 *
...
const normalizeImportedSlide = normalizeSlideWith({
  ...
  onDropped: (_element, error) => {
    console.warn(
      `[@magicclass/importer] dropping element that fails DSL normalization: ${
    ...
    normalizeSlideWith({
      ...
      onDropped: (element, error) => {
        ...
        try {
          ...
          console.error('[@magicclass/importer] onWarning sink threw (ignored):', sinkErr);
        }
      },
    })(slide),
  );
...
/**
 * Parse a .pptx file and convert it into magic class canvas slides.
 *
