...
**magic class 扩展**：在 `parse()`（PPTX → 中间 JSON）之上新增了完整的 import pipeline —— `importPptx()` 直接吃 `.pptx` 文件、吐 magic class 画布需要的 `Slide[]`，并可选地把所有 base64 图片 / blob 媒体转交给你提供的 OSS 上传函数。详见下方「🚀 进阶用法」。
...
# 🚀 进阶用法：PPTX → magic class 画布 `Slide[]`

`parse()` 只完成「PPTX → 可读 JSON」这一步，元素的字段还是 PPT 语义（`type: 'text' | 'shape' | 'image' …`、单位 `pt`、媒体是 base64 / `blob:` URL）。

如果你需要的最终产物是 **magic class 画布直接可渲染的 `Slide[]`**（`PPTTextElement` / `PPTShapeElement` / `PPTImageElement` 等、单位 `px`、媒体替换为 OSS URL），用 `importPptx()`。
...
| `CanvasSlide` | magic class `Slide` 类型 | 用于消费方做类型注解 |
...
```ts
import { importPptx } from '@magicclass/importer';
...
```ts
import { importPptx, type OssUpload } from '@magicclass/importer';
...
```ts
import { parse, parsedToSlides } from '@magicclass/importer';
...
`maic-importer` 源码依赖 `pdfjs-dist`，其动态 `require()` 模式会被 Turbopack 拒绝。magic class 的做法：
...
```ts
import type * as PptxtojsonPro from '@magicclass/importer';
...
现生成（`pnpm --filter @magicclass/importer build` + `node scripts/sync-maic-importer.mjs`）。
