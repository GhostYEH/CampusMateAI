...
Composable slide-editing package for magicclass.

- `@magicclass/editor/core`: document operations, transactions, and undo/redo history.
- `@magicclass/editor/react`: editable slide interaction surface and rich-text editors.
- `@magicclass/editor/ui`: editor toolbars, insertion controls, and context menus.

## Dependencies

```text
@magicclass/editor
├─> @magicclass/renderer   (reuses the read-only slide renderer)
└─> @magicclass/dsl        (document and element data contracts)
```

`@magicclass/renderer` does not depend on `@magicclass/editor`, so the package boundary remains
...
`onTransaction` sink. `@magicclass/editor` owns built-in element adapters, insertion defaults,
...
```bash
pnpm add @magicclass/editor @magicclass/dsl @magicclass/renderer \
...
`node_modules/@magicclass/renderer/dist/**/*.{js,cjs}` as described in the
[`@magicclass/renderer` setup](https://www.npmjs.com/package/@magicclass/renderer#tailwind-4-setup).
...
import type { SlideContent } from '@magicclass/dsl';
import { applyEditorTransaction, createEditorHistory, type EditorTransaction } from '@magicclass/editor/core';
import { EMPTY_SELECTION, type Selection } from '@magicclass/editor/react';
import { EditableSlideCanvasWithUI, type EditorInsertItem } from '@magicclass/editor/ui';
