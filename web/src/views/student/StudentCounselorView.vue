      ...
      <aside class="counselor-reference-right">
        <section class="counselor-panel services-panel">
          ...
          <div class="reference-service-list">
            <button v-for="item in quickServices" :key="item.label" @click="send(`请介绍${item.label}相关的办理流程`)"><span class="service-reference-icon"><UiIcon :name="item.icon" :size="20" /></span><span><strong>{{ item.label }}</strong><small>{{ item.hint }}</small></span><UiIcon name="PhCaretRight" :size="15" /></button>
          </div>
        </section>
