...
const messages = ref([{ role:"assistant", text:"你好，我是 AI 导员小夏。你可以问我课程流程、奖助政策、校园服务等问题。回答基于校园知识库，仅供辅助参考。" }]);
...
async function runExtract() {
  ...
  try {
    extracted.value = await extractNotice(noticeText.value);
...
<template>
  ...
  <main v-else class="feature-page page-enter">
    <div class="page-title"><div><h1>{{ titleMap[section] || "校园服务" }}</h1><p>{{ section === "counselor" ? "基于校园知识库的事务问答，回答会标注检索证据。" : "集中处理与当前模块相关的校园事务。" }}</p></div><span class="mode-badge"><i></i>{{ store.backendOnline ? "后端已连接" : "后端未连接" }}</span></div>
    ...
    <section v-else-if="section === 'study'" class="study-layout">
      ...
      <div class="data-panel expression-panel"><div class="section-head"><h2>表情识别辅助</h2><em>CNN 接口预留</em></div><div class="expression-state"><span class="soft-icon large"><UiIcon name="PhSmiley" :size="34" /></span><div><strong>当前表情识别尚未启用</strong><p>识别结果仅供辅助参考，不代表心理状态或医学判断。</p></div></div><div class="alert info">接入模型后，低置信度时会显示“暂时无法稳定判断当前表情”，且不会触发情绪安慰。</div></div>
    ...
    <section v-else-if="section === 'profile'" class="profile-grid">
      ...
      <div class="surface settings"><h2>偏好设置</h2><div class="status-setting"><strong>后端服务</strong><small>{{ store.backendOnline ? "已连接 FastAPI，业务请求将使用真实服务" : "未连接 FastAPI，请先启动后端服务" }}</small></div><label class="switch-row"><span><strong>减少动态效果</strong><small>减少页面切换与卡片进入动画</small></span><input type="checkbox" :checked="store.reduceMotion" @change="store.setReduceMotion($event.target.checked)" /></label><label class="switch-row"><span><strong>截止提醒</strong><small>Web 端使用站内提醒</small></span><input type="checkbox" checked /></label></div>
    </section>
