...
- 个人中心提供"恢复演示数据"与"清除本地数据"入口,带二次确认
- 损坏数据自动降级,启动失败也不阻断应用
...
- 后端不可用时自动降级为本地 Mock 数据,演示模式始终可用
...
- **CNN 模型训练**: PyTorch + FER2013 — 当前仅 Mock
- **LiteRT / 原生真实推理**: 表情识别仍为预留接口
