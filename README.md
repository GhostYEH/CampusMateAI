...
Android 的**专注自习（Focus）**是唯一正式入口，已接入 CameraX、ML Kit 本地人脸检测和 LiteRT 表情分类，保留 Mock/Real 双模式。只有用户主动开启“学习状态辅助”、明确授予相机权限、专注计时运行且页面在前台时，才在本机内存分析；暂停、关闭、离开页面或进入后台会立即暂停并解绑摄像头。表情识别仅在专注模式运行，短休息和长休息期间自动暂停分析。画面不保存、不上传、不写日志。
