...
test('refuses to start without an internal secret', () => {
  const result = startService({ MAGICCLASS_INTERNAL_SECRET: '', MAGICCLASS_DATABASE_URL: ':memory:' });
  assert.notEqual(result.status, 0);
  assert.match(`${result.stdout}${result.stderr}`, /MAGICCLASS_INTERNAL_SECRET is required/);
});

test('refuses to start without a database location', () => {
  const result = startService({ MAGICCLASS_INTERNAL_SECRET: SECRET, MAGICCLASS_DATABASE_URL: '' });
  assert.notEqual(result.status, 0);
  assert.match(`${result.stdout}${result.stderr}`, /MAGICCLASS_DATABASE_URL is required/);
});

test('comes up on a complete configuration and serves authenticated readiness', async () => {
  const directory = mkdtempSync(join(tmpdir(), 'magicclass-startup-'));
  const port = 4399;
  const child = spawn(node, ['--experimental-strip-types', main], {
    env: {
      ...process.env,
      MAGICCLASS_INTERNAL_SECRET: SECRET,
      MAGICCLASS_DATABASE_URL: join(directory, 'service.db'),
      MAGICCLASS_HOST: '127.0.0.1',
      MAGICCLASS_PORT: String(port),
      // Keep this startup contract independent of a developer's local .env.
      MAGICCLASS_PROVIDER_BASE_URL: '',
      MAGICCLASS_PROVIDER_API_KEY: '',
      MAGICCLASS_TTS_BASE_URL: '',
      MAGICCLASS_TTS_API_KEY: '',
    },
