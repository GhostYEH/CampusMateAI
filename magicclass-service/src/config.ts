...
/**
 * Root of the `magicclass-service` package, derived from this module's URL so the
...
function positiveInteger(value: string | undefined, fallback: number) {
  ...
  if (!Number.isInteger(parsed) || parsed <= 0 || parsed > 65535) {
    throw new ConfigError('MAGICCLASS_PORT must be a valid TCP port');
  }
  return parsed;
}
...
export function resolveDatabasePath(raw: string | undefined, serviceRoot = SERVICE_ROOT) {
  ...
  if (!value) throw new ConfigError('MAGICCLASS_DATABASE_URL is required');
  ...
  if (scheme && scheme[1].length > 1) {
    throw new ConfigError('MAGICCLASS_DATABASE_URL must point to a local SQLite file');
  }
...
function readTtsVoice(value: string | undefined): string {
  ...
  if (voice.length > 80) throw new ConfigError('MAGICCLASS_TTS_VOICE must be at most 80 characters');
  // eslint-disable-next-line no-control-regex
  if (/[\u0000-\u001f]/.test(voice)) throw new ConfigError('MAGICCLASS_TTS_VOICE must not contain control characters');
  return voice || '苏打';
}

function readRenderEndpoint(env: Record<string, string | undefined>): RenderConfig | undefined {
  const baseUrl = optionalEndpointUrl(env.MAGICCLASS_RENDER_SERVICE_URL, 'MAGICCLASS_RENDER_SERVICE_URL');
  const token = (env.MAGICCLASS_RENDER_SERVICE_TOKEN ?? '').trim();
  if (!baseUrl && !token) return undefined;
  if (!baseUrl || !token) throw new ConfigError('MAGICCLASS_RENDER_SERVICE_URL and MAGICCLASS_RENDER_SERVICE_TOKEN must be configured together');
  const rawTimeout = (env.MAGICCLASS_RENDER_SERVICE_TIMEOUT_SECONDS ?? '').trim();
  const seconds = rawTimeout ? Number(rawTimeout) : 120;
  if (!Number.isFinite(seconds) || seconds <= 0 || seconds > 600) throw new ConfigError('MAGICCLASS_RENDER_SERVICE_TIMEOUT_SECONDS must be between 1 and 600');
  return { baseUrl, token, timeoutMs: seconds * 1000 };
}

export function loadConfig(env = process.env): ServiceConfig {
  const internalSecret = (env.MAGICCLASS_INTERNAL_SECRET ?? '').trim();
  if (!internalSecret) {
    // Fail closed: an unauthenticated internal service is worse than no service.
    throw new ConfigError('MAGICCLASS_INTERNAL_SECRET is required');
  }
  const externalCdnUrl = (env.MAGICCLASS_EXTERNAL_CDN_URL ?? '').trim();
  const provider = readEndpoint(env, 'MAGICCLASS_PROVIDER', { defaultTimeoutSeconds: 120 });
  const ttsEndpoint = readEndpoint(env, 'MAGICCLASS_TTS', {
    defaultModel: 'mimo-v2.5-tts',
    defaultTimeoutSeconds: 180,
    maxTimeoutSeconds: 600,
  });
  const tts: TtsConfig | undefined = ttsEndpoint ? { ...ttsEndpoint, voice: readTtsVoice(env.MAGICCLASS_TTS_VOICE) } : undefined;
  const render = readRenderEndpoint(env);
  return {
    host: (env.MAGICCLASS_HOST ?? '').trim() || '127.0.0.1',
    port: positiveInteger(env.MAGICCLASS_PORT, 4010),
    databasePath: resolveDatabasePath(env.MAGICCLASS_DATABASE_URL),
    internalSecret,
