...
export const AUDIENCE = 'magicclass-service';
...
function secretFrom(options: VerifyOptions) {
  const secret = options.secret ?? process.env.MAGICCLASS_INTERNAL_SECRET;
  if (!secret) {
    throw new ServiceAssertionError('MAGICCLASS_INTERNAL_SECRET is not configured', 'assertion_missing');
  }
  return secret;
}
