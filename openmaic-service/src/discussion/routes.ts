...
export function createDiscussionRoutes(options: { database: ServiceDatabase; available?: boolean }): RouteDefinition[] {
  const available = options.available ?? false;
  return [{
    method: 'POST', pattern: '/internal/courses/:courseId/discussion', scopes: [DISCUSSION_SCOPE], courseScoped: true, capabilities: [DISCUSSION_CAPABILITY],
    handler: (request): RouteResponse => {
      try {
        requiredPrompt(parseBody(request).prompt);
        if (!available) return { status: 503, body: { error: 'provider_unavailable', message: 'multi-agent provider is unavailable' } };
