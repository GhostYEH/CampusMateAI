...
熔断状态机不只是"计数 + 时间戳"。**代次只在真正的状态迁移时推进**：

| 状态迁移 | 触发条件 | 推进代次 |
| --- | --- | --- |
| CLOSED → OPEN | 失败数达到阈值（`_record_failure`） | ✅ |
| OPEN / HALF_OPEN → CLOSED | 半开探测成功（`_record_success`） | ✅ |
| HALF_OPEN → OPEN | 半开探测失败后重新打开 | ✅ |
| （OPEN 已打开且冷却未结束）再失败 | 不构成迁移，也不刷新冷却起点 | ❌ |
| **CLOSED 状态下的一般成功** | 只清零失败计数 | ❌ |

最后一行是关键：CLOSED 下的成功**不是**迁移。如果它推进代次，同一批并发请求里
只要有一个先成功，其余携带旧代次的失败就会被判为"陈旧结果"丢弃 ——
候选服务**部分故障**时（一个成功、其余失败）熔断永远打不开。
该场景有专门的并发回归测试（`test_closed_concurrent_success_must_not_disable_later_failures`）。

一次调用在放行时记下当时的代次，返回时只有代次仍然一致，才允许改动熔断状态：
...
> 测试夹具注意：闸门式假模型必须同时放宽 `timeout_ms` 与 `concurrency_limit`
> （默认 1500ms / 2）。否则第 3 个请求会卡在信号量上、先到的请求先超时，
> 测到的就不是"同一批并发"而是"排队 + 超时"。
...
`docs/multi-worker-readiness.md` —— 那份清单还包含另外两个同样只在多 worker 下才暴露的问题
（检索索引跨 worker 陈旧、Chaoxing 同步互斥失效），是水平扩展前的完整前置条件。
