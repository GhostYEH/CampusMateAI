...
| 切片 | 内容 | 状态 | 提交号 | 备注 |
| --- | --- | --- | --- | --- |
| P0-A | 模板根页视觉、课程绑定生成与最近课堂闭环 | 进行中 | — | 双浏览器实测确认现有 `/courses` 不是模板首页等价实现；本切片先于 P1-C |
...
- 下一个切片：**P0-A**
- 打算怎么做：按 `03-待办.md` 的 P0-A 契约，双浏览器对照模板根页和 CampusMate `/courses`，
  先以真实课程归属、幂等任务和最近课堂 API 接通模板式首页，再做同分辨率截图与提交闭环。
- 卡住的地方：无
...
- P1-B：新增受限 `slide.element.move` 命令，只更新既有 slide 元素的有限 `left/top`，保留
  ...
  错误。
- 2026-09-20 P0-A：用户以参考/当前两张实拍图指出首页未对齐。双独立浏览器 context 已实测：
  模板根页为居中生成工作台（内部 composer 约 776px 宽），当前 `/courses` 为“进入课堂 +
  最近内容 + 工具 Tab + 工作台 CRUD + 31 门课程”业务页。修订 `03-待办.md`，P0-A 必须先于
  P1-C；后续验收必须比较两个项目的真实浏览器页面，不能只跑 CampusMate 单站 E2E。
