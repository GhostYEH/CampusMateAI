...
本目录用 **`$REF`** 指代参考项目根目录：OpenMAIC v1.0.x 源码
...
**用户最初的核心诉求已经交付**：「我的课程 → 进入课堂」现在直接进入学习态，左侧场景栏
带真实幻灯片缩略图，中间是真实的 16:9 幻灯片 / 测验 / 互动 / 项目式学习，底部是参考
项目那条 192px 圆桌面板（能真跑多智能体讨论）。

**剩下的**主要是「工作台编辑器」（现在还是 JSON 文本框）以及若干版式与行为补齐。
详见 `03-待办.md`。
...
| 受管服务单测 | `npm test`（在 `openmaic-service/`）→ 279 passed / 0 failed |
| 后端 openmaic 用例 | `pytest tests -k openmaic`（在 `backend/`）→ 370 个用例 |
| 浏览器验收 | `run_openmaic_workbench_e2e.py` → 11 步全过 |
...
| 词 | 指什么 |
| --- | --- |
| **参考项目 / `$REF`** | OpenMAIC v1.0.x 源码，见上文。**只读**。 |
| **受管服务** | `openmaic-service`，仓库内自带的 Node 运行时（:4010），承载 DSL / 编辑 / 播放 / 生成 / 讨论。 |
| **网关** | `backend`，FastAPI（:8000），负责身份、课程权限与短时断言签发。 |
| **移植层** | `webreact/src/maic/**`，从参考项目逐字搬过来的界面代码。 |
| **集成层** | `webreact/src/components/openmaic/**`，把移植层接到 CampusMate 真实数据上。 |
