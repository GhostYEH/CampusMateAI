...
本文是 CampusMate 内部受管 magic class 运行时的部署说明。用户入口始终是 CampusMate Web；magic class 不作为独立产品首页、独立登录或独立课程库暴露。
...
- Provenance、能力清单和 SHA-256 清单见 `third_party/magicclass/`

受管服务只接入经过审计的必要 DSL、renderer、editor、importer/exporter 和服务端能力；不复制 `.git`、`node_modules`、`.next`、运行时数据、日志、密钥或本机配置。

### 入库的上游应用：`magicclass-app/`

除受管服务外，仓库根目录 `magicclass-app/` 还**入库了同一份 v1.0.3 的完整应用**
（再叠加一层可重放的品牌补丁，见下），
供导航栏「学习空间」以独立进程运行（见 `docs/magicclass-migration/01-约束.md` 的
"例外：导航栏「学习空间」"）。它与受管服务是两条独立链路，不要互相顶替：

- **来源同一**：tag `v1.0.3`、commit `e693e11a81644f84c258df73dbda378643520a62`，
  与上表一致；可对照 `third_party/magicclass/source-manifest.sha256` 核对。
- **品牌补丁**：用户可见品牌词换成 `magic class`、npm 身份换成 `magicclass` /
  `@magicclass/*`（含 `packages/@magicclass/` → `packages/@magicclass/` 目录改名），
  规则只有 `scripts/magicclass-brand.mjs` 这一处。清单仍然钉住**上游**字节：
  `node scripts/magicclass-brand.mjs verify` 会把补丁逆向还原再逐文件比哈希，
  通过即说明这份树除了该文件定义的改动与上游一致。刻意保留的部分：`MAGICCLASS_*`
  环境变量、`X-MagicClass-*` 响应头、数据库/容器/MIME 等内部管道名、上游自带 Markdown、
  上游 MIT `LICENSE`，以及 `render-service/` 那两个来自 npm 的已发布 `@magicclass/*` 依赖。
...
- **启动前必须安装依赖**：`cd magicclass-app && pnpm install`。该仓库是 pnpm workspace，
...
- **两个环境变量不能缺**，缺了都不会报错、只会表现为"页面不对"：
  ...
  | 变量 | 缺了会怎样 | 谁给 |
  | --- | --- | --- |
  | `ALLOWED_FRAME_ANCESTORS` | 应用发 `X-Frame-Options: SAMEORIGIN` + `frame-ancestors 'self'`，**跨源内嵌被浏览器拒掉，「学习空间」一片空白**（无控制台报错） | 本地由 `scripts/magicclass-local.mjs` 注入本站两个 Origin |
  | `npm_package_version` | `/api/health` 自报 `0.1.0`，落在后端 `MAGICCLASS_ALLOWED_VERSIONS`（默认 `>=1.0.0 <2.0.0`）之外，「学习空间」以"版本不一致"拒绝加载 | 同上，取入库 `magicclass-app/package.json` 的版本 |
...
开发机上另有一份 magic class 运行副本（1.0.1）。它**不是**上述来源，也不能作为 provenance：

- `package.json` 版本为 `1.0.1`，比固定的 `v1.0.3` 旧；
- 没有 `.git` 元数据，无法证明 commit；
- 含构建产物、运行时数据和本地凭据文件。

逐文件比对（`third_party/magicclass/source-manifest.sha256`，2976 条）：2760 条一致、166 条不同、50 条缺失。
其中 `packages/@magicclass/dsl/src/**` 全部 16 个文件与 v1.0.3 **逐字节一致**，因此 DSL 契约的移植结论对该运行副本同样成立。
它可以被当作只读的"实际运行行为"参考，但任何提交产物、清单或 NOTICE 都不得引用它，也不得把它的路径写进仓库文件。
入库的 `magicclass-app/` 不是这一份：前者有确定的 tag 与 commit，后者没有。
...
CampusMate Web (React 18)
        ...
        ▼                                            ▼
CampusMate FastAPI                          上游 magic class 应用
  身份、课程、作业、资料、权限、上下文、审批、审计      magicclass-app，独立进程、独立 Origin
        │ 短时 user/course/scope 断言
        ▼
magicclass-service (private Node runtime)
...
`docs/magicclass-migration/01-约束.md`；要点是它不并入本站身份、不反向代理、
公开 Origin 未配置时不渲染内嵌。
...
- `MAGICCLASS_FUSION_ENABLED`：融合运行时总开关，默认关闭。
- `MAGICCLASS_SERVICE_URL`：仅服务端可见的内部 origin。
- `MAGICCLASS_INTERNAL_SECRET`：短时断言共享密钥，只通过 secret provider 注入。
- `MAGICCLASS_SERVICE_TIMEOUT_SECONDS`：内部调用超时。
- 既有 `MAGICCLASS_*` 课堂适配配置继续由 `backend/app/services/magicclass/**` 使用。

### 三套地址，不要互相顶替

| 变量 | 本地默认 | 谁看得见 | 指向 |
| --- | --- | --- | --- |
| `MAGICCLASS_SERVICE_URL` | `http://127.0.0.1:4010` | 仅服务端 | 仓库内自研受管服务（融合链路只看它） |
| `MAGICCLASS_BASE_URL` | `http://127.0.0.1:3000` | 仅服务端 | 上游应用 `magicclass-app` 的内部地址 |
| `MAGICCLASS_EMBED_ORIGIN` | `http://127.0.0.1:3000` | **浏览器可见** | 同一个上游应用的公开 Origin，经 `/learning-space` 跨源内嵌 |

本地开发三者同机，所以后两者取值相同；**生产部署下这两者必须拆开**——
内部走容器网络名（如 `http://magicclass:3000`），公开走对外子域
（如 `https://classroom.example.edu`，且必须 HTTPS）。

`MAGICCLASS_ENABLED` + `MAGICCLASS_BASE_URL` + `MAGICCLASS_EMBED_ORIGIN` 三个键齐备时，
导航栏「学习空间」才可用；`MAGICCLASS_EMBED_ORIGIN` 缺失时后端不下发任何公开地址，
页面 fail-closed 不渲染内嵌。`MAGICCLASS_BASE_URL` 同时是**课程级互动课堂适配层**的
上游地址，因此开启「学习空间」也会一并点亮课程详情页里既有的互动课堂入口。

服务模板位于 `magicclass-service/.env.example`。除地址/密钥/数据库外，服务还接受可选的
上游 provider 配置；两组变量必须成对出现，缺一在启动即报错（fail-closed）：

- `MAGICCLASS_PROVIDER_BASE_URL` / `MAGICCLASS_PROVIDER_API_KEY` / `MAGICCLASS_PROVIDER_MODEL`：
  OpenAI 兼容的 chat-completions 上游，用于学习内容生成与圆桌讨论。
  未配置时生成退回内置 local template，讨论如实返回 `provider_unavailable`。
- `MAGICCLASS_TTS_BASE_URL` / `MAGICCLASS_TTS_API_KEY` / `MAGICCLASS_TTS_MODEL` / `MAGICCLASS_TTS_VOICE`：
  语音合成上游（MiMo TTS：`mimo-v2.5-tts`，默认音色 `苏打`，响应为 base64 WAV）。
  未配置时语音合成如实返回 `provider_unavailable`。
- `MAGICCLASS_RENDER_SERVICE_URL` / `MAGICCLASS_RENDER_SERVICE_TOKEN`：私有 MP4 render-service；
...
```powershell
pwsh -NoProfile -File magicclass-service/scripts/start.ps1
```
...
| 进程 | 默认端口 | 作用 |
| --- | --- | --- |
| `magicclass-service` | 4010 | 受管 Node 运行时（DSL、编辑器、播放器、导入导出、受限 Provider） |
| FastAPI | 8000 | 网关：身份、课程权限、断言签发 |
| `magicclass-app` | 3000 | 上游 magic class 应用，承载导航栏「学习空间」（独立 Origin） |
| Vite | 5174 | Web 客户端；`/api` 反代到 FastAPI |

仓库根目录提供了一条可执行的启动与诊断路径（`scripts/magicclass-local.mjs`）：

```bash
cd magicclass-app && pnpm install            # 只需一次；含 postinstall 构建 workspace 包
node scripts/magicclass-local.mjs env        # 幂等补齐 backend/.env 与 magicclass-service/.env
node scripts/magicclass-local.mjs doctor     # 只读诊断：Node 版本、依赖、密钥一致性、端口与健康
node scripts/magicclass-local.mjs start      # 按序拉起四个进程
```

四个容易混淆、也最容易造成"服务都起来了却仍然 503"的点：

1. **三套地址不能混用。** 见上节表格：`MAGICCLASS_SERVICE_URL` 是融合网关用的受管服务
   （默认 4010）；`MAGICCLASS_BASE_URL` 是上游应用的服务端内部地址（默认 3000）；
   `MAGICCLASS_EMBED_ORIGIN` 是下发给浏览器的公开 Origin。把 3000 填进
   `MAGICCLASS_SERVICE_URL` 只会得到一个连不上的网关。
2. **两侧密钥必须一致。** `backend/.env` 的 `MAGICCLASS_INTERNAL_SECRET` 与
   `magicclass-service/.env` 的同名变量必须是同一个值，否则每次调用都会被断言拒绝
   （表现为 `state=unavailable`、`reason=assertion_rejected`）。
3. **开关与地址/密钥要一起给。** 只开 `MAGICCLASS_FUSION_ENABLED=true` 而不配地址/密钥时，
   `Settings` 在校验期就拒绝启动（fail-closed），不会等到请求时才失败。
4. **「学习空间」不反向代理，所以它必须有自己的 Origin。** `MAGICCLASS_EMBED_ORIGIN`
   不能等于 Web 站点自身的地址（同源内嵌会被前端硬拒），也不能只配内部地址而指望
   浏览器够得着。上游应用用 `next dev` 起（不预构建），首次打开该页要现编译，慢是正常的。
   反向同理：上游应用的 `frame-ancestors` 只放行 Web 客户端声明过的 Origin（本地为
   `127.0.0.1` / `localhost:5174`），所以 Web 客户端必须跑在 5174 上——Vite 已用
   `strictPort` 固定端口；若站点跑在别的端口（例如重复启动漂移到 5175），浏览器会以
   CSP `frame-ancestors` 违规拒掉 iframe，「学习空间」打不开并只在控制台报错。
5. **3000 端口上必须是我们入库的这一份。** 同一台机器很容易还开着另一份 magic class
   ...
   `MAGICCLASS_BASE_URL` 与 `MAGICCLASS_EMBED_ORIGIN`。
...
- CampusMate `GET /api/v1/magicclass/fusion/status`：唯一面向浏览器的能力状态。
  响应里的 `state` 是唯一判据，取值 `disabled` / `unavailable` / `degraded` / `ready`：

  | state | 含义 | `capabilities` |
  | --- | --- | --- |
  | `disabled` | `MAGICCLASS_FUSION_ENABLED=false`，网关不去连服务 | 空 |
...
- 503 不是一句话，而是带稳定原因的。网关把每一种"不可用"翻译成
  `MAGICCLASS_FUSION_UNAVAILABLE` + `details.reason`，浏览器据此区分"稍后重试"与"找管理员"：
...
- `GET /api/v1/magicclass/fusion/recent?limit=20`：当前用户**所有可见课程**的最近学习内容，
...
服务离线时，课程、作业查看、保存和提交仍由 CampusMate 正常提供；magic class 入口显示不可用或降级状态，不使用静态假数据。
...
- `MAGICCLASS_SERVICE_URL` 启动时拒绝凭据、query、fragment 和路径；后续出站调用仍需遵守 SSRF allowlist。
...
完整逐项状态以 `docs/magicclass-capability-matrix.md` 为准。当前状态是“部分完成”：
...
都是结构性的：一是入口原先把工作台失败降级成了 `/counselor`，绕过了 magic class 的持久化
生成链路；二是它把失败写进了页级 `error`，页级 error 由 `AsyncState` 消费，于是整页被
替换。修复后，课程入口只有在服务真实上报 `workspace + generation` 时才创建/复用工作台，
随后进入原生 magic class 工作台并自动提交生成；服务异常只显示在输入区，绝不跳小助手或伪造
工作台关联。以下各项经 `run_magicclass_courses_e2e.py` 在真实三服务上验证：
...
- 运行中停掉受管服务后点击提交：得到 **503** 与局部中文错误
  （`暂时连不上受管 magic class 服务，请稍后重试。`），**31 门课程列表原样保留**、
...
- 课程页框架是 `height: calc(100dvh - 112px)` + `overflow: hidden`（它只允许内部列表自己滚）。
  ...
  现在 `.magicclass-home` 是唯一滚动容器，折叠区已可达（E2E 用几何断言兜住）。
- `.magicclass-workspace-item` 是三列栅格（图标/文案/操作），却平铺了 6 个子节点，
  操作被自动放进隐式第二行：第一列只有 32px，"进入工作台"被压成**每行一个字**，
  中间的"内容"被 `minmax(0,1fr)` 拉成一整条。现在操作收进 `.magicclass-workspace-item__actions`。

**仍未执行浏览器验收的部分**（保持"部分完成"，不得用源码测试冒充浏览器证据）：
C 切片的内容发现（文件夹树与站内搜索：`magicclass-service/src/discovery/**`、网关
`magicclass_discovery.py`、Web `DiscoveryPanel.jsx`，含工作台归档）已落地并通过
服务端/网关/Web 自动化测试，但其面板在本轮只验证了**渲染与可达**，文件夹增删改、
搜索、归档的浏览器交互未执行；E 切片中的课程资料部分
（上传/解析/引用：`magicclass-service/src/material/**`、`material_extraction.py`、
`magicclass_materials.py`、`MaterialsPanel.jsx`）同样已落地并通过三层自动化测试，
**尚未执行浏览器验收；原始字节在 2 MiB 内受限落库，并可随 stage 引用进入归档**；
`.maic.zip` 的单份 stage 导出与导入（`magicclass-service/src/archive/**`、
`magicclass_archive.py`、`archiveModel.js`、`WorkspacePanel.jsx`）已打通服务端、网关与 Web
