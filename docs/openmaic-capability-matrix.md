...
|---|---|---|---|---|---|---|---|---|
| 融合首页 | `app/page.tsx`、`components/discovery/**` | DSL、workspace 数据 | `webreact/src/pages/ParityPages.jsx` | `/courses` 聚合课程与 workspace 摘要 | JWT；仅当前用户课程 | `webreact/tests/openmaic-home.test.mjs` | `node --test webreact/tests/openmaic-home.test.mjs`; Vite build | 已验证（`fb6d4081`；文件夹/导入等未实现入口保持禁用） |
| 最近项目 | `app/page.tsx`、`lib/utils/database.ts` | workspace 映射 | `webreact/src/features/openmaic/**` | 归属用户的最近 stage | userId 归属 | `webreact/tests/openmaic-home.test.mjs` | `node --test webreact/tests/openmaic-home.test.mjs` | 部分完成（`fb6d4081`；展示现有课程课堂记录，不冒充 native stage） |
...
| 选课与切课 | — | 课程详情、会话作用域 | `CourseDetailPage.jsx`、`CounselorPage.jsx`、`data/epochGuard.js` | 当前 `courseId` 显式传递 | JWT + course permission | `interactive-classroom.test.mjs`、`classroom-poll-scope.test.mjs`、`classroom-job-machine.test.mjs` | 精确 node test | 部分完成（已有课程作用域/epoch；尚未有独立 OpenMAIC workspace） |
...
| 安全闭环 | `middleware.ts`、upload routes、renderer | JWT/origin/SSRF/sanitize | backend + service security modules | redacted errors, bounded input | user/course/session/mapping | `openmaic-service/tests/*.mjs`、OpenMAIC backend tests | Node tests + Python compile（pytest 环境待修复） | 部分完成（`c688bdbf`；仍需 renderer/import/security 全链路复核） |
