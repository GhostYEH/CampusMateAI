# magic class 与 CampusMate 完整融合设计

本文件与 `docs/magicclass-capability-matrix.md`、`docs/magicclass-deployment.md` 共同描述当前唯一有效边界。它不把现有课堂适配层称为完整迁移。
...
CampusMate 保留四项事实源：JWT 身份、课程/作业、资料和权限。magic class 运行时只提供经过审计的内容 DSL、渲染、编辑、导入导出和 Provider 能力。用户在 `/courses`、课程详情、工作台和作业页面内完成全部操作，不跳转独立 magic class 首页，不出现第二导航、账号或课程库。
...
- `backend/app/services/magicclass/**` 的 HTTP 客户端、课程上下文、生成编排、状态/重试/历史和组成解析。
- `backend/app/api/routes/magicclass_classroom.py` 的 JWT、课程权限、状态、计划、生成、轮询、历史、组成和重试接口。
...
magic class 不存在的取消接口不被伪造：生成任务的“停止查看”只停止本地轮询；重试表示 CampusMate 按原始用户输入提交新任务；恢复表示从 CampusMate 历史重新打开归属内容。
...
3. C：课程映射、快速询问和工作台。**进行中**：工作台的持久化
   （`magicclass-service/src/workspace/**`）、FastAPI 网关
   （`backend/app/api/routes/magicclass_workspaces.py`，含 `Idempotency-Key` /
   `If-Match` 强制、412→409 翻译、内部地址不外泄）与课程内工作台面板
   （`webreact/src/components/magicclass/WorkspacePanel.jsx`，按真实 capability
   开关）已落地并验证；原生工作台、生成恢复、编辑器与播放器已接入，快速询问入口会复用/创建
   workspace 并把其归属传入 SSE；服务端对 workspace 做归属复核。
   本轮补齐了**内容发现**这一层：文件夹树与站内搜索
   （`magicclass-service/src/discovery/**` + `backend/app/api/routes/magicclass_discovery.py`
   + `webreact/src/components/magicclass/DiscoveryPanel.jsx`），并让工作台可以归档到
   文件夹（`folder_id`）。文件夹与搜索的浏览器验收仍未执行。
4. D：DSL、编辑器和播放器。**进行中**：DSL 已移植并有版本/迁移/净化/校验；
   编辑器本轮接通命令层（`magicclass-service/src/dsl/commands.ts`）、内部路由
   （`src/editor/routes.ts`）、FastAPI 网关（`magicclass_editor.py`）与 Web
...
5. E：材料、导入导出、高级能力和 Provider 设置。**进行中**：材料上传/解析/引用已接通
   （`magicclass-service/src/material/**` 的迁移 4 与路由、`backend/app/services/magicclass/material_extraction.py`
   的取字策略、`magicclass_materials.py` 网关、`MaterialsPanel.jsx`）。三条边界在这里定死：
   ...
   编解码器（`magicclass-service/src/archive/zip.ts`）只用 `node:zlib`，因为该服务刻意保持
