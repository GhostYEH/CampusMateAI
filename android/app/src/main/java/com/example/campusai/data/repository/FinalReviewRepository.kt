...
class FinalReviewRepository(
    ...
    suspend fun generatePlan(campaignId: String): Result<Unit> = runCatching {
        val key = AgentIdempotency.stableKey(userIdProvider(), "fr_generate_plan", campaignId)
        val response = api.agentGenerateFinalReviewPlan(campaignId, key)
    ...
    suspend fun activatePlan(campaignId: String): Result<Unit> = runCatching {
        val key = AgentIdempotency.stableKey(userIdProvider(), "fr_activate", campaignId)
        val response = api.agentActivateFinalReviewPlan(campaignId, key)
    ...
    suspend fun completeDailyItem(itemId: String, evidence: FinalReviewEvidenceRequest): Result<Unit> = runCatching {
        ...
        val response = api.agentCompleteFinalReviewDailyItem(itemId, evidence, key)
    ...
    suspend fun analyzeAdjustments(campaignId: String): Result<Unit> = runCatching {
        ...
        val response = api.agentAnalyzeFinalReviewAdjustments(campaignId, key)
